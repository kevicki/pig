function alertShow(meg, but_num, but_left, but_right, left_fn, right_fn) {
    if (!$("#alert_bg").length) {
        var alertHtml = "<div id='alert_bg' style=\" width: 100%; position: absolute; top:0px; left: 0px; z-index: 1000; background: rgba(0, 0, 0, 0.1)\"></div>" + '<div id="alert_box">' + '<div id="show_mes">淇℃伅</div>' + '<div id="but_div">' + "</div></div>";
    ...
    var _default = {
        ...
        success: function(data, index) {
            if (data.code === 1) {
                ...
                $("#imgShow" + index).find("img").attr("src", data.url).attr("url", data.origin);
				$("#imgShow" + index).find("input").val(data.origin);
                _this.getItemCount()
            } else {
                alertShow(data.msg, 1, "鍏抽棴", "", cancel_but);
                $("#imgShow" + index).remove();
                _this.getItemCount()
            }
        },
        error: function(index) {
            alertShow("璇锋眰澶辫触", 1, "鍏抽棴", "", cancel_but);
            $("#imgShow" + index).remove();
            _this.getItemCount()
        },
        timeoutfn: function(index) {
            alertShow("涓婁紶澶辫触锛岃妫€鏌ョ綉缁滅幆澧冩垨鍘嬬缉鍥剧墖瀹归噺鍚庝笂浼�", 1, "鍏抽棴", "", cancel_but);
            $("#imgShow" + index).remove();
            _this.getItemCount()
        },
        errFn: function(err) {
            alertShow(err, 1, "鍏抽棴", "", cancel_but)
        }
    };
    this.opt = $.extend({},
    _default, cfg);
...
ImgUpload.prototype = {
    ...
    init: function() {
        ...
        function() {
            ...
            if (file.size > maxSize) {
                ...
                err = "璇蜂笂浼� jpg/jpeg/gif/png鏍煎紡鐨勫浘鐗囷紒"
            }
            if (err) {
            ...
            _this.liIndex += 1;
            var loadingStr = '<li class="upload_item loading_item" id="imgShow' + _this.liIndex + '">' + '<a href="javascript:void(0);" class="upload_delete" title="鍒犻櫎"></a>' + '<img src="" class="upload_image loading_img" /><input type="hidden" name="inputimg[]" value=""><br>' + "</li>";
    ...
    submit: function(files, index) {
        ...
        if (file == "") {
            alertShow("璇峰崌绾ф祻瑙堝櫒鎴栭€夋嫨鍏朵粬娴忚鍣�", 1, "鍏抽棴", "", cancel_but)
        } else {
            $.ajax({
                ...
                success: function(ret) {
                    ...
                    if (imgdata.url && imgdata.url.indexOf("jpg") != -1) {
                        imgdata.url =imgdata.origin = imgdata.url;
                    }
