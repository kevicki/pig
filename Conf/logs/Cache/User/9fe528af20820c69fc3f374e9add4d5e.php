...
<div class="content">
	<div class="cLineB">
		<h4><?php if($type == 1): ?>添加幻灯片<?php else: ?>添加背景图<?php endif; ?></h4>
	...
	<div class="msgWrap">
		<form class="form" method="post"   action="<?php echo U('Store/flashadd',array('type' => $type));?>"  target="_top" enctype="multipart/form-data" >
			...
			<TABLE class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellSpacing="0" cellPadding="0" width="100%">
				<TBODY>
					<TR>
					  <TH valign="top"><label for="keyword"><?php if($type == 1): ?>幻灯片描述：<?php else: ?>背景图描述：<?php endif; ?></label></TH>
					  <TD><input type="input" class="px" id="keyword" value="<?php echo ($flash['info']); ?>" name="info" style="width:500px" ><br />
					                  30个字简短分类描述，可为空   </TD>
					  <TD>&nbsp;</TD>
					</TR>
					<TR>
					  <TH valign="top"><label for="keyword"><?php if($type == 1): ?>幻灯片图片：<?php else: ?>背景图图片：<?php endif; ?></label></TH>
					  <TD><input type="input" class="px" id="img" value="<?php echo ($flash['img']); ?>" name="img" style="width:500px" >  <script src="/tpl/static/upyun.js?2013"></script><a href="###" onclick="chooseFile('img','<?php if($type == 1): ?>focus<?php else: ?>background<?php endif; ?>')" class="a_upload">选择</a> <a href="###" onclick="upyunPicUpload('img',<?php if($type == 1): ?>700,420<?php else: ?>640,1136<?php endif; ?>,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('img')">预览</a> 建议图片(<?php if($type == 1): ?>宽640高320<?php else: ?>宽640高1136<?php endif; ?>)  </TD>
					  <TD>&nbsp;</TD>
					</TR>
					<?php if($type == 1): ?><TR>
					  <TH valign="top"><label for="keyword">幻灯片链接地址 ：</label></TH>
						<TD>
							<input type="text" class="px" id="url" value="<?php echo ($flash['url']); ?>" name="url" style="width:500px" ><a href="###" onclick="addLink('url',0)" class="a_choose">从功能库里添加</a><br />
						</TD>
					  <TD>&nbsp;</TD>
					</TR>
					<?php else: ?>
...
<script type="text/javascript">
function addSort(domid,iskeyword){
	art.dialog.data('domid', domid);
	art.dialog.open('?g=User&m=Link&a=Shop&iskeyword='+iskeyword,{lock:true,title:'插入链接或关键词',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
</script>
</div>
</div>
