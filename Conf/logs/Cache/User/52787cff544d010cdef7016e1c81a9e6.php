...
<div class="content">
          ...
          <div class="cLine">
            <div class="pageNavigator left">
				<a href="<?php echo U('Api/add');?>" title="新增接口" class="btnGrayS vm bigbtn"><img src="/tpl/User/default/common/images/api.png" class="vm">新增接口</a>　
          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			  ...
			  <?php if(is_array($api)): $i = 0; $__LIST__ = $api;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$api): $mod = ($i % 2 );++$i;?><tr>
                  <td>  <input type="checkbox" name="del_id[]" value="3266" class="checkitem"></td>
                  <td><?php echo ($api["keyword"]); ?></td>
                  <td><?php echo ($api["url"]); ?></td>
                  <td><?php if($api['type'] == 1): ?>标准xml<?php else: ?>文本格式<?php endif; ?></td>
					<td><?php if($api['status'] == 1): ?>启用<?php else: ?>关闭<?php endif; ?></td>
                    <td><?php echo (date('Y-m-d',$api["time"])); ?></td>
                   <td class="norightborder"> 
					<?php if($api['status'] == 1): ?><a href="<?php echo U('Api/setInc',array('id'=>$api['id'],'type'=>2));?>" title="">关闭</a>
					<?php else: ?>
						
						<a href="<?php echo U('Api/setInc',array('id'=>$api['id'],'type'=>1));?>" title="">启用</a><?php endif; ?>
					<a href="<?php echo U('Api/edit',array('id'=>$api['id']));?>" title="">编辑</a> 
					<a href="<?php echo U('Api/del',array('id'=>$api['id']));?>">删除</a></td>
                ...
                <tr>
					<td colspan="7" class="edit norightborder"> 
						...
						<a  class="btnGreens" ><span>确定</span></a>
					</td>
                </tr>
              </tbody>
            </table>
