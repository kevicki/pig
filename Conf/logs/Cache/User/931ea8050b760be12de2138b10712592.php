...
<script src="/tpl/static/upyun.js?<?php echo date('YmdHis',time());?>"></script>
  <div class="content"> 
   <div class="cLineB"> 
    <h4>商城设置</h4> 
    <a href="<?php echo U('Store/index',array('token'=>$token));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
   </div>
    ...
    <div class="msgWrap bgfc"> 
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody>
       <tr>
        <th>免邮设置(满 )：</th> 
        <td><input type="text" name="price" value="<?php echo ($setting["price"]); ?>" class="px"/>  元免邮(注：写-1表示不免邮)</td> 
       </tr>
       
       <tr>
        <th>积分兑换：</th> 
        <td><input type="text" name="score" value="<?php echo ($setting["score"]); ?>" class="px"/>积分=1元 (注：写0表示积分不可兑换)</td> 
       </tr>
       
       <tr>
        <th>是否支持货到付款：</th> 
        <td>
	        <input type="radio" name="paymode" value="0" id="paymode_0" <?php if($setting['paymode'] == 0): ?>checked<?php endif; ?> /><label for="paymode_0">不支持</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="paymode" value="1" id="paymode_1" <?php if($setting['paymode'] == 1): ?>checked<?php endif; ?> /><label for="paymode_1">支持</label>
        </td> 
       </tr>
       <?php if($showgroup == 1): ?><tr>
        <th>是否支持分组：</th> 
        <td>
	        <input type="radio" name="isgroup" value="0" id="isgroup_0" <?php if($setting['isgroup'] == 0): ?>checked<?php endif; ?> /><label for="isgroup_0">不支持</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="isgroup" value="1" id="isgroup_1" <?php if($setting['isgroup'] == 1): ?>checked<?php endif; ?> /><label for="isgroup_1">支持</label>
        </td> 
       </tr><?php endif; ?>
		<TR>
		  <TH valign="top"><label for="keyword">首页模板：</label></TH>
		  <TD>
			<input type="text" id="selecttpl" class="px" value="<?php if($setting['tpid'] > 0): ?>已选择模板<?php echo ($setting['tpid']); else: ?>点击右侧按钮选择<?php endif; ?>" disabled size="40"  />
			<a class="a_upload" onclick="chooseTpl('tpid','selecttpl',1)">选择模板</a>
			<a class="a_upload" onclick="viewTpl()">预览</a>
			<a class="a_upload" onclick="deletetpl()">恢复默认模板</a>
			<script>
				function viewTpl(){
					var tid = $('#tpid').val();
					chooseTpl(tid,'',2);
			...
			<input type="hidden" class="px" id="tpid" value="<?php if($setting['tpid'] > 0): echo ($setting['tpid']); else: ?>1<?php endif; ?>" name="tpid" style="width:250px">
		  </TD>
		  <TD>
			&nbsp;
		  </TD>
		...
		<TR>
		  <TH valign="top"><label for="keyword">底部导航：</label></TH>
		  <TD>
			
			
			<input type="text" id="showfooter" class="px" value="已选择样式 <?php echo ($setting['footerid']); ?>" disabled size="40"  />
			<a class="a_upload" onclick="chooseMenu('footerid', 'showfooter')">选择样式 </a>
			<a class="a_upload" onclick="viewMenu()">预览</a>
			<script>
				function viewMenu(){
					var tid = $('#footerid').val();
					chooseMenu(tid,'',2);
				}
			</script>
			<input type="hidden" class="px" id="footerid" value="<?php echo ($setting['footerid']); ?>" name="footerid" style="width:250px">
		  </TD>
		  <TD>
			&nbsp;
		  </TD>
       ...
       <tr>
        <th>顶部导航背景色：</th> 
        <td id="header">
        <input type="hidden" name="headerid" value="<?php echo ($setting['headerid']); ?>" id="headerid"/>
		<div style="background:#D80000;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 1): ?>border:dotted 2px;<?php endif; ?>" id="1"></div>
		<div style="background:#DF0300;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 2): ?>border:dotted 2px;<?php endif; ?>" id="2"></div>
		<div style="background:#E70303;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 3): ?>border:dotted 2px;<?php endif; ?>" id="3"></div>
		<div style="background:#EE5C5C;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 4): ?>border:dotted 2px;<?php endif; ?>" id="4"></div>
		<div style="background:#FF8BB7;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 5): ?>border:dotted 2px;<?php endif; ?>" id="5"></div>
		<div style="background:#D66AD6;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 6): ?>border:dotted 2px;<?php endif; ?>" id="6"></div>
		<div style="background:#9962CF;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 7): ?>border:dotted 2px;<?php endif; ?>" id="7"></div>
		<div style="background:#6161CF;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 8): ?>border:dotted 2px;<?php endif; ?>" id="8"></div>
		<div style="background:#318FEC;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 9): ?>border:dotted 2px;<?php endif; ?>" id="9"></div>
		<div style="background:#5DD1F8;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 10): ?>border:dotted 2px;<?php endif; ?>" id="10"></div>
		<div style="background:#83F1F1;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 11): ?>border:dotted 2px;<?php endif; ?>" id="11"></div>
		<div style="background:#A8F56B;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 12): ?>border:dotted 2px;<?php endif; ?>" id="12"></div>
		<div style="background:#92D60B;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 13): ?>border:dotted 2px;<?php endif; ?>" id="13"></div>
		<div style="background:#64BD0C;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 14): ?>border:dotted 2px;<?php endif; ?>" id="14"></div>
		<div style="background:#A0A002;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 15): ?>border:dotted 2px;<?php endif; ?>" id="15"></div>
		
		<div style="background:#D3D303;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 16): ?>border:dotted 2px;<?php endif; ?>" id="16"></div>
		<div style="background:#EEEE4D;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 17): ?>border:dotted 2px;<?php endif; ?>" id="17"></div>
		<div style="background:#FCFC00;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 18): ?>border:dotted 2px;<?php endif; ?>" id="18"></div>
		<div style="background:#FFEC61;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 19): ?>border:dotted 2px;<?php endif; ?>" id="19"></div>
		<div style="background:#FFE000;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 20): ?>border:dotted 2px;<?php endif; ?>" id="20"></div>
		<div style="background:#FFC200;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 21): ?>border:dotted 2px;<?php endif; ?>" id="21"></div>
		<div style="background:#FD9D67;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 22): ?>border:dotted 2px;<?php endif; ?>" id="22"></div>
		<div style="background:#DA9E62;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 23): ?>border:dotted 2px;<?php endif; ?>" id="23"></div>
		<div style="background:#B88B61;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 24): ?>border:dotted 2px;<?php endif; ?>" id="24"></div>
		<div style="background:#AC5600;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 25): ?>border:dotted 2px;<?php endif; ?>" id="25"></div>
		<div style="background:#3A3A3D;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 26): ?>border:dotted 2px;<?php endif; ?>" id="26"></div>
		<div style="background:#697077;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 27): ?>border:dotted 2px;<?php endif; ?>" id="27"></div>
		<div style="background:#A0A0A0;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 28): ?>border:dotted 2px;<?php endif; ?>" id="28"></div>
		<div style="background:#CFCDCD;width:20px;height:20px;float:left;margin-left:2px; <?php if($setting['headerid'] == 29): ?>border:dotted 2px;<?php endif; ?>" id="29"></div>
		<!-- <div style="background:#fff,#f9f9f9);width:20px;height:20px;float:left;margin-left:2px;border:dotted 2px;">白</div> -->
        </td> 
       </tr>
       ...
       <th>&nbsp;</th>
       <td>
       <button type="submit" name="button" class="btnGreen">保存</button> &nbsp; <a href="<?php echo U('Store/index',array('token' => $token));?>" class="btnGray vm">取消</a></td> 
       </tr> 
      </tbody> 
     </table> 
     </div>
...
<script type="text/javascript">
  $(document).ready(function(){
	  $("#header div").click(function(){
		 $(this).css("border", "dotted 2px").siblings().css("border", "");
		 $("#headerid").val($(this).attr("id"));
	  });
  });
  function deletetpl()
  {
