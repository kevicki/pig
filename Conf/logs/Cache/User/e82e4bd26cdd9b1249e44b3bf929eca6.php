		...
		<table class="ListProduct" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%">
			<tbody>
				<tr>
					...
					<?php if($usertplid == 2): ?><td width="110">是否显示</td>
					<td width="120">是否必填</td>
					<?php else: ?>
					<td>是否显示</td>
					<td>是否必填</td><?php endif; ?>
					<td>输入类型 </td>
					<td>排序</td>
					<td>输入限制</td>
				</tr>
				<!--tr>
					...
					<td>
						<input type="radio" name="wechaname" id="wechaname" <?php if($usertplid == 2): ?>style="margin-left:-23px;"<?php endif; ?> <?php if($conf['wechaname'] != 0): ?>checked<?php endif; ?> value="1" /> <label for="wechaname">显示 &nbsp; &nbsp; &nbsp;</label>
					...
					<td>
						<input type="radio" name="is_wechaname" id="is_wechaname" <?php if($conf['is_wechaname'] == 1): ?>checked<?php endif; ?> value="1" /> <label for="is_wechaname">必填 &nbsp; &nbsp; &nbsp;</label>
						<input type="radio" name="is_wechaname" id="is_nowechaname" <?php if($conf['is_wechaname'] == 0): ?>checked<?php endif; ?> value="0" /> <label for="is_wechaname">不必填</label>
					</td>
				...
				<?php if(is_array($fields)): $i = 0; $__LIST__ = $fields;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fd): $mod = ($i % 2 );++$i;?><tr class="custom">
					<td width="120">
						<input name="field_name[<?php echo ($i); ?>]" class="px" value="<?php echo ($fd['field_name']); ?>"/>
						<input name="field_id[<?php echo ($i); ?>]" value="<?php echo ($fd['field_id']); ?>" type="hidden" />
					</td>
					<td>
						<label><input type="radio" name="is_show[<?php echo ($i); ?>]" <?php if($fd['is_show'] == 1): ?>checked<?php endif; ?> value="1" /> 显示 &nbsp; &nbsp;</label>
						<label><input type="radio" name="is_show[<?php echo ($i); ?>]" <?php if($fd['is_show'] == 0): ?>checked<?php endif; ?> value="0" /> 隐藏 </label>
					</td>
					<td>
						<label><input type="radio" name="is_empty[<?php echo ($i); ?>]" <?php if($fd['is_empty'] == 1): ?>checked<?php endif; ?> value="1" /> 必填 &nbsp; &nbsp;</label>
						<label><input type="radio" name="is_empty[<?php echo ($i); ?>]" <?php if($fd['is_empty'] == 0): ?>checked<?php endif; ?> value="0" /> 可不填</label>
					</td>
					<td>
					
						<?php if(!in_array($fd['item_name'], array('tel', 'wechaname', 'portrait', 'truename'))): ?><select name="field_type[<?php echo ($i); ?>]"><?php echo ($fd['field_type_html']); ?></select>
						<span <?php if(!in_array($fd['field_type'], array('select', 'checkbox', 'radio'))): ?>style="display:none"<?php endif; ?>>
							<br />多个选项值，多个用“|”隔,请不要使用空格！<br />
							<textarea name="field_option[<?php echo ($i); ?>]" class="px" style="width:240px; height:70px" cols="60" rows="8"><?php echo ($fd['field_option']); ?></textarea>
						</span>
						
						<?php elseif($fd['item_name'] == 'tel'): ?>
						单行文本框<input type="hidden" name="field_type[<?php echo ($i); ?>]" value="text"/>
						<?php elseif($fd['item_name'] == 'truename'): ?>
						单行文本框<input type="hidden" name="field_type[<?php echo ($i); ?>]" value="text"/>
						<?php elseif($fd['item_name'] == 'wechaname'): ?>
						单行文本框<input type="hidden" name="field_type[<?php echo ($i); ?>]" value="text"/>
						<?php elseif($fd['item_name'] == 'portrait'): ?>
						上传图片<input type="hidden" name="field_type[<?php echo ($i); ?>]" value=""/><?php endif; ?>
						
					</td>
					<td>
						<input name="item_name[<?php echo ($i); ?>]" class="px" value="<?php echo ($fd['item_name']); ?>" type="hidden" />
						<input name="sort[<?php echo ($i); ?>]" value="<?php echo ($fd['sort']); ?>" class="px" style="width:40px"/>
					</td>
					<td>
						<?php if(!in_array($fd['item_name'], array('tel', 'wechaname', 'portrait', 'truename'))): ?><select name="field_match[<?php echo ($i); ?>]"><?php echo ($fd['field_match_html']); ?></select>
						<input value="删除" type="button" class="btnGreen del"/>
						<?php elseif($fd['item_name'] == 'tel'): ?>
						手机号<input type="hidden" name="field_match[<?php echo ($i); ?>]" value="<?php echo ($tel); ?>"/>
						<?php elseif($fd['item_name'] == 'truename'): ?>
						英文数字汉字<input type="hidden" name="field_match[<?php echo ($i); ?>]" value=""/>
						<?php elseif($fd['item_name'] == 'wechaname'): ?>
						英文数字汉字<input type="hidden" name="field_match[<?php echo ($i); ?>]" value=""/>
						<?php elseif($fd['item_name'] == 'portrait'): ?>
						图片<input type="hidden" name="field_match[<?php echo ($i); ?>]" value=""/><?php endif; ?>
					</td>
...
<?php if(session('isQcloud') != true): endif; ?>