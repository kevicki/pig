...
<div class="cLineB">
  <h4>微店回复配置 </h4><a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div>
		...
		<table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%">
			<tbody>
				<tr>
				  ...
				  <td>
					<span class="red"><input class="px" type="text" name="keyword" value="<?php echo (($set["keyword"])?($set["keyword"]):'微店'); ?>" /></span></td>
				</tr>
				<tr>
					<th width="120"><span class="red">*</span>回复标题：</th>
					<td><input type="text" name="title" value="<?php echo (($set["title"])?($set["title"]):'微店'); ?>" class="px" style="width:550px;"></td>
				</tr>
				<tr>
					<th width="120">内容介绍：</th>
					<td><textarea style="width:560px;height:75px" name="description" id="description" class="px"><?php echo (($set["description"])?($set["description"]):'微店正式上线了'); ?></textarea><br/>最多填写120个字</td>
				</tr>
				<tr>
					<th>回复图片：</th>
					<td><input type="text" name="img" value="<?php echo ($set["img"]); ?>" id="pic" class="px" style="width:550px;"> <script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('pic',700,420,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic')">预览</a></td>
				</tr>
				<tr>
					<th>选择店铺：</th>
					<td>
						<select name="sid" style="width:220px">
							<?php if(($get_store_list) == ""): ?><option disabled="disable" value="0">暂未创建店铺，请创建后再选择</option>
							<?php else: ?>
								<?php if(is_array($get_store_list)): $i = 0; $__LIST__ = $get_store_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$get_store_list): $mod = ($i % 2 );++$i;?><option <?php if($set['sid'] == $get_store_list['store_id']): ?>selected<?php endif; ?> value="<?php echo ($get_store_list["store_id"]); ?>"><?php echo ($get_store_list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>


						</select> <span style="color:red">请选择一个店铺，如果您还没有店铺，请创建后再回来这里选择，保存。</span>
					</td>
				</tr>
