...
<div class="content">
          ...
          <?php if($usertplid == 2): ?><div class="alert alert-success alert-dismissable">
        <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
    <?php else: ?>
 			<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-top:5px;"><?php endif; ?>此会员卡号,会员领取一张，这里将会少一张,当全部被领取完了,请再次开卡</div>         
          <div class="cLine">
			<div class="pageNavigator left">
				<?php if($usertplid == 2): ?><i class="fa fa-hand-o-right" style="color:#1ab394;"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 
				<a  href="<?php echo U('Member_card/create_add',array('token'=>$token,'id'=>$thisCard['id']));?>" title="创建会员卡号" >创建会员卡号
</a>&nbsp;&nbsp;<?php if($usertplid == 2): ?><i class="fa fa-times" style="color:#1ab394;"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> <a href="###" onclick="if(confirm('确定删除吗')){$('#info').submit()}">删除</a>&nbsp;&nbsp; <strong>已经领取 <span style="color:red"><?php echo ($usecount); ?></span> 张，剩余<span style="color:red"><?php echo ($ucount); ?></span>张未领取，一共 <span style="color:red"><?php echo ($count); ?></span>张</strong>
			</div>	    
			
			<div class="right">
				<a href="###" onclick="$('#import').css('display','')" class="btnGrayS vm">导入会员卡</a>
				<a href="?g=User&m=Member_card&a=exportCard&id=<?php echo (intval($_GET['id'])); ?>" class="btnGrayS vm">导出会员卡</a>
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($data_vip)): $i = 0; $__LIST__ = $data_vip;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data_vip): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td> 
                      <?php if($data_vip['wecha_id'] != false): ?><!--<a href="index.php?g=User&m=Member_card&a=member&itemid=<?php echo ($data_vip["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>">查看详情</a>-->      
                    	<a href="index.php?g=User&m=Member_card&a=members&itemid=<?php echo ($data_vip["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>">查看详情</a><?php endif; ?>
