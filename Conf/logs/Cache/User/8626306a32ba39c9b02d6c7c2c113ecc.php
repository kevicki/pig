...
<div class="content">
  <div class="cLineB">
      <h4><?php echo ($op); ?>秒杀商品</h4><a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
  </div>
  <div class="msgWrap">
    <form class="form" method="post" action="<?php echo U('Seckill/shop_add', array("type" => $_GET['type']));?>" method="post" id="formID" enctype="multipart/form-data">
      <table class="userinfoArea" style=" margin:20px 0 0 0;" border="0" cellspacing="0" cellpadding="0" width="100%">
        <tbody>
            
            <tr>
              <input type="hidden" class="px" id="id" value="<?php echo ($_GET['aid']); ?>" name="aid" >
              <input type="hidden" class="px" id="id" value="<?php echo ($shop_id); ?>" name="sid" >
            </tr>
            <tr>
              <th valign="top"><label for="title">*商品名称：</label></th>
              <td><input type="input" class="px" id="title" value="<?php echo ($shop_name); ?>" name="shop_name" style="width:300px" data-validation-engine="validate[required,minSize[1],maxSize[30]]" data-errormessage-value-missing="必填项">
                请输入商品名称,30个字以内 </td>
              <td></td>
            </tr>

            <tr>
              <th valign="top"><label for="title">*商品库存：</label></th>
              <td><input type="input" class="px" id="title" value="<?php echo ($shop_num); ?>" name="shop_num" style="width:300px" data-validation-engine="validate[required,minSize[1],maxSize[10],onlyNumber]" data-errormessage-value-missing="必填项">
                请输入商品库存 </td>
              <td></td>
            </tr>

            <tr>
              <th valign="top"><label for="title">*商品单价：</label></th>
              <td><input type="input" class="px" id="title" value="<?php echo ($shop_price); ?>" name="shop_price" style="width:300px" data-validation-engine="validate[required,minSize[1],maxSize[10],onlyNumber]" data-errormessage-value-missing="必填项"> 元
                请输入商品库存 </td>
              <td></td>
            </tr>

            <tr>
              <th valign="top"><label for="title">*商品运费：</label></th>
              <td><input type="input" class="px" id="title" value="<?php echo ($shop_tran); ?>" name="shop_tran" style="width:300px" data-validation-engine="validate[required,minSize[1],maxSize[10],onlyNumber]" data-errormessage-value-missing="必填项"> 元
                请输入商品运费 </td>
              <td></td>
            </tr>

            <tr>
            <th valign="top"><label for="keyword">*商品展示图1：</label></th>
            <td>
            <?php if($tdata[0]['shop_big'] != ''): ?><img class="thumb_img" id="picurl_src" src="<?php echo ($tdata[0]['shop_big']); ?>" style="max-height:100px;max-width: 150px;"><?php endif; ?>
            <input type="input" class="px" id="picurl" value="<?php echo ($tdata[0]['shop_big']); ?>" name="shop_big[]" data-validation-engine="validate[required,custom[url]]"
                    data-errormessage-value-missing="必须上传图片!"  data-errormessage-custom-error="如果是手动填写,正确的网址,如: http://www.baidu.com/images/demo.png">
<a href="###" onclick="upyunPicUpload('picurl',738,400,'<?php echo ($token); ?>')" class="a_upload">上传</a>
<a href="###" onclick="viewImg('picurl')">预览</a> [738*300] </td>
            <td></td>
          </tr>

            <tr>
            <th valign="top"><label for="keyword">*商品展示图2：</label></th>
            <td>
              <?php if($tdata[1]['shop_big'] != ''): ?><img class="thumb_img" id="roompicurl_src" src="<?php echo ($tdata[1]['shop_big']); ?>" style="max-height:100px;"><?php endif; ?>
            <input type="input" class="px" id="roompicurl" value="<?php echo ($tdata[1]['shop_big']); ?>" name="shop_big[]" data-validation-engine="validate[required,custom[url]]"
                    data-errormessage-value-missing="必须上传图片!"  data-errormessage-custom-error="如果是手动填写,正确的网址,如: http://www.baidu.com/images/demo.png">
<a href="###" onclick="upyunPicUpload('roompicurl',738,400,'<?php echo ($token); ?>')" class="a_upload">上传</a>
<a href="###" onclick="viewImg('roompicurl')">预览</a> [738*300] </td>
            <td></td>
          </tr>

            <tr>
              <th valign="top"><label for="action_rule">*商品描述：</label></th>
              <td>
                <textarea class="px" id="action_rule" name="shop_detail" style="width: 605px; height: 350px;" ><?php echo ($shop_detail); ?></textarea>
              </td>
              <td></td>
            </tr>

            <tr>
              <th valign="top"><label for="action_rule">*是否开启：</label></th>
              <td>
                <input name="shop_open" type="radio" value = '0' id="action_rule" <?php if($shop_open == 0): ?>checked<?php endif; ?>/> 开启&nbsp;&nbsp;&nbsp;&nbsp;

                <input name="shop_open" type="radio" value = '1' id="action_rule" <?php if(($shop_open == 1)): ?>checked<?php endif; ?> /> 关闭 
