...
<div class="content">
	  ...
	  <tr>
	      <th>活动名称：</th>
	      <td>
	      	<input type="input" class="px" class="name" value="<?php echo ($set["name"]); ?>" name="name"> 
	      </td>
	 </tr>             		
    ...
    <tr>
		<th>活动持续时间：</th>
		<td>	
			<input type="input" class="px" class="start" value="<?php if($set["start"] != ''): echo (date('Y-m-d',$set["start"])); endif; ?>" name="start" onClick="WdatePicker()">
			-
			<input type="input" class="px" class="end" value="<?php if($set["end"] != ''): echo (date('Y-m-d',$set["end"])); endif; ?>" name="end"   onClick="WdatePicker()">
		</td>
	</tr>
...
<tr>
	...
	<td>
		<input type="radio" class="type" name="type" value="1" <?php if($set["type"] == '1' or $set["type"] == ''): ?>checked<?php endif; ?>>积分 &nbsp;
		<input type="radio" class="type" name="type" value="2" <?php if($set["type"] == '2'): ?>checked<?php endif; ?>>优惠劵
	</td>
</tr>
<tr class="value" <?php if($set["type"] == '2'): ?>style="display:none;"<?php endif; ?>>
	<th valign="top">赠送积分：</th>
	<td>
		<input type="text" name="item_value" value="<?php echo ($set["item_value"]); ?>" style="width:65px;" class="px scroe">分
	</td>
</tr>
<tr class="attr" <?php if($set["type"] == '1' or $set["type"] == ''): ?>style="display:none;"<?php endif; ?>>
	<th valign="top">赠送优惠劵：</th>
	<td>
		<select name="item_attr" class="item_attr">
			<option value="">选择劵类</option>
			<option value="1" <?php if($set["item_attr"] == '1'): ?>selected<?php endif; ?>>优惠劵</option>
			<option value="0" <?php if($set["item_attr"] == '0'): ?>selected<?php endif; ?>>代金券</option>
			<option value="2" <?php if($set["item_attr"] == '2'): ?>selected<?php endif; ?>>礼品劵</option>
		</select>
		<select name="item_value" class="item_value" disabled="true">
...
<tr>
	<th>状态：</th>
	<td>
		<input type="radio" name="is_open" value="1" <?php if($set["is_open"] == '1' or $list["is_open"] == ''): ?>checked<?php endif; ?>>开启
...
<script>
	$(function(){
		...
		$('.item_attr').change(function(){
			var attr 	= $(this).val();
			if(attr != ''){
				$.getJSON("<?php echo U('Member_card/get_value',array('token'=>$token));?>",{cardid:<?php echo ($thisCard['id']); ?>,item_attr:attr},function(data){
