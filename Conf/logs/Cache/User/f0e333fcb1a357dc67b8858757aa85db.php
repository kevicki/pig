...
<div class="cLineB">
<ul id="tags">
  <li class="selectTag"><a href="<?php echo U('Member_card/center',array('token'=>$token));?>">已领卡会员</a> </li>
  <li><a href="<?php echo U('Member_card/center_all',array('token'=>$token));?>">所有注册会员</a></li>
  <div class="clr"></div>
</ul>
<div class="clr"></div>
</div>
...
<div class="cLine">
<form method="post"  action="<?php echo U('Member_card/center',array('token'=>$token));?>" id="info" >
	会员卡
	<select name="cardid">
		<option value="">请选择..</option>
		<?php if(is_array($cards)): $i = 0; $__LIST__ = $cards;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cards): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cards["id"]); ?>" <?php if($cards['id'] == $cardid): ?>selected<?php endif; ?>><?php echo ($cards["cardname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	</select>
	会员卡号：
	<input type="text" class="px" name="number" value="<?php echo ($number); ?>" style="height:25px;" placeholder="请输入会员卡号">
...
<TD><?php if($item["createtime"] == 0): ?>无时间记录<?php else: echo (date('Y-m-d',$item["createtime"])); endif; ?></TD>
<TD><?php echo ($item["score"]); ?></TD>
<TD><?php echo ($item["expense"]); ?></TD>
<TD><?php echo ($item["balance"]); ?></TD>
<TD><a href="<?php echo U('Member_card/member',array('token'=>$this->token,'cardid'=>$item['cardid'],'number'=>$item['number']));?>">消费记录</a></TD>
