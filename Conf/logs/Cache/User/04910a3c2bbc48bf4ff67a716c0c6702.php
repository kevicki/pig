...
.dtas p{line-height:30px;}
  ...
  <div style="line-height: 32px;margin:10px 0"> <a href="<?php echo U('Hardware/orderprintSet');?>" class="btnGrayS vm bigbtn" >添加打印机</a>　 </div>
  <table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
    <thead>
      <tr>
        <th style=" width:60px;">打印机类型</th>
        <th style=" width:170px;">商户编码/用户ID</th>
        <th style=" width:215px;">API密钥（易联云）</th>
        <th style=" width:90px;">终端号</th>
        <th style=" width:65px;">密钥</th>
        
        <th style=" width:90px;" class="norightborder">操作</th>
      </tr>
    </thead>
    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$actinfo): $mod = ($i % 2 );++$i;?><tr>
        <td><?php if($actinfo['printtype'] == 0): ?>飞印打印机
            <?php else: ?>
            易联云打印机<?php endif; ?></td>
        <td><?php echo ($actinfo["mc"]); ?><br><font color="#FF0000"><?php echo ($actinfo["modules"]); ?></font></td>
        <td><?php echo ($actinfo["apikey"]); ?></td>
        <td><?php echo ($actinfo["mcode"]); ?></td>
        <td><?php echo ($actinfo["mkey"]); ?></td>
        
        <td class="norightborder"><a href="<?php echo U('Hardware/orderprintSet',array('id'=>$actinfo['id'],'token'=>$_SESSION['token']));?>" >修改</a>&nbsp;<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hardware/orderprintDelete',array('token'=>$token,'id'=>$actinfo['id']));?>');">删除</a></td><?php endforeach; endif; else: echo "" ;endif; ?>
    </tr>
    
  </table>
  <div class="dtas" style="line-height:30px;font-size:14px;"> </div>
</div>
</div>
</div>
</div>
