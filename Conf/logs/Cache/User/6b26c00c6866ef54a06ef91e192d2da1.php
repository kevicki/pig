...
<a href="<?php echo U('Voteimg/add_voteimg',array('token'=>$token,'id'=>$id));?>" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加图片投票</a>  
</div>
	...
	<tr>
	  ...
	  <th style="width:13%;">活动开始结束时间</th>
	  <th style="width:13%;">报名开始结束时间</th>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody><tr>
		  ...
		  <td><?php echo (date("Y-m-d H:i",$vo["apply_start_time"])); ?><br><?php echo (date("Y-m-d H:i",$vo["apply_end_time"])); ?></td>
		  <td>
		  <?php if($_SERVER['REQUEST_TIME'] > $vo['apply_start_time'] && $_SERVER['REQUEST_TIME'] < $vo['apply_end_time']){ echo '<span style="color:green">报名期</span>';}else{ echo '<span style="color:red">非报名期</span>';} ?>
		  /
		  <?php if($_SERVER['REQUEST_TIME'] > $vo['start_time'] && $_SERVER['REQUEST_TIME'] < $vo['end_time']){ echo '<span style="color:green">活动期</span>';}else{ echo '<span style="color:red">非活动期</span>';} ?>
		  </td>
 ...
 <style type="text/css">
   #brg<?php echo ($vo["id"]); ?>{ z-index:9999;width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($vo["id"]); ?>{z-index:9999; display:none;position:fixed; *position:absolute; width:100px; height:100px; top:30%; left:50%; margin:-50px 0 0 -50px;}
 ...
 <div id="showdiv<?php echo ($vo["id"]); ?>">
   ...
   <div id="testdiv<?php echo ($vo["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["id"]); ?>&modle=Voteimg" width="300" height="300" class="cateimg_small"></div>
 </div>
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><a id="lay<?php echo ($vo["id"]); ?>" style="width:191px;">查看二维码</a><?php endif; endif; ?>
		  <a href="<?php echo U('Voteimg/item_list',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">投票选项</a>
		  <a href="<?php echo U('Voteimg/banner_manage',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">横幅管理</a>
		  <a href="<?php echo U('Voteimg/stat_list',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">统计栏管理</a>
		  <a href="<?php echo U('Voteimg/menu_list',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">菜单管理</a>
		  <a href="<?php echo U('Voteimg/bottom_list',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">底部管理</a>
		  <a href="<?php echo U('Voteimg/apply_list',array('vote_id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">报名管理</a>
		 <a href="<?php echo U('Voteimg/vote_log',array('vote_id'=>$vo['id'], 'token'=>$token));?>" class="btnGreen">查看投票记录</a>
		  <a href="<?php echo U('Voteimg/add_voteimg',array('id'=>$vo['id'],'token'=>$token));?>" class="btnGreen">编辑</a>
		  <a href="<?php echo U('Voteimg/action_del',array('id'=>$vo['id'],'token'=>$token));?>" onclick="return(confirm('确定要删除吗？'))" class="btnGreen">删除</a> </td>
		</tr>
