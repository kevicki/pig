...
<script type="text/javascript">
	...
	$(document).ready(function(){
        ...
        <div class="content">
          <div class="cLineB">
              <h4 class="left"><?php echo ($activityTypeName); ?>活动信息 (<?php echo ($count); ?>) 条<span class="FAQ">你本月有 <?php echo ($group["activitynum"]); ?> 次机会可免收活动创建费，已经使用了 <?php echo ($activitynum); ?> 次机会!</span></h4>
                 
              <div class="clr"></div>
          </div>
          <div class="cLine">
            <div class="pageNavigator left">
  <a href='<?php echo U('Autumns/add',array('token'=>$token));?>' title='新增<?php echo ($activityTypeName); ?>活动' class='btnGrayS vm bigbtn'><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 新增<?php echo ($activityTypeName); ?>活动</a>
...
<TH class="norightborder">操作</TH>
          ...
          <TD><?php echo ($list["joinnum"]); ?></TD>                
                  <TD><?php echo (date('Y-m-d H:i',$list["statdate"])); ?><br /><?php echo (date('Y-m-d H:i',$list["enddate"])); ?></TD>
                  <TD>
   ...
   $(document).ready(function(){
      ...
      $("#lay<?php echo ($list["id"]); ?>").click(function(){
   $("#brg<?php echo ($list["id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($list["id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($list["id"]); ?>").click(function(){
      $("#brg<?php echo ($list["id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($list["id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($list["id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($list["id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute; width:100px; height:100px; top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($list["id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($list["id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($list["id"]); ?>"></div>
 <div id="showdiv<?php echo ($list["id"]); ?>">
   <div id="close<?php echo ($list["id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($list["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($list["id"]); ?>&modle=Autumns" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($list["id"]); ?>"  style="width:303px;">查看二维码</a><br><?php endif; endif; ?>
            <a href="index.php?g=User&m=Autumns&a=sn&id=<?php echo ($list["id"]); ?>&type=1">SN码管理</a> 
            <a href="index.php?g=User&m=Autumns&a=cheat&id=<?php echo ($list["id"]); ?>">作弊</a> 
            <a href="index.php?g=User&m=Autumns&a=edit&id=<?php echo ($list["id"]); ?>">编辑</a>  


           <a href="
           <?php if($list['status'] == 1): ?>javascript:drop_confirm('你确定要结束活动吗，结束后不可再开启本活动!', 'index.php?g=User&m=Autumns&a=endLottery&id=<?php echo ($list["id"]); ?>');<?php else: ?>index.php?g=User&m=Autumns&a=startLottery&id=<?php echo ($list["id"]); endif; ?>">
           <?php if($list['status'] == 0): ?>开始<?php else: ?>结束<?php endif; ?>           
           </a>
          
           <a href="index.php?g=User&m=Autumns&a=del&id=<?php echo ($list["id"]); ?>">删除</a></TD>
