...
<?php $__FOR_START_1626638759__=1;$__FOR_END_1626638759__=12;for($i=$__FOR_START_1626638759__;$i < $__FOR_END_1626638759__;$i+=1){ ?><option value="<?php echo ($i); ?>" <?php if($i == $current_month): ?>selected = selected<?php endif; ?>><?php echo ($i); ?>月</option><?php } ?>
</select>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
选择设备：
<select name="device_id" id="device_id">
<?php if($devices == ''): ?><option value="0">---请先添加并激活设备--</option>
<?php else: ?>
<?php if(is_array($devices)): $i = 0; $__LIST__ = $devices;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["device_id"]); ?>"><?php echo ($vo["device_comment"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
</select>
</div>
...
<?php $__FOR_START_1889817651__=1;$__FOR_END_1889817651__=12;for($i=$__FOR_START_1889817651__;$i < $__FOR_END_1889817651__;$i+=1){ ?><option value="<?php echo ($i); ?>" <?php if($i == $current_month): ?>selected = selected<?php endif; ?>><?php echo ($i); ?>月</option><?php } ?>
</select>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
选择页面：
<select name="page_id" id="page_id">
<?php if($devices == ''): ?><option value="0">---请先添加页面--</option>
<?php else: ?>
<?php if(is_array($pages)): $i = 0; $__LIST__ = $pages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["page_id"]); ?>"><?php echo ($vo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endif; ?>
</select>
</div>
...
<script type="text/javascript">
  $(function(){
	...
	$("#shakearound_index").html("<img src='<?php echo ($staticPath); ?>/tpl/static/seckill/images/loading.gif'/>");
	$("#shakearound_index").load("/index.php?g=User&m=Shakearound&a=statistics_by_device&device_id="+device_id+"&token=<?php echo ($token); ?>&month="+month);
	
	var pagemonth = $("#pagemonth").val();
	var page_id = $("#page_id").val();
	$("#shakearound_page").html("<img src='<?php echo ($staticPath); ?>/tpl/static/seckill/images/loading.gif'/>");
	$("#shakearound_page").load("/index.php?g=User&m=Shakearound&a=statistics_by_page&page_id="+page_id+"&token=<?php echo ($token); ?>&pagemonth="+pagemonth);
