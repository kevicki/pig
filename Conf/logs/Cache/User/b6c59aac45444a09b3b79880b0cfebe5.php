...
<TR>
  ...
  <TD>
	<input type="input" class="px" id="keyword" value="<?php if($set["keyword"] == ''): ?>微助力<?php else: echo ($set["keyword"]); endif; ?>" name="keyword" style="width:400px" ><br />
  	<span class="red">只能写一个关键词</span>，用户输入此关键词将会触发此活动。
  </TD>
  <TD rowspan="4" valign="top">
	  <div style="margin-left:20px">
		<img id="pic1_src" src="<?php if($set['reply_pic'] == ''): echo ($staticPath); ?>/tpl/static/helping/images/default.jpg<?php else: echo ($set["reply_pic"]); endif; ?>" width="373px" >
		<br />
		<input class="px"  name="reply_pic" value="<?php if($set['reply_pic'] == ''): echo ($staticPath); ?>/tpl/static/helping/images/default.jpg<?php else: echo ($set["reply_pic"]); endif; ?>" id="pic1" style="width:363px;"  />
		<br />
...
<tr>
  ...
  <TD>
	<input type="input" class="px" id="title" value="<?php if($set["title"] == ''): ?>微助力，助你成功<?php else: echo ($set["title"]); endif; ?>" name="title" style="width:400px" />
  	<br />
  	请不要多于50字!
  </TD>
</tr>
<TR>
	...
	<td><input type="input" class="px" id="statdate" value="<?php if($set['start'] != ''): echo (date("Y-m-d H:i",$set["start"])); else: echo ($start_date); ?> 00:00<?php endif; ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" name="start" />
		到
		<input type="input" class="px" id="enddate" value="<?php if($set['end'] != ''): echo (date("Y-m-d H:i",$set["end"])); else: echo ($end_date); ?> 00:00<?php endif; ?>" name="end"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"  />
	</td>
</TR>
<tr>
  	<th valign="top"><span class="red"></span>活动简介：</th>
  	<td>
  		<textarea class="px" id="desc" name="intro" style="width:400px; height: 40px;"><?php echo ($set["intro"]); ?></textarea> 消息回复时的简介内容
  	</td>
</tr>
<TR>
...
<tr>
  ...
  <TD colspan="2">
	<input type="input" class="px" id="fxtitle" value="<?php echo ($set["fxtitle"]); ?>" name="fxtitle" style="width:500px" />
  	<br />
...
<tr>
  ...
  <TD colspan="2">
	<input type="input" class="px" id="fxinfo" value="<?php echo ($set["fxinfo"]); ?>" name="fxinfo" style="width:500px" />
  	<br />
...
<tr>
  ...
  <TD colspan="2">
	<input type="number" class="px" id="rank_num" value="<?php echo ($set["rank_num"]); ?>" name="rank_num" style="width:80px" />
  	<br />
...
<tr>
	...
	<td>
		<input type="radio" name="is_newtp" id="is_newtp0" value="0" <?php if($set['is_newtp'] == 0): ?>checked<?php endif; ?>><label for="is_newtp0">旧版本</label>
		&nbsp;&nbsp;&nbsp;
		<input type="radio" name="is_newtp" id="is_newtp1" value="1" <?php if($set['is_newtp'] == 1): ?>checked<?php endif; ?>><label for="is_newtp1">新版本</label>
	</td>
</tr>
<!-- 新版本 -->
...
<tr class="newtp news_imgurl">
	...
	<td colspan="2">
		...
		<a href="###" onclick="upyunPicUpload('news_imgurl_1',900,500,'<?php echo ($token); ?>')" class="a_upload">上传</a>
		&nbsp;
...
<tr class="newtp prize_imgurl">
	...
	<td colspan="2">
		...
		<a href="###" onclick="upyunPicUpload('prize_imgurl_1',200,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
		&nbsp;
...
<tr class="newtp">
	...
	<td colspan="2">
		<input type="radio" name="is_sms" id="is_sms1" value="1" <?php if($set['is_sms'] == 1): ?>checked<?php endif; ?>><label for="is_sms1">是</label>
		&nbsp;&nbsp;&nbsp;
		<input type="radio" name="is_sms" id="is_sms0" value="0" <?php if($set['is_sms'] == 0): ?>checked<?php endif; ?>><label for="is_sms0">否</label>
		&nbsp;&nbsp;&nbsp;<span class="red">（需要购买短信，只支持新版本，在分享出去后别人进入页面点击返回我的会跳出短信验证）</span>
	</td>
</tr>
<?php }else{ ?>
	<!-- 宣传图片 -->
<?php if(is_array($news_list)): $news_i = 0; $__LIST__ = $news_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$news): $mod = ($news_i % 2 );++$news_i;?><tr class="newtp news_imgurl">
	<th valign="top"><span class="red">*</span>宣传图片<?php echo ($news_i); ?>：</th>
	<td colspan="2">
		<input type="input" class="px" id="news_imgurl_<?php echo ($news_i); ?>"  name="news_imgurl[]" value="<?php echo ($news["imgurl"]); ?>"  style="width:250px"/>
		<a href="###" onclick="upyunPicUpload('news_imgurl_<?php echo ($news_i); ?>',900,500,'<?php echo ($token); ?>')" class="a_upload">上传</a>
		&nbsp;
		<a href="###" onclick="viewImg('news_imgurl_<?php echo ($news_i); ?>')">预览</a>
		&nbsp;推荐尺寸：900*500
		<span class="red">&nbsp;请让每个宣传图片的尺寸相同！否则会导致页面错位！</span>
	</td>
</tr>
<tr class="newtp news_title">
	<th valign="top">宣传图片标题<?php echo ($news_i); ?>：</th>
	<td colspan="2">
		<input type="input" class="px" id="news_title_<?php echo ($news_i); ?>" name="news_title[]" value="<?php echo ($news["title"]); ?>" style="width:400px" />&nbsp;手机端最多显示22个字
	</td>
</tr>
<tr class="newtp news_url">
	<th valign="top">宣传图片链接<?php echo ($news_i); ?>：</th>
	<td colspan="2">
		<input type="input" class="px" id="news_url_<?php echo ($news_i); ?>" name="news_url[]" value="<?php echo ($news["url"]); ?>" style="width:400px" /><a href="###" onclick="addLink('news_url_<?php echo ($news_i); ?>',0)" class="a_choose">从功能库添加</a>
	</td>
...
<tr class="newtp">
	...
	<!-- 奖品 -->
<?php if(is_array($prize_list)): $prize_i = 0; $__LIST__ = $prize_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$prize): $mod = ($prize_i % 2 );++$prize_i;?><tr class="newtp prize_title">
	<th valign="top"><span class="red">*</span><?php echo ($prize_i); ?>等奖名称：</th>
	<td colspan="2">
		<input type="input" class="px" id="prize_title_<?php echo ($prize_i); ?>" name="prize_title[]" value="<?php echo ($prize["title"]); ?>" style="width:400px" />&nbsp;手机端最多显示13个字
	</td>
</tr>
<tr class="newtp prize_imgurl">
	<th valign="top"><span class="red">*</span><?php echo ($prize_i); ?>等奖图片：</th>
	<td colspan="2">
		<input type="input" class="px" id="prize_imgurl_<?php echo ($prize_i); ?>"  name="prize_imgurl[]" value="<?php echo ($prize["imgurl"]); ?>"  style="width:250px"/>
		<a href="###" onclick="upyunPicUpload('prize_imgurl_<?php echo ($prize_i); ?>',200,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
		&nbsp;
		<a href="###" onclick="viewImg('prize_imgurl_<?php echo ($prize_i); ?>')">预览</a>
		&nbsp;推荐尺寸：200*200
	</td>
</tr>
<tr class="newtp prize_time">
	<th><span class="red">*</span><?php echo ($prize_i); ?>等奖兑奖时间：</th>
	<td><input type="input" class="px" id="prize_starttime_<?php echo ($prize_i); ?>" value="<?php echo (date('Y-m-d',$prize["starttime"])); ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="prize_starttime[]" />
		到
		<input type="input" class="px" id="prize_stoptime_<?php echo ($prize_i); ?>" value="<?php echo (date('Y-m-d',$prize["stoptime"])); ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="prize_stoptime[]"  />
	</td>
</tr><?php endforeach; endif; else: echo "" ;endif; ?>
<tr class="newtp">
	<td></td>
...
function prize_more(){
	if(prizenum < 10){
		prizenum++;
		var prize_html = '<tr class="newtp prize_title"><th valign="top"><span class="red">*</span>'+prizenum+'等奖名称：</th><td colspan="2"><input type="input" class="px" id="prize_title_'+prizenum+'" name="prize_title[]" value="" style="width:400px" />&nbsp;手机端最多显示13个字</td></tr><tr class="newtp prize_imgurl"><th valign="top"><span class="red">*</span>'+prizenum+'等奖图片：</th><td colspan="2"><input type="input" class="px" id="prize_imgurl_'+prizenum+'"  name="prize_imgurl[]" value=""  style="width:250px"/><a href="###" onclick="upyunPicUpload(\'prize_imgurl_'+prizenum+'\',200,200,\'<?php echo ($token); ?>\')" class="a_upload">上传</a>&nbsp;<a href="###" onclick="viewImg(\'prize_imgurl_'+prizenum+'\')">预览</a>&nbsp;推荐尺寸：200*200</td></tr><tr class="newtp prize_time"><th><span class="red">*</span>'+prizenum+'等奖兑奖时间：</th><td><input type="input" class="px" id="prize_starttime_'+prizenum+'" value="" onClick="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" name="prize_starttime[]" />到<input type="input" class="px" id="prize_stoptime_'+prizenum+'" value="" onClick="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" name="prize_stoptime[]"  /></td></tr>';
