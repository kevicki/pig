...
<a href="###" onclick="upyunPicUpload('pic2',1000,500,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic2')">预览</a>&nbsp;消息回复图片
  </div>
...
<tr>
	...
	<td><input type="input" class="px" id="start_time" value="<?php if($set['start_time'] != ''): echo (date("Y-m-d H:i",$set["start_time"])); else: echo date('Y-m-d H:i',mktime(0, 0, 0, date("m") , date("d"), date("Y"))); endif; ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" name="start_time" />
		到
		<input type="input" class="px" id="end_time" value="<?php if($set['end_time'] != ''): echo (date("Y-m-d H:i",$set["end_time"])); else: echo date('Y-m-d H:i',mktime(0, 0, 0, date("m") , date("d")+3, date("Y"))); endif; ?>" name="end_time" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"  />
	</td>
</tr>
  	...
  	<table class="userinfoArea" style=" margin: 0;" border="0" cellspacing="0" cellpadding="0" width="100%">
        ...
        <tr>
            ...
            <td>
              <input  type="radio" class="px packet_type" id="" name="packet_type" value="1" <?php if($set["packet_type"] != '2'): ?>checked<?php endif; ?>>手气红包&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              <input  type="radio" class="px packet_type" id="" name="packet_type" value="2" <?php if($set["packet_type"] == '2'): ?>checked<?php endif; ?>>普通红包
              &nbsp;
...
$(document).ready(function(){
	$("form").submit(function(){
		...
		var desc = $('#desc').val();//活动介绍
		var start_time = $('#start_time').val();//活动开始时间
		var end_time = $('#end_time').val();//活动结束时间
		var people = $('#people').val();//领取人数
		var daynums = $('#daynums').val();//领取次数
