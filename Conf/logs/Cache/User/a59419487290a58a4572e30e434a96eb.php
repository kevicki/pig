...
<?php if($_SESSION['is_syn']!= 2): if($_SESSION['is_syn'] != 1): ?>温馨提示：本功能使用了模板消息中的"中奖结果通知"模板消息,模板消息编号为TM00695。<br/>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Research/saveInfo', array('token' => $token));?>" title="添加" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加调研</a></div>
<div class="clr"></div>
</div>
<!--tab end-->
<div class="msgWrap">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
   ...
   $(document).ready(function(){
      ...
      $("#lay<?php echo ($tablelist["id"]); ?>").click(function(){
   $("#brg<?php echo ($tablelist["id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($tablelist["id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($tablelist["id"]); ?>").click(function(){
      $("#brg<?php echo ($tablelist["id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($tablelist["id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($tablelist["id"]); ?>{z-index: 9999;width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($tablelist["id"]); ?>{z-index:9999; display:none;position:fixed; *position:absolute; top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($tablelist["id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($tablelist["id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($tablelist["id"]); ?>"></div>
 <div id="showdiv<?php echo ($tablelist["id"]); ?>">
   <div id="close<?php echo ($tablelist["id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($tablelist["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($tablelist["id"]); ?>&modle=Research" width="300" height="300" class="cateimg_small"></div>
 </div>
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><a id="lay<?php echo ($tablelist["id"]); ?>" style="width:191px;">查看二维码</a><br><?php endif; endif; ?>
