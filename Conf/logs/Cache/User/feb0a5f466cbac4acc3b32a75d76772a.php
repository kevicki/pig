...
<form class="form" method="post" action="" target="_top" enctype="multipart/form-data">
  ...
  <input type="hidden" name="id" value="<?php echo ($mbkid); ?>">

<table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"><tbody><tr>
<th width="120">图文消息标题：</th>
<td><input type="text" name="title" style="width:200px;" class="px" value="<?php if($mbk['title'] == ''): ?>快来代理赚钱啊<?php else: echo ($mbk["title"]); endif; ?>"></td>
</tr>
<th width="120">触发关键词：</th>
<td><input type="text" name="keyword" value="<?php if($mbk['keyword'] == ''): ?>全民经纪人<?php else: echo ($mbk["keyword"]); endif; ?>" class="px"><br><span class="red"></span></td>
</tr>
<tr>
<th valign="top">图文消息封面：</th>
<td>
<?php if($mbk['imgreply'] != ''): ?><img class="thumb_img" id="imgreply_src" src="<?php echo ($mbk['imgreply']); ?>" style="max-height:100px;max-width: 150px;"><?php endif; ?>
<input type="input" class="px" id="imgreply" value="<?php echo ($mbk['imgreply']); ?>" name="imgreply" data-validation-engine="validate[required,custom[url]]"
...
<a href="###" onclick="upyunPicUpload('imgreply',900,500,'<?php echo ($token); ?>')" class="a_upload">上传</a>
...
<th width="120">图文消息简介：</th>
	<td><textarea style="width:490px;height:100px" name="rinfo" id="rinfo" class="px"><?php echo ($mbk['rinfo']); ?></textarea><br/>最多填写120个字</td>
</tr>

<tr>
<th valign="top">活动封面图：</th>
<td>
<?php if($mbk['picurl'] != ''): ?><img class="thumb_img" id="picurl_src" src="<?php echo ($mbk['picurl']); ?>" style="max-height:100px;max-width: 150px;"><?php endif; ?>
<input type="input" class="px" id="picurl" value="<?php echo ($mbk['picurl']); ?>" name="picurl" data-validation-engine="validate[required,custom[url]]"
...
<a href="###" onclick="upyunPicUpload('picurl',640,960,'<?php echo ($token); ?>')" class="a_upload">上传</a>
...
<td>
<?php if($mbk['bgimg'] != ''): ?><img class="thumb_img" id="bgimg_src" src="<?php echo ($mbk['bgimg']); ?>" style="max-height:100px;max-width: 150px;"><?php endif; ?>
<input type="input" class="px" id="bg_img" value="<?php echo ($mbk['bgimg']); ?>" name="bgimg" data-validation-engine="validate[required,custom[url]]"
...
<a href="###" onclick="upyunPicUpload('bg_img',1080,630,'<?php echo ($token); ?>')" class="a_upload">上传</a>
<a href="###" onclick="viewImg('bg_img')">预览</a> [1080*630] </td>
<td></td>
</tr>

<th width="120">产品顾问邀请码：</th>
<td><input type="text" name="invitecode" value="<?php echo ($invitecode); ?>" class="px"><br><span class="red">16位以内英文数字字符</span></td>
</tr>
<tr>
<th>截至时间：</th>
<td><input type="input" class="px" id="statdate" value="<?php if($mbk['statdate'] != ''): echo (date("Y-m-d",$mbk["statdate"])); endif; ?>" onClick="WdatePicker()" name="statdate">
到
<input type="input" class="px" id="enddate" value="<?php if($mbk['enddate'] != ''): echo (date("Y-m-d",$mbk["enddate"])); endif; ?>" name="enddate" onClick="WdatePicker()"></td>
</tr>
<tr>
...
<?php if(!empty($items) AND is_array($items)): if(is_array($items)): $key = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($key % 2 );++$key;?><tbody class="item<?php echo ($val["id"]); ?>" name="div_add_del">
<tr>
    <td width="130"><input type="text" name="add[xmname][]" value="<?php echo ($val["xmname"]); ?>" class="px" style="width:120px;"></td>
	<?php if($val["xmtype"] != 2): ?><td style="text-align: left;"> <input type="checkbox" name="add[xmtype][]" value="1" checked="checked" class="moneytype">&nbsp;固定金额<br/><input type="checkbox" name="add[xmtype][]" value="2" class="moneytype">&nbsp;百分比
     </td>
     <td class="xmnum">
       <input type="text" name="add[xmnum][]" value="<?php echo ($val["xmnum"]); ?>" style="width:65px;" class="px"><span class="moneytype1">&nbsp;元</span><span class="moneytype2" style="display:none;">&nbsp;%</span>
      </td>
	<?php else: ?>
	<td style="text-align: left;"><input type="checkbox" name="add[xmtype][]" value="1" class="moneytype">&nbsp;固定金额<br/><input type="checkbox" checked="checked" name="add[xmtype][]" value="2" class="moneytype">&nbsp;&nbsp;百分比
     ...
     <td class="xmnum">
       <input type="text" name="add[xmnum][]" value="<?php echo ($val["xmnum"]); ?>" style="width:65px;" class="px"><span class="moneytype1" style="display:none;">&nbsp;元</span><span class="moneytype2">&nbsp;%</span><?php endif; ?>

     <td width="200">
         <input type="text" name="add[xmimg][]" value="<?php echo ($val["xmimg"]); ?>" class="px"  id="xmimg_<?php echo ($val["id"]); ?>" style="width:130px;">
         <a href="###" onclick="upyunPicUpload('xmimg_<?php echo ($val["id"]); ?>',500,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
		 <a href="###" onclick="viewImg('xmimg_<?php echo ($val["id"]); ?>')">预览</a>
      </td>
    <td width="100"><input type="text" name="add[tourl][]"  value="<?php echo ($val["tourl"]); ?>" class="px" style="width:100px;" id="tourl_u<?php echo ($val["id"]); ?>"><br/><a href="###" onclick="addLink('tourl_u<?php echo ($val["id"]); ?>',0)" class="a_choose" style="margin-top: 7px">从功能库添加</a></td>
	<input type="hidden" name="add[id][]" readonly="readonly" value="<?php echo ($val["id"]); ?>">
	<?php if(($key > 1)): ?><td width="80" class="norightborder"><a href="javascript:void(0);" itemid="<?php echo ($val["id"]); ?>" class="del">删除</a></td>
	<?php else: ?>
	  <td width="80" class="norightborder">此项不可删除,只可修改</td><?php endif; ?>
</tr>
    ...
    <tr>
     <input type="hidden" name="add[id][]" readonly="readonly"  value="0">
        ...
        <td width="200">
          ...
          <a href="###" onclick="upyunPicUpload('xmimg_1',500,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
    ...
    <tr>
     <input type="hidden" name="add[id][]" readonly="readonly" value="0">
        ...
        <td width="200">
          ...
          <a href="###" onclick="upyunPicUpload('xmimg_2',500,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
...
<a href="<?php echo U('MicroBroker/index',array('token'=>$token));?>" class="btnGray vm">取消</a>
</td>
</tr>
</tbody>
</table>
  ...
  $(function(){
    $('#addItem').click(function(){
        ...
        var str   = "";
          ...
          str  += '<td width="200"><input type="text" name="add[xmimg][]" value="" class="px" id="xmimg_x'+len+'" style="width:130px;"><a href="###" onclick="upyunPicUpload(\'xmimg_x'+len+'\',500,200,\'<?php echo ($token); ?>\')" class="a_upload">上传</a><a href="###" onclick="viewImg(\'xmimg_x'+len+'\')"> 预览</a></td><td width="100"><input type="text" name="add[tourl][]"  value="" class="px" style="width:100px;" id="tourl_a'+len+'"><br/><a href="###" onclick="addLink(\'tourl_a'+len+'\',0)" class="a_choose" style="margin-top: 7px">从功能库添加</a></td><td width="50" class="norightborder"><a href="javascript:void(0);" itemid="0" class="del">删除</a></td></tr></tbody>';
    ...
    $(".ListProduct").on('click','.del',function(e){
      ...
      if(itemid == 0){
        ...
        if(confirm('确定要删除吗？')){   
            ...
            $.post("<?php echo U('MicroBroker/delitem');?>", obj,
                function(data) {
