...
<div class="content">
          ...
          <div class="cLineB">
              <h4 class="left">
               ...
               <a class="right btnGrayS vm" href="<?php echo U('Member_card/coupons',array('token'=>$token));?>">返回</a>
              <div class="clr"></div>
          </div>
          	<div class="cLine">
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><tr>
					  ...
					  <td><?php echo ($c["cancel_code"]); ?></td>
					  <td><nobr><?php echo (date('Y-m-d H:i:s',$c["add_time"])); ?></nobr></td>
					  <td><?php echo ($c["wechaname"]); ?></td>
					  <td><?php echo ($c["tel"]); ?></td>
					  <td>
						<?php if($c["is_use"] == 1): ?>已核销
						<?php else: ?>
							未使用<?php endif; ?>
					  </td>
					  <td>
						<?php if($c["where"] == 1): echo ($c["wherename"]); ?>
						<?php else: ?>
							会员卡<?php endif; ?>
					  </td>
					  <td class="norightborder">
					  		<?php if($c["is_use"] == '0'): ?><a href="javascript:void(0);" onclick="showLog(<?php echo ($c["cancel_code"]); ?>)">核销</a><?php endif; ?>
					  		<a href="javascript:drop_confirm('您确定要删除吗?','<?php echo U('Member_card/coupons_record_del',array('itemid'=>$c['id'],'id'=>$thisCard['id']));?>');">删除</a>
					  </td>
