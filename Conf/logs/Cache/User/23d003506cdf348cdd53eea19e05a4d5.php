...
<div class="content">
	...
	<div class="cLine servicelist">
		<div class="pageNavigator left">
			<a href="<?php echo U('Service/add',array('token'=>$token,'type'=>'setup'));?>" title="添加客服" class="btnGrayS vm bigbtn">添加客服</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入客服名称"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
	...
	<div class="msgWrap servicelist">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($service_list)): $i = 0; $__LIST__ = $service_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><img src="<?php echo ($vo["headimgurl"]); ?>" width="60px"/></td>
						<td><?php echo ($vo["nickname"]); ?></td>
						<td><?php echo ($vo["wxname"]); ?></td>
						<td class="norightborder" style="border-right:1px solid #eee">
							<a href="<?php echo U('Service/update',array('token'=>$token,'type'=>'setup','id'=>$vo['pigcms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["nickname"]); ?>】吗?', '<?php echo U('Service/del',array('token'=>$token,'id'=>$vo['pigcms_id'],'type'=>'setup'));?>')">删除</a>
						</td>
...
<script>
	...
	function seach(){
		var name = $("#name").val();
		if(name != ""){
			window.location.href="<?php echo U('Service/servicelist',array('token'=>$token,'type'=>'setup'));?>&name="+name;
		}else{
			window.location.href="<?php echo U('Service/servicelist',array('token'=>$token,'type'=>'setup'));?>";
		}
	}
</script>
</div>
