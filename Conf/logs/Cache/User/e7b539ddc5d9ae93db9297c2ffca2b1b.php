...
<div class="cLineB">
	...
	<div class="left" style="margin-right:10px;">
		<form method="POST" action="<?php echo U('WeixinBill/index',array('token'=>$token));?>">
			搜索：
			<select name="key">
			<option value="orderid" <?php if($key == 'orderid'): ?>selected<?php endif; ?>>订单号</option>
			<option value="appid" <?php if($key == 'appid'): ?>selected<?php endif; ?>>收款账号的APPID</option>
			<option value="mchid" <?php if($key == 'mchid'): ?>selected<?php endif; ?>>收款账号的商户号</option>
			</select>　
    ...
    <div class="msgWrap bgfc">
	  <form class="form" method="post" action="" target="_top" enctype="multipart/form-data">	 
		<table border="0" cellspacing="0" cellpadding="0" width="100%" class="platform_table">
			...
			<?php if(is_array($bill_list)): $i = 0; $__LIST__ = $bill_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($vo['from']); ?></td>
					<td><?php echo ($vo['orderid']); ?></td>
					<td><?php echo ($vo['price']); ?></td>
					<td><?php echo (date('Y-m-d H:i:s',$vo['time'])); ?></td>
					<td><?php echo ($vo['plat_type_name']); ?></td>
					<td><?php echo ($vo['appid']); ?></td>
					<td><?php echo ($vo['mchid']); ?></td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			<tr>
				<td colspan="7"></td>
			</tr>
			<tr>
				<td colspan="7"><div style="float:left;">此页总计：总金额 (<font color="blue"><?php echo floatval($bill_count['total']);?>元</font>) ，本账号收款金额 (<font color="blue"><?php echo floatval($bill_count['this']);?>元</font>) ，其他账号收款金额 (<font color="red"><?php echo floatval($bill_count['other']);?>元</font>) ，平台账号收款金额 (<font color="red"><?php echo floatval($bill_count['plat']);?>元</font>)</div></td>
			</tr>
			<tr>
				<td colspan="7"><div style="float:left;">全部总计：总金额 (<font color="blue"><?php echo floatval($total_bill['total']);?>元</font>) ，本账号收款金额 (<font color="blue"><?php echo floatval($total_bill['this']);?>元</font>) ，其他账号收款金额 (<font color="red"><?php echo floatval($total_bill['other']);?>元</font>) ，平台账号收款金额 (<font color="red"><?php echo floatval($total_bill['plat']);?>元</font>)</div></td>
			</tr>
			<tr>
				<td colspan="7"><div style="float:right;"><?php echo ($page); ?></div></td>
			</tr>
		</table>
	</form>
  </div> 
