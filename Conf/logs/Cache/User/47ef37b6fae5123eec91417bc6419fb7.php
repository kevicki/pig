...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
    ...
    <div class="msgWrap form">
        <ul id="tags">
            ...
            <li><a target="_blank" href="<?php echo U('CustomTmpls/myDynamic',array('type'=>'myDynamicTmpls','token'=>$token));?>" style="color:#000;">我的模板</a> </li>
            <li><a target="_blank" href="<?php echo U('CustomTmpls/myDynamic',array('type'=>'dynamicTmpls','token'=>$token));?>" id="listTab" href="javascript:void(0)" style="color:#F56666;">创建动态模板</a> </li>
            <div class="clr"></div>
        </ul>
<!--首页模板选择-->
        ...
        <div class="tagContent selectTag" id="tagContent0" style="display: block;">
            <fieldset>
				<ul class="cateradio g grid" id="grid">
					<?php if(is_array($tmpls)): $i = 0; $__LIST__ = $tmpls;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tpl): $mod = ($i % 2 );++$i;?><li class="mix <?php echo ($tpl["attr"]); if($dynamicTmpls == $tpl['id']){echo ' ck active';} ?>" data-id="<?php echo ($tpl["id"]); ?>">
							<div class="mbtip"><?php echo (($tpl["title"])?($tpl["title"]):'暂无模板描述'); ?></div>
								<img src="<?php echo STATICS;?>/tmpls/images/loading.png" data-original="<?php echo ($tpl["thumbnail"]); ?>" style="width: 143px;height: 207px;margin-top: 88px;margin-left: 1px;margin-bottom: 70px;display: inline;">
								<div class="qrcode" qrcode-url = "<?php echo ($f_siteUrl); ?>/index.php?g=Wap&m=Tmpls&a=show&id=<?php echo ($tpl['id']); ?>&token=<?php echo ($token); ?>" style="width: 115px;height: 115px;z-index: 13;top: 115px;left: 28px;position: absolute;display:none;">
								</div>
								<?php if($tpl['sys_tmpls'] == 0): ?><a href="<?php echo U('CustomTmpls/myDynamic',array('type'=>'editDynamicTmpls','id'=>$tpl['id'],'token'=>$token));?>" target="_blank" class="tmpls_set">编辑</a><?php endif; ?>
								<p>
								<input class="radio" type="radio"<?php if($dynamicTmpls == $tpl['id']): ?>checked<?php endif; ?> name="optype" value="<?php echo ($tpl["id"]); ?>">
								<?php echo ($tpl["title"]); ?>
								</p>
