...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4><?php echo ($thisCard["title"]); ?>--微贺卡设置</h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div>
              ...
              <TBODY>
               <TR>
                ...
                <?php if(is_array($selfs)): $i = 0; $__LIST__ = $selfs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
                  <TH><span class="red"></span><label for="title"><?php echo ($item["title"]); ?>：</label></TH>
                  <TD>
				  <?php
 if($item['infotype']=='select'){ $option = explode('|',$item['rule']); ?>
					<select id="self_<?php echo ($item["id"]); ?>" name="self_<?php echo ($item["id"]); ?>" style="width:380px;">
						<?php if(is_array($option)): $i = 0; $__LIST__ = $option;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option <?php if($item['defaultvalue'] == $vo){ echo 'selected'; }?>><?php echo $vo?></option><?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				  <?php }elseif($item['infotype']=='textarea'){ ?>
					<textarea class="px" id="self_<?php echo ($item["id"]); ?>" name="self_<?php echo ($item["id"]); ?>" style="width:380px;height:120px;"><?php echo (htmlspecialchars_decode($item["defaultvalue"])); ?></textarea>
				  <?php }else{?>
					<input type="text" class="px" id="self_<?php echo ($item["id"]); ?>" value="<?php echo (htmlspecialchars_decode($item["defaultvalue"])); ?>" name="self_<?php echo ($item["id"]); ?>" style="width:380px;">
				  <?php } ?>
                  <?php
 if ($item['infotype']=='image'){ echo "&nbsp;<a href=\"###\" onclick=\"upyunPicUpload('self_".$item['id']."',700,420,'".$token."')\" class=\"a_upload\">上传</a>&nbsp;<a href=\"###\" onclick=\"viewImg('self_".$item['id']."')\" class=\"a_upload\">预览</a>"; } ?>
				  <?php
 if ($item['infotype']=='audio'){ echo "&nbsp;<a href=\"###\" onclick=\"upyunPicUpload('self_".$item['id']."',0,0,'".$token."')\" class=\"a_upload\">上传</a>&nbsp;<a href=\"###\" onclick=\"chooseFile('self_".$item['id']."','music')\" class=\"a_upload\">选择</a>"; } ?>
				  <br/><?php echo ($item["tips"]); ?>
                  </TD>
