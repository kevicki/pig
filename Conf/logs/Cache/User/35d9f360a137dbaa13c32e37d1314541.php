...
<?php if($ischild != 1): ?><div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-top:5px;">
<span>温馨提示：</span>
...
<li class="tabli" id="tab1"><a href="<?php echo U('Hotels/image', array('token' => $token,'cid' => $cid));?>">酒店图集</a></li>
</ul>
</div>
<!--tab end-->
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Hotels/add',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm">添加房间类型</a></div>
查看当天的预定情况：
<select onchange="change(this);">
	<!-- <option value="0">全部预定情况</option> -->
	<?php if(is_array($dates)): $i = 0; $__LIST__ = $dates;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$date): $mod = ($i % 2 );++$i; if($date['k'] == $status): ?><option value="<?php echo ($date['k']); ?>" selected><?php echo ($date['v']); ?></option>
	<?php else: ?>
	<option value="<?php echo ($date['k']); ?>"><?php echo ($date['v']); ?></option><?php endif; endforeach; endif; else: echo "" ;endif; ?>
</select>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		...
		<td><?php echo ($tablelist["price"]); ?>/<?php echo ($tablelist["vprice"]); ?></td>
		<td><?php echo ($tablelist["area"]); ?>(㎡)</td>
		<td><?php echo ($tablelist["num"]); ?></td>
		<td><?php echo ($tablelist["houses"]); ?></td>
		<td><?php echo ($tablelist["usenum"]); ?></td>
...
<script type="text/javascript">
function change(obj) {
	location.href = "<?php echo U('Hotels/index',array('token' => $token, 'cid' => $cid));?>&date=" + $(obj).val();
}
</script>
</div>
</div>
