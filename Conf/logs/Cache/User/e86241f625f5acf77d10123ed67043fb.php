...
<div class="content">
  ...
  <div class="cLineB">
    ...
    <div class="alert">
    <p><span class="bold">使用方法：</span>
   <strong><font color='red'>设置物业公司相关内容,注意：第一次使用，请点击添加物业公司等相关设置，并点击保存 </font></strong>
    </p>
    </div>
   </div>
</div>
...
<a href="<?php echo U('Seckill/action_add',array('token'=>$token,'type'=>$type));?>" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加微秒杀</a>
</div>
<?php if($_SESSION['is_syn']!= 2): ?><div class="alert alert-success alert-dismissable">温馨提示：本功能使用了模板消息中的"订单完成通知"模板消息,模板消息编号OPENTM202521011。<br/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;开通微信支付功能的公众号在使用此功能的时候可以在"基本设置——微信模板消息"中配置对应的模板消息。
</div><?php endif; ?>
        <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
          <thead>
            <tr>
  <th style=" width:15%;">活动标题</th>
  <?php if($_SESSION['is_syn'] == 0): ?><th style=" width:15%;">回复关键词</th><?php endif; ?>
  ...
  <th style=" width:15%;">活动结束时间</th>
			...
			<?php if(is_array($action)): $i = 0; $__LIST__ = $action;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tbody><tr>
						  <td><?php echo ($vo["action_name"]); ?></td>
						  <?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($vo["reply_keyword"]); ?></td><?php endif; ?>
						  <td><?php echo (date("Y-m-d H:i:s",$vo["action_sdate"])); ?></td>
						  <td><?php echo (date("Y-m-d H:i:s",$vo["action_edate"])); ?></td>
						  <td class="norightborder">

<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
   $(document).ready(function(){
      var bheight=document.body.clientHeight;
      $("#lay<?php echo ($vo["action_id"]); ?>").click(function(){
   $("#brg<?php echo ($vo["action_id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($vo["action_id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($vo["action_id"]); ?>").click(function(){
      $("#brg<?php echo ($vo["action_id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($vo["action_id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($vo["action_id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($vo["action_id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute;top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($vo["action_id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($vo["action_id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($vo["action_id"]); ?>"></div>
 <div id="showdiv<?php echo ($vo["action_id"]); ?>">
   <div id="close<?php echo ($vo["action_id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($vo["action_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["action_id"]); ?>&modle=Seckill" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($vo["action_id"]); ?>" style="width:70px;">查看二维码</a><?php endif; endif; ?>
						  <a href="<?php echo U('Seckill/shop_detail',array('aid'=>$vo['action_id'], 'type'=>'seckill'));?>" class="btnGreen">管理活动商品</a>
						  <a href="<?php echo U('Seckill/order_man',array('aid'=>$vo['action_id'], 'type'=>'seckill'));?>" class="btnGreen">查看活动订单</a>
						  <a href="<?php echo U('Seckill/action_add',array('id'=>$vo['action_id'],'op'=>'mod', 'type'=>'seckill'));?>" class="btnGreen">编辑</a>
						  <a href="<?php echo U('Seckill/action_del',array('id'=>$vo['action_id'],'type'=>'del', 'type'=>'seckill'));?>" onclick="return(confirm('确定要删除吗？'))" class="btnGreen">删除</a> </td>
						</tr>
					  </tbody><?php endforeach; endif; else: echo "" ;endif; ?> 
            </table>
			<div class="footactions" style="padding-left:10px">
			  <div class="pages" style="text-align:right;"> <?php echo ($page); ?></div>
			</div>
    </div>
  </div>
</div>
