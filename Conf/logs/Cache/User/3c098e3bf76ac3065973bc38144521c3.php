...
<script>
var diyTool   = "<?php echo U('Img/diyTool');?>";//素材页面   items 添加diyTool
var editor;
...
'insertunorderedlist', '|', 'emoticons', 'image','multiimage', 'link', 'unlink', 'music', 'video','diyTool',
'clearhtml','undo','hr','baidumap','lineheight','table','anchor','preview','print','template','cut','fullscreen']
	});
});
</script>
...
<input type="hidden" id="catUrl" value="<?php echo U('Product/ajaxCatOptions',array('token'=>$token));?>" />
  <div class="content"> 
   <div class="cLineB"> 
    <h4>团购设置</h4> 
    <a href="<?php echo U('Groupon/products',array('token'=>$token));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
   </div> 
    ...
    <div class="msgWrap bgfc"> 
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody> 
       ...
       <tr id="groupon_num_row"> 
        <th><span class="red">*</span>库存：</th> 
        <td><input type="text" id="groupon_num" name="groupon_num" value="<?php echo ($set["groupon_num"]); ?>" class="px" style="width:80px;" /> <span style="color:red;">正常团购需要填写 库存不会用于虚拟商品</span></td> 
       </tr>
       <tr> 
        ...
        <td>
        	...
        	模糊查询：
        	<label>否 <input type="radio" name="precisions" value="0" class="px" <?php if(empty($set['precisions'])):?>checked="checked"<?php endif;?> /></label>　
        	<label>是 <input type="radio" name="precisions" value="1" class="px" <?php if(!empty($set['precisions'])):?>checked="checked"<?php endif;?> /></label>
        </td> 
       </tr>
        <tr> 
        <th><span class="red">*</span>Logo地址：</th>
        <td>
        	<input type="text" name="logourl" value="<?php echo ($set["logourl"]); ?>" class="px" style="width:400px;" id="pic" />
			<script src="/tpl/static/upyun.js"></script>
			<a href="###" onclick="upyunPicUpload('pic',430,230,'<?php echo ($token); ?>')" class="a_upload">上传</a>
			<a href="###" onclick="viewImg('pic')">预览</a>
			<span style="color:red;">图片大小建议 宽430 高230</span>
		</td> 
       </tr>
        <tr class="sn_radio"> 
        <th><span class="red">*</span>虚拟商品：</th>
        <td>
        	<label>否 <input type="radio" name="sn" value="0" class="px" <?php if(empty($set['sn'])):?>checked="checked"<?php endif;?> /></label>　
        	<label>是 <input type="radio" name="sn" value="1" class="px" <?php if(!empty($set['sn'])):?>checked="checked"<?php endif;?> /></label>
        	<span style="color:red;">是虚拟商品的话会自动发贷</span>
        </td> 
       </tr>
        <tr class="sn"> 
        <th>虚拟商品名称：</th>
        <td>
        	<input type="text" name="sn_name" value="<?php echo ($set["sn_name"]); ?>" class="px" />
        	<span style="color:red;">虚拟商品名称（如：电话充值卡账号）</span>
        </td> 
       </tr>
        <tr class="sn"> 
        <th>虚拟商品密码：</th>
        <td>
        	<input type="text" name="sn_pass" value="<?php echo ($set["sn_pass"]); ?>" class="px" />
        	<span style="color:red;">虚拟商品密码（如：电话充值卡密码）</span>
        </td> 
       </tr>
        <TR>
...
$(function(){
	...
	var sn = $('.sn_radio td input:checked').val();
	if (1 == sn ) {
		$('.sn').show();
		$('#groupon_num_row').hide();
	} else {
		$('.sn').hide();
		$('#groupon_num_row').show();
	}
	$('.sn_radio').change(function () {
		$('.sn').toggle();
		$('#groupon_num_row').toggle();
	});
