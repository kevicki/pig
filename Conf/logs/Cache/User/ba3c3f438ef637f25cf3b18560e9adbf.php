...
<div class="pageNavigator left"> 
	<a href='/index.php?g=User&m=Member_card&a=design&token=<?php echo ($token); ?>' title='新增会员卡' class='btnGrayS vm bigbtn'>
	<?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i>
	<?php else: ?>
	<img class="vm" src="./tpl/User/default/common/images/product/add.png"><?php endif; ?>
	添加会员卡</a>
</div>
...
<a href="<?php echo U('Member_card/members', array('token' => $token,'id'=>$item['id']));?>">会员卡管理</a> 
|
<a href="<?php echo U('Member_card/notice', array('token' => $token,'id'=>$item['id']));?>">会员通知</a>
|
<a href="<?php echo U('Member_card/exchange', array('token' => $token,'id'=>$item['id']));?>">积分设置</a> 
|
<?php if($item["is_donate"] == 1): ?><a href="<?php echo U('Member_card/donate', array('token' => $token,'id'=>$item['id']));?>">充值赠送</a> 
|<?php endif; ?>
<a href="<?php echo U('Member_card/create', array('token' => $token,'id'=>$item['id']));?>">会员开卡</a> 
<br />
<a href="<?php echo U('Member_card/design', array('token' => $token,'id'=>$item['id']));?>">修改</a> 
|
<a href="javascript:drop_confirm('您确定要删除吗?', '/index.php?g=User&m=Member_card&a=delete&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>');">删除</a></TD>
			...
			<div class="proc_put_tick" style="padding:0;">
				<div class="frm_control_group radio_row">
					...
					<div class="frm_controls frm_vertical_lh" style="line-height:30px;  padding: 15px 0;">
						...
						<?php if(is_array($cards)): $i = 0; $__LIST__ = $cards;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sitem): $mod = ($i % 2 );++$i;?><option value="<?php echo ($sitem['id']); ?>|<?php echo ($sitem['miniscore']); ?>" <?php if($sitem['sub_give'] == 1): ?>selected<?php endif; ?>><?php echo ($sitem['cardname']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
 ...
 <script>
	$(function(){
		...
		$('#sub').click(function(){
			...
			$.post("<?php echo U('Member_card/change_card_set',array('token'=>$token));?>", {'cardid':cardid}, function(data){
				$('.mask,.dialog_wrp').hide();
			}, 'json');
		});
	});
