...
<form class="form" method="post" action="<?php echo U('Hongbao/action_add',array('token'=>$this->token));?>"  target="_top">
<div class="content">
<!--活动开始-->
<div class="cLineB">
...
<tbody>
  ...
  <td><input type="input" class="px" id="action_name" value="<?php echo ($set["action_name"]); ?>" name="action_name" style="width:400px" data-validation-engine="validate[required,minSize[1],maxSize[40]]" data-errormessage-value-missing="必填项">
  	<br>
...
<tr>
  ...
  <td><input type="input" class="px" id="reply_title" value="<?php echo ($set["reply_title"]); ?>" name="reply_title" style="width:400px"><br>最多填写32个字母(一个汉字相当于3个字母)!
  </td>
</tr>
<tr>
  <th width="120"><span class="red">*</span>回复内容：</th>
  <td><textarea style="width:560px;height:75px" name="reply_content" id="reply_content" class="px"><?php echo ($set["reply_content"]); ?></textarea><br/>最多填写128个字母(一个汉字相当于3个字母)</td>
</tr>

<tr>
  <th>&nbsp;回复图片：</th>
  <td>
  <?php if($set["reply_pic"] != ''): ?><img class="thumb_img" id="picurl_src1" src="<?php echo ($set["reply_pic"]); ?>" style="max-height:100px;max-width: 200px;">
	<?php else: ?>
	<img class="thumb_img" id="picurl_src1" src="<?php echo ($f_siteUrl); ?>/tpl/static/hongbao/images/weixinheti.png" style="max-height:100px;max-width: 200px;"><?php endif; ?>
  <input type="text" name="reply_pic" value='<?php if($set['reply_pic'] != ''): echo ($set["reply_pic"]); else: echo ($f_siteUrl); ?>/tpl/static/hongbao/images/weixinheti.png<?php endif; ?>' id="pic1" class="px" style="width:200px;" >
...
<tr>
  ...
  <td><input type="input" class="px" id="remind_word" value="<?php echo ($set["remind_word"]); ?>" name="remind_word" style="width:200px"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;链接<input type="input" class="px" id="remind_link" value="<?php echo ($set["remind_link"]); ?>" name="remind_link" style="width:200px"> <?php if(session('isQcloud') != true): ?><a href="###" onclick="addLink('remind_link',0)" class="a_choose">从功能库添加</a><?php endif; ?></td>
</tr>
<tr>
  	<th valign="top"><span class="red">*</span>活动介绍：</th>
  	<td><textarea class="px" id="action_desc" name="action_desc" style="width:400px; height: 200px;"><?php echo ($set["action_desc"]); ?></textarea>最多填写256个字母(一个汉字相当于3个字母)!
	</td>
</tr>
<tr>
	<th><span class="red">*</span>活动时间：</th>
	<td><input type="input" class="px" id="start_time" value="<?php echo (date('Y-m-d H:i',($set["start_time"])?($set["start_time"]):time())); ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})" name="start_time" />
		到
		<input type="input" class="px" id="end_time" value="<?php echo (date('Y-m-d H:i',($set["end_time"])?($set["end_time"]): strtotime('+1 month'))); ?>" name="end_time" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'})"  />
	</td>
</tr>
</tbody>
</table>
  	<table class="userinfoArea" style=" margin: 0;" border="0" cellspacing="0" cellpadding="0" width="100%">
  		<tbody>			
			<tr>
  				<th>未关注是否可以参与：</th>
  				<td>
	  				<input type="radio" name="need_follow" value="1" <?php if($set["need_follow"] == 1 || $set["need_follow"] == ''): ?>checked<?php endif; ?>>否&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  				<input type="radio" name="need_follow" value="2" <?php if($set["need_follow"] == 2): ?>checked<?php endif; ?>>是&nbsp;&nbsp;&nbsp;&nbsp;<span><?php echo ($__access_remind__); ?></span>
  				</td>
  			</tr>
			 <tr>
  				<th>是否需要粉丝信息：</th>
  				<td>
	  				<input type="radio" name="need_phone" value="1" <?php if($set["need_phone"] == 1 || $set["need_phone"] == ''): ?>checked<?php endif; ?>>是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  				<input type="radio" name="need_phone" value="2" <?php if($set["need_phone"] == 2): ?>checked<?php endif; ?>>否
					 &nbsp;&nbsp;&nbsp;&nbsp;(没有粉丝详细信息参加活动的处理方式)
  				</td>
  			</tr>
        ...
        <tr>
          ...
          <td>
            <input type="input" class="px" id="min_money" name="min_money"  value="<?php echo ($set["min_money"]); ?>" />  到
			
			 <input type="input" class="px" id="max_money" name="max_money"  value="<?php echo ($set["max_money"]); ?>" /> /元 &nbsp;&nbsp;&nbsp;最大值和最小值之间的数,结果保留2位小数
          </td>
          </tr>
		<tr>
          <th valign="top"><span class="red">*</span>红包总金额：</th>
          <td>
            <input type="input" class="px" id="total_money" name="total_money"   value="<?php echo ($set["total_money"]); ?>" />/元
          </td>
          </tr>
		<tr>
          <th valign="top"><span class="red">*</span>分享人数：</th>
          <td>
            <input type="input" class="px" id="sharetimes" name="sharetimes"   value="<?php echo ($set["sharetimes"]); ?>" />
          </td>
          </tr>
		   <tr>
  				<th>领取方式：</th>
  				<td>
	  				<input type="radio" name="getway" value="1" <?php if($set["getway"] == 1 || $set["getway"] == ''): ?>checked<?php endif; ?>>微信红包&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  				<input type="radio" name="getway" value="2" <?php if($set["getway"] == 2): ?>checked<?php endif; ?>>会员卡充值
  				</td>
  			</tr>
		   <tr>
  				<th>分享朋友圈按钮：</th>
  				<td>
	  				<input type="radio" name="timeline_hide" value="0" <?php if($set["timeline_hide"] == 0 || $set["timeline_hide"] == ''): ?>checked<?php endif; ?>>显示&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  				<input type="radio" name="timeline_hide" value="1" <?php if($set["timeline_hide"] == 1): ?>checked<?php endif; ?>>隐藏 &nbsp;&nbsp;&nbsp;<span style="color:red">红包涉及金钱的交易，由于微信的限制，如果被大量分享到朋友圈可能存在被封号的风险</span>
  				</td>
  			</tr>
  			<tr>
  				<th>是否开启活动：</th>
  				<td>
	  				<input type="radio" name="status" value="1" <?php if($set["status"] == 1 || $set["status"] == ''): ?>checked<?php endif; ?>>开启&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  				<input type="radio" name="status" value="2" <?php if($set["status"] == 2): ?>checked<?php endif; ?>>关闭
  				</td>
  			</tr>
  			<tr>
  				<th></th>
