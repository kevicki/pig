...
<div class="content">
	<div class="cLineB">
		<h4 class="left"><?php if($rname != ''){echo "【".$rname."】";}?>二维码扫描分析</h4>

		<div class="clr"></div>
	</div>
	<?php if(ALI_FUWU_GROUP){?>
	<div class="msgWrap form">
		<ul id="tags" style="width:100%">
			<li <?php if($is_ali != 'y'){?>class="selectTag"<?php }?>>
				<a href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid'],'name'=>$_GET['name'],'is_ali'=>'n'));?>">微信数据</a> 
			</li>
			<li <?php if($is_ali == 'y'){?>class="selectTag"<?php }?>>
				<a href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid'],'name'=>$_GET['name'],'is_ali'=>'y'));?>">服务窗数据</a> 
			</li>
		</ul>
	</div>
	<?php }?>
	...
	<script>
	function doit(month){
	location.href= '<?php echo U("RecognitionData/index",array("token"=>$token,"rid"=>$_GET["rid"],"name"=>$_GET["name"]));?>&month='+month+'&year=<?php echo ($year); ?>';
	}
	</script>
	...
	<?php if($_GET['rid'] != ''){?>
	<div style="height:50px;width:960px;line-height:50px;margin-bottom:-36px"><span style="font-size:20px;font-weight:900;color:#666"><?php if($charts["ifnull"] == 1): ?>【<?php echo ($rname); ?>】本月每日扫描次数和人数统计示例图<span style="color:red">（没有数据）</span><?php else: ?>【<?php echo ($rname); ?>】本月每日扫描次数和人数统计<?php endif; ?></span></div>
	<div id="main" style="height:400px;width:960px;"></div>
	<?php }else{?>
	<div style="height:50px;width:480px;float:left;text-align:center;line-height:50px;margin-bottom:-30px"><span style="font-size:20px;font-weight:900;color:#666"><?php if($cishu["ifnull"] == 1): ?>本月扫描次数分析示例图<span style="color:red">（没有数据）</span><?php else: ?>本月扫描次数分析图<?php endif; ?></span></div> 
	<div style="height:50px;width:470px;float:right;text-align:center;line-height:50px;margin-bottom:-30px"><span style="font-size:20px;font-weight:900;color:#666"><?php if($renshu["ifnull"] == 1): ?>本月扫描人数分析示例图<span style="color:red">（没有数据）</span><?php else: ?>本月扫描人数分析图<?php endif; ?></span></div>
	...
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid'],'name'=>$_GET['name'],'is_info'=>'n'));?>" class="btnGrayS vm bigbtn">
				<img src="tpl/User/default/common/images/text.png" class="vm">查看基本数据
			</a>
			<a href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid'],'name'=>$_GET['name'],'is_info'=>'y'));?>" class="btnGrayS vm bigbtn">
				<img src="tpl/User/default/common/images/text.png" class="vm">查看具体时间数据
			</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入昵称点击搜索"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
		...
		<script>
			...
			function seach(){
				var name = $("#name").val();
				if(name != ""){
					window.location.href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid']));?>&name="+name;
				}else{
					window.location.href="<?php echo U('User/RecognitionData/index',array('token'=>$token,'month'=>$_GET['month'],'year'=>$_GET['year'],'rid'=>$_GET['rid']));?>";
				}
			}
		</script>
		<div class="clr"></div>
	...
	<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="90%">
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				<td>&nbsp;
				<?php if($vo['portrait'] != ''){?>
				<img src="<?php echo ($vo['portrait']); ?>" height="40px">
				<?php }else{echo '暂无';}?>
				</td>
				<td><?php if($vo['nickname'] != ''){ echo ($vo["nickname"]); }else{echo '暂无';}?></td>
