		...
		<li class="tabli <?php if($type == 6): ?>current<?php endif; ?>"><a href="<?php echo U('Member_card/gifts', array('token' => $token,'id'=>$thisCard['id'],'type' => 6));?>">开卡赠送</a></li>
          ...
          <div class="cLine">
			<div class="pageNavigator left">
				<a  href="<?php echo U('Member_card/add_gifts',array('token'=>$token,'id'=>$thisCard['id']));?>" title="添加开卡即送" class="btnGrayS vm bigbtn">
					<img class="vm" src="./tpl/User/default/common/images/product/add.png">
					添加开卡即送
				</a>
			</div>	  
			<a href="<?php echo U('Member_card/index',array('token'=>$token));?>" class="right btnGrayS vm">返回</a>     
			<div class="clr"></div>
<!-- 导入end -->		
          </div>
<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-top:5px;">用户领卡成功后，将会兑现设置好的奖励，赠送优惠劵请先在优惠劵活动中添加“赠送劵”</div>             
          <div class="msgWrap">
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
					  ...
					  <td>
					  	<?php if($list["type"] == '1'): ?>　积分 - <?php echo ($list["item_value"]); ?>点
					  	<?php else: ?>
					  		<?php if($list["item_attr"] == '1'): ?>优惠劵 -
					  		<?php elseif($list["item_attr"] == '2'): ?>
					  		代金劵 -<?php endif; ?>
					  		<?php echo ($list["item_name"]); endif; ?>
					  	</td>
					  <td>
						<?php echo (date("Y-m-d",$list["start"])); ?>
					  </td>
					   <td>
						<?php echo (date("Y-m-d",$list["end"])); ?>
					  </td>
					  <td>
                      ...
                      <td class="norightborder">                       
                    	<a href="index.php?g=User&m=Member_card&a=edit_gifts&gid=<?php echo ($list["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>">修改</a>
                       	<a href="index.php?g=User&m=Member_card&a=del_gifts&gid=<?php echo ($list["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>" onclick="return confirm('确定要删除吗?');">删除</a>
                    </td>
