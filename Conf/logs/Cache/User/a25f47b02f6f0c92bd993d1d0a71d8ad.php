...
<span>温馨提示：</span>
<p>1、<b>餐饮订单支持无线打印，购买无线打印机（购买请直接联系<?php echo ($f_siteName); ?>）后便可以实现订单的自动打印</b><br>此处只显示总店信息，连锁店订单及菜品请进入相应平台进行管理，连锁店登录账号密码及登录地址请在LBS里设置查看</p>
<p>2、本功能使用了模板消息中的“订单完成通”知模板消息，模板编号OPENTM202521011。
          开通微信支付功能的公众号在使用此功能的时候可以在”基本设置——微信模板消息“中配置对应的模板消息</p></div>
<!--tab start-->
<div class="tab">
<ul>
<li class="current tabli" id="tab0"><a href="<?php echo U('Repast/company', array('token' => $token,'cid' => $cid));?>">餐厅管理</a></li>
...
<li class="tabli" id="tab4"><a href="<?php echo U('Repast/Statistics', array('token' => $token,'cid' => $cid));?>">菜品统计</a></li>
<?php if($ischild != 1): ?><li class="tabli" id="tab5"><a href="<?php echo U('Reply_info/set', array('token'=>$token,'infotype'=>'Dining'));?>">订餐回复配置</a></li><?php endif; ?>
</ul>
</div>
<!--tab end-->
     ...
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
	  ...
	  <!--
       <tr> 
        ...
        <option value="0"  <?php if(0 == $company['istakeaway']): ?>selected<?php endif; ?> >不支持</option>
        <option value="1"  <?php if(1 == $company['istakeaway']): ?>selected<?php endif; ?> >支持</option>
        </select>
        </td> 
       </tr>
       <tr> 
        <th>外卖费：</th>
        <td><input type="text" name="price" value="<?php echo ($company["price"]); ?>" class="px" id="pic" style="width:80px;" /> 元</td> 
       </tr>--->
	   <tr> 
        <th>营业时间段：</th>
        <td>
		<div>时间段一从 <input type="input" class="px" name="starttime" id="starttime" value="<?php if($company['starttime'] > 0): echo (date("H:i",$company['starttime'])); endif; ?>" onClick="WdatePicker({dateFmt:'H:mm'})"> 开始
		&nbsp; 到 &nbsp;
		<input type="input" class="px" name="endtime" id="endtime" value="<?php if($company['endtime'] > 0): echo (date("H:i",$company['endtime'])); endif; ?>"  onClick="WdatePicker({dateFmt:'H:mm'})"> 结束</div>
		<br/>
		<div>
		时间段二从 <input type="input" class="px" name="starttime2" id="starttime2" value="<?php if($company['starttime2'] > 0): echo (date("H:i",$company['starttime2'])); endif; ?>" onClick="WdatePicker({dateFmt:'H:mm',minDate:'5:00'})"> 开始
		&nbsp; 到 &nbsp;
		<input type="input" class="px" name="endtime2" id="endtime2" value="<?php if($company['endtime2'] > 0): echo (date("H:i",$company['endtime2'])); endif; ?>"  onClick="WdatePicker({dateFmt:'H:mm',minDate:'5:00'})"> 结束</div>
		...
		<?php if($ischild != 1): ?><tr> 
			<th>分店统一菜品：</th>
			<td>
				<label><input type="radio" name="dishsame" value="1"  id="is_dishSameon" style="width:30px;" <?php if($company['dishsame'] == 1): ?>checked="checked"<?php endif; ?> />是</label>
				&nbsp;&nbsp;&nbsp;
				<label><input type="radio" name="dishsame" value="0"  id="is_dishSameoff" style="width:30px;" <?php if($company['dishsame'] != 1): ?>checked="checked"<?php endif; ?>/>否</label>
        ...
        <th>关闭选择餐桌：</th>
        <td>
			<label><input type="radio" name="offtable" value="1"  id="is_offtableon" style="width:30px;" <?php if($company['offtable'] == 1): ?>checked="checked"<?php endif; ?> />是</label>
			&nbsp;&nbsp;&nbsp;
			<label><input type="radio" name="offtable" value="0"  id="is_offtableoff" style="width:30px;" <?php if($company['offtable'] != 1): ?>checked="checked"<?php endif; ?>/>否</label>
	   ...
	   <tr> 
		...
		<div class="bdrcontent">
		  ...
		  <?php if(!empty($Dcname)): if(is_array($Dcname)): $i = 0; $__LIST__ = $Dcname;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr class="itemtr"><td  style="padding:0;"><input type="text" class="px" name="biaoq[]" style="height: 25px;" value="<?php echo ($item['name']); ?>"><input type="hidden" name="bqid[]" value="<?php echo ($item['id']); ?>"></td><td style="text-align: center;padding:0;"><a href="javascript:void(0);" class="btnGray" onclick="DelThis($(this),<?php echo ($item['id']); ?>)">删 除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; endif; ?>
	   ...
	   <tr> 
        <th>餐厅图片：</th>
		<td><input type="text" name="imgs" value="<?php echo ($company['imgs']); ?>" class="px" id="imgshop" style="width:380px;" /><a href="###" onclick="upyunPicUpload('imgshop',600,400,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('imgshop')"> &nbsp;预览</a>&nbsp;&nbsp;&nbsp;图片建议尺寸[600*400]</td> 
       </tr>
       <tr> 
        <th>开启菜品会员折扣：</th>
        <td>打
		<input type="text" name="discount" class="px" id="discount" style="width:50px;" <?php if($company['discount'] > 0): ?>value="<?php echo ($company['discount']); ?>" <?php else: ?> value=""<?php endif; ?> /> &nbsp;折
	   ...
	   <tr> 
        <th>开启菜品库存管理：</th>
        <td>
		<label><input type="radio" name="kconoff" value="0"  id="is_kcoff" style="width:30px;" <?php if($company['kconoff'] != 1): ?>checked="checked"<?php endif; ?> />关闭</label>
		&nbsp;&nbsp;&nbsp;
		<label><input type="radio" name="kconoff" value="1"  id="is_kcon" style="width:30px;" <?php if($company['kconoff'] == 1): ?>checked="checked"<?php endif; ?>/>开启</label>
       ...
       <!--<tr> 
        ...
        <td>
	        ...
	        <option value="0"  <?php if(0 == $company['payonline']): ?>selected<?php endif; ?> >不要</option>
	        <option value="1"  <?php if(1 == $company['payonline']): ?>selected<?php endif; ?> >需要</option>
	        </select>
        </td> 
       </tr>-->
	    <tr> 
        ...
        <td>
	        ...
	        <option value="0"  <?php if(0 == $company['nowpay']): ?>selected<?php endif; ?> >不需要</option>
			<option value="1"  <?php if(0 != $company['nowpay']): ?>selected<?php endif; ?> >需要</option>
	        </select>
			<span style="display:none;" id="advancepay">
			先预付金额：
			<input type="text" name="advancepay" <?php if($company['advancepay'] > 0): ?>value="<?php echo ($company['advancepay']); ?>"<?php else: ?> value=""<?php endif; ?> class="px" style="width:80px;" /> 元订金就餐（余额再做支付）</span>
			<span class="red">当选择 不需要 时，必须填上预付金额，如果预付金额没填或为0将等同选择了 需要</span>
        </td> 
       </tr>
       <tr> 
        <th>餐桌预订金：</th>
        <td><input type="text" name="subscription" value="<?php echo ($company['subscription']); ?>" class="px" id="subscription" style="width:80px;" /> 元</td> 
       </tr>
	   <tr> 
        <th>本餐厅二维码：</th>
		<?php if($wxuser['winxintype'] == 3): ?><td>
		  <?php if($erweima): if(!empty($erweima['c1']['code_url'])){ echo '<img src="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.$erweima['c1']['code_url'].'" style="width: 250px;height: 250px">&nbsp;&nbsp;&nbsp;<span><a href="'.U('Repast/DownWxewm', array('token' => $token,'cid' => $cid,'ticket'=>$erweima['c1']['code_url'])).'" class="red">下载二维码</a></span>'; }else{ echo '<span><a href="'.U('Recognition/get_Wxticket', array('token' => $token,'cid' => $cid,'rid'=>$erweima['c1']['id'])).'" class="red">点击获取二维码</a></span>'; } ?>
		   <?php else: ?>
		     <?php if ($ischild != 1){ echo '请到 订餐回复配置 中设置关键词回复，设置后这里将显示二维码。'; }else{ echo '<a href="javascript:;" onclick="Get_EWM();" class="red">请点击这里生成渠道来源二维码</a>'; } endif; ?>
		 </td> 
		<?php else: ?>
        <td><img src="<?php echo U('Repast/QRcode', array('token' => $token,'cid' => $cid));?>" style="width: 250px;height: 250px">
		&nbsp;&nbsp;&nbsp;<span><a href="<?php echo U('Repast/QRcode', array('token' => $token,'cid' => $cid,'down'=>1));?>" class="red">下载二维码</a></span>
		</td><?php endif; ?>
       </tr>
	   <tr> 
        <th>餐桌员工独立管理：</th>
		<?php if($wxuser['winxintype'] == 3): ?><td>
		  <?php if($erweima): if(!empty($erweima['c2']['code_url'])){ echo '<img src="https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.$erweima['c2']['code_url'].'" style="width: 250px;height: 250px">&nbsp;&nbsp;&nbsp;<span><a href="'.U('Repast/DownWxewm', array('token' => $token,'cid' => $cid,'ticket'=>$erweima['c2']['code_url'])).'" class="red">下载二维码</a></span>'; }else{ echo '<span><a href="'.U('Recognition/get_Wxticket', array('token' => $token,'cid' => $cid,'rid'=>$erweima['c2']['id'])).'" class="red">点击获取二维码</a></span>'; } ?>
		   <?php else: ?>
		     <?php if ($ischild != 1){ echo '请到 订餐回复配置 中设置关键词回复，设置后这里将显示二维码。'; }else{ echo '<a href="javascript:;" onclick="Get_EWM();" class="red">请点击这里生成渠道来源二维码</a>'; } endif; ?>
		 </td> 
		<?php else: ?>
        <td><img src="<?php echo U('Repast/QRcode', array('token' => $token,'cid' => $cid,'typ'=>'mtable'));?>" style="width: 250px;height: 250px">
		&nbsp;&nbsp;&nbsp;<span><a href="<?php echo U('Repast/QRcode', array('token' => $token,'cid' => $cid,'typ'=>'mtable','down'=>1));?>" class="red">下载二维码</a></span>
		</td><?php endif; ?>
...
NoNowPay(<?php echo ($company['nowpay']); ?>);
...
function DelThis(obj,nid){
   ...
   nid=parseInt(nid);
   if(nid>0){
    $.post("<?php echo U('Repast/delDname', array('token' => $token,'cid' => $cid));?>",{bqid:nid},function(){});
   }
}
</script>
</div>
