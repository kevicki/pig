...
<div class="content">
	<div class="cLineB">
		...
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入昵称搜索"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
...
<form method="post" action="<?php echo U('User/FuwuUser/setGroup',array('token'=>$token));?>">
			<!-- <div style="width:70%;float:left">
				...
				&nbsp;&nbsp;按分组查看粉丝：<select id="wechatgroupid" onchange="location.href='?g=User&m=Wechat_group&a=index&token=<?php echo ($token); ?>&wechatgroupid='+this.value">
					<option value="">未分组</option>
					<?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["wechatgroupid"]); ?>"<?php if ($_GET['wechatgroupid']==$g['wechatgroupid']){echo ' selected';}?>><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div> -->
		</div>
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($fuwuuser_list)): $i = 0; $__LIST__ = $fuwuuser_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><input type="checkbox" value="<?php echo ($vo["pigcms_id"]); ?>" class="cbitem" name="id_<?php echo ($list["id"]); ?>"></td>
						<td><?php echo ($vo["pigcms_id"]); ?></td>
						<td><?php echo ($vo["real_name"]); ?></td>
						<td><?php if($vo["gender"] == "F"): ?>女<?php elseif($vo["gender"] == "M"): ?>男<?php else: ?>未知<?php endif; ?></td>
						<td><?php if($vo["type"] == 0): ?>未分组<?php endif; ?></td>
						<td><?php echo ($vo["province"]); ?></td>
						<td><?php echo ($vo["city"]); ?></td>
						<td><img src='<?php echo ($vo["avatar"]); ?>' class="cateimg_small" width="80px"/></td>
						<td class="norightborder"><?php echo (date('Y-m-d',$vo["addtime"])); ?></td>
...
<script>
	...
	function seach(){
		var name = $(".name").val();
		if(name != ""){
			window.location.href="<?php echo U('FuwuUser/index',array('token'=>$token));?>&name="+name;
		}else{
			window.location.href="<?php echo U('FuwuUser/index',array('token'=>$token));?>";
		}
	}
