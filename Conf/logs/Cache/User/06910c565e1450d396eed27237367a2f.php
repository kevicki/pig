...
<div class="cLineB">
  <h4> 
    <?php if($vo['type'] == 'img' or ($type == 'img')): ?>图片
    <?php elseif($vo['type'] == 'text' or ($type == 'text')): ?>
      文本
    <?php else: ?>
      微现场<?php endif; ?>-投票设置 
</h4>
...
<th width="120">关键词：</th>
<td><input type="text" name="keyword" value="<?php if($vo['keyword'] == ''): ?>投票<?php else: echo ($vo["keyword"]); endif; ?>" class="px" style="width:550px;"><br><span class="red"><?php if($type == 'scene'): ?>添加现场投票，关键词设置失效，无需修改<?php else: ?>只能写一个关键词,功能面板必须开启投票<?php endif; ?></span></td>
</tr>
...
<th>投票图片：</th>
<td>
<input type="text" name="picurl" value="<?php echo ($vo["picurl"]); ?>" class="px" id="picurl" style="width:300px;">
<script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('picurl',700,400,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('picurl')">预览</a>&nbsp;填写图片外链地址，大小为720x400</td>
</tr>
<tr>
<th>图片显示：</th>
<td><label>
...
<tr>
    ...
    <td width="50">排序</td>
     <td width="50">票数</td>
    <?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?><td width="320">图片外链地址&nbsp;&nbsp;&nbsp;&nbsp;如：http://www.pigcms.com</td>
    <td width="120">图片跳转地址</td><?php endif; ?>
    <td class="norightborder" width="180"></td>
</tr>
</tbody>
<?php if($items != ''): if(is_array($items)): $key = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($key % 2 );++$key;?><tbody class="item<?php echo ($ivo["id"]); ?> edit" name="div_add_del">
<tr>
    <td><input type="text" name="item" value="<?php echo ($ivo["item"]); ?>" class="px" style="width:120px;"></td>
    <td><input type="text" name="rank" value="<?php echo ($ivo["rank"]); ?>" style="width:20px;" class="px"></td>
    <td><input type="text" name="vcount" value="<?php echo ($ivo["vcount"]); ?>" style="width:20px;" class="px"></td>
    <?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?><td>
      <input type="text" name="startpicurl" value="<?php echo ($ivo["startpicurl"]); ?>" class="px" id="startpicurl_<?php echo ($key); ?>" style="width:130px;">
<a href="###" onclick="upyunPicUpload('startpicurl_<?php echo ($key); ?>',700,400,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('startpicurl_<?php echo ($key); ?>')">预览</a>
</td>
    <td><input type="text" name="tourl" value="<?php echo ($ivo["tourl"]); ?>" style="width:100px;" class="px"></td><?php endif; ?>
    <td class="norightborder">
      <a href="javascript:void(0);" vid="<?php echo ($ivo["id"]); ?>" class="save">修改</a>
      <a href="javascript:void(0);" vid="<?php echo ($ivo["id"]); ?>" class="del">删除</a>    
    </td>
</tr>
...
<tbody class="item">
    <tr >
     <input type="hidden" name="add[id][]" readonly="readonly" disabled="disabled"  value="" style="width:20px;" >
        ...
        <?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?><td width="200">
          <input type="text" name="add[startpicurl][]" value="" class="px"  id="img_1" style="width:130px;">
          <a href="###" onclick="upyunPicUpload('img_1',700,400,'<?php echo ($token); ?>')" class="a_upload">上传</a>
    ...
    <tr >
     <input type="hidden" name="add[id][]" readonly="readonly" disabled="disabled"  value="" style="width:20px;" >
        ...
        <?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?><td width="200">
          ...
          <a href="###" onclick="upyunPicUpload('img_2',700,400,'<?php echo ($token); ?>')" class="a_upload">上传</a>
...
<tbody>
        <tr>
            <td></td>
            <td colspan="<?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?>6<?php else: ?>4<?php endif; ?>" class="norightborder">
              排序默认为1，票数默认为0，如果是图片投票必须添加图片,图片跳转地址没有可以不填写。
              <a href="javascript:void(0);" class="btnGray right" types="<?php if($vo['type'] == 'img' OR $type == 'img' OR ($vo['type'] == 'scene') OR ($type == 'scene')): ?>1<?php else: ?>0<?php endif; ?>" id="addItem">添加选项</a>
            </td>
        </tr>
...
<th> 参与人数：</th>
<td>
  <input type="text" name="count" value="<?php echo ($vo["count"]); ?>"  class="px">人
</p>
</td>
</tr>
...
<th>投票方式：</th>
<td>
  <input type="radio" name="is_reg" value="1" <?php if($vo["is_reg"] == '1' or $vo["is_reg"] == ''): ?>checked<?php endif; ?>>注册投票&nbsp;&nbsp;
  <input type="radio" name="is_reg" value="2" <?php if($vo["is_reg"] == '2'): ?>checked<?php endif; ?>>匿名投票  &nbsp;(注册投票必须填写个人信息后才能就行投票，匿名投票只要进入页面就能投票)
</td>
</tr>
...
<th>防恶意刷新：</th>
<td>
  <input type="radio" name="refresh" value="1" <?php if($vo["refresh"] == '1' or $vo["refresh"] == ''): ?>checked<?php endif; ?>>
  开启
  　　
  <input type="radio" name="refresh" value="0" <?php if($vo["refresh"] == '0'): ?>checked<?php endif; ?>>关闭  &nbsp;(为防止刷票，推荐开启防刷新选项)
</td>
</tr>
...
<script>
  $(function(){
    $('#addItem').click(function(){
        ...
        if(type == 1){
          str  += '<td width="200"><input type="text" name="add[startpicurl][]" value="" class="px" id="url_'+len+'" style="width:130px;"><a href="###" onclick="upyunPicUpload(\'url_'+len+'\',700,400,\'<?php echo ($token); ?>\')" class="a_upload">上传</a><a href="###" onclick="viewImg(\'url_'+len+'\')">预览</a></td><td width="100"><input type="text" name="add[tourl][]" value="" class="px" style="width:100px;"></td>';
        }
    ...
    $(".save").on('click',function(){
      ...
      $.post("<?php echo U('Vote/save_item',array('token'=>$token,'id'=>$vo['id']));?>", data,
          function(res) {
            if(res == 1){
              alert('修改成功');
            }else{
