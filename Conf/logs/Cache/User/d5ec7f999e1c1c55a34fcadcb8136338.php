...
<link rel="stylesheet" href="<?php echo ($staticPath); ?>/tpl/static/card/css/card_dispel.css"/>
<div class="content">
	<div class="cLineB">
	...
	<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-top:5px;">
		“普通卡券”可被粉丝领取。 <?php if($wxuser["wx_coupons"] == 1): ?>“微信卡券”通过审核后才能领取。<?php endif; ?> “赠送券”无法领取，设置“开卡赠送”时可选择使用
		<br/>
		“状态”若显示“活动中”则说明此优惠券在微活动优惠券里使用中，会员卡里将不再显示此优惠券，只能在微活动优惠券中抽取。
	</div>  
	<div class="cLine">
		<div class="left">
			<a href="javascript:void(0);" title="添加卡券" class="btnGrayS vm bigbtn" id="addCard"><img class="vm" src="./tpl/User/default/common/images/product/add.png">普通卡券</a>
			<!-- <a href="<?php echo U('Member_card/coupons_set',array('token'=>$token,'coupon_type'=>'local'));?>" title="新增会员卡" class="btnGrayS vm bigbtn"><img class="vm" src="./tpl/User/default/common/images/product/add.png">普通卡券</a>
			<a href="<?php echo U('Member_card/coupons_set',array('token'=>$token,'coupon_type'=>'gift'));?>" title="新增会员卡" class="btnGrayS vm bigbtn"><img class="vm" src="./tpl/User/default/common/images/product/add.png">赠送卡券</a>
			<?php if($wxuser["wx_coupons"] == 1): ?><a href="<?php echo U('Member_card/coupons_set',array('token'=>$token,'coupon_type'=>'weixin'));?>" title="新增会员卡" class="btnGrayS vm bigbtn"><img class="vm" src="./tpl/User/default/common/images/product/add.png">微信卡券</a><?php endif; ?> -->
		</div>
		<div class="right" style="line-height:30px;margin:5px 0;">
       	<form action="<?php echo U('Member_card/coupons',array('token'=>$token));?>" method="POST">
		   <select name="cardid">
				<option value="">请选择所属会员卡</option>
				<?php if(is_array($allcard)): $i = 0; $__LIST__ = $allcard;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$allcard): $mod = ($i % 2 );++$i;?><option value="<?php echo ($allcard["id"]); ?>" <?php if($allcard['id'] == $cardid): ?>selected<?php endif; ?>>ID<?php echo ($allcard["id"]); ?>-<?php echo ($allcard["cardname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
          	<select name="is_weixin">
				<option value="-1"<?php if($is_weixin == ''): ?>selected<?php endif; ?>>请选择卡券类型</option>
				<option value="0" <?php if($is_weixin != '' and $is_weixin == 0): ?>selected<?php endif; ?>>普通卡券</option>
				<option value="1" <?php if($is_weixin == 1): ?>selected<?php endif; ?>>微信卡券</option>
				<option value="2" <?php if($is_weixin == 3): ?>selected<?php endif; ?>>赠送卡券</option>
			</select>
          		     
	        <select name="type">
	            <option value="-1"<?php if($type == ''): ?>selected<?php endif; ?>>请选择卡券属性</option>
	            <option value="1" <?php if($type == 1): ?>selected<?php endif; ?>>优惠券</option>
	            <option value="2" <?php if($type == 2): ?>selected<?php endif; ?>>礼品券</option>
				<option value="0" <?php if($type != '' and $type == 0): ?>selected="true"<?php endif; ?>>代金券</option>
	        </select>
			-
          	<input type="text" name="keyword" value="<?php echo ($keyword); ?>" placeholder="请填写卡券名称" class="px">
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
                ...
                <tr>
					...
					<th class="time" style="width:40px;">
						...
						<a target="999" class="tooltips">
							<img src="./tpl/User/default/common/images/price_help.png" align="absmiddle">
							<span>
              ...
              <tbody>
                <?php if(is_array($data_vip)): $i = 0; $__LIST__ = $data_vip;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data_vip): $mod = ($i % 2 );++$i;?><tr>
					<td>
						<?php if($data_vip["is_weixin"] == 1): ?>(微)
					  	<?php elseif($data_vip["attr"] == '1'): ?>
							(赠)
						<?php else: ?>
							(普)<?php endif; ?>
						<?php if($data_vip["type"] == '1'): ?>优惠券
					  	<?php elseif($data_vip["type"] == '2'): ?>
							礼品券
						<?php else: ?>
							代金券<?php endif; ?>
					 </td> 
					 <td>
						<?php echo ($data_vip["title"]); ?>
						<!--<p>门店：<?php echo ($data_vip["company_name"]); ?></p>
						<p>有效期：<?php echo (date('Y-m-d',$data_vip["statdate"])); ?> 至 <?php echo (date('Y-m-d',$data_vip["enddate"])); ?></p>-->
					  </td>
					   
					  <!-- <td><?php echo ($data_vip["cardname"]); ?></td> -->
					  <td>
						<span class="total_<?php echo ($data_vip["id"]); ?>"><?php echo ($data_vip["total"]); ?></span><br/>
						<input type="text" name="stock" class="px number_<?php echo ($data_vip["id"]); ?>" style="width:40px;"><input type="button" data-id="<?php echo ($data_vip["id"]); ?>" class="btnGrayS editStock" value="确定">
					  </td>
					  <td><?php echo ($data_vip["count"]); ?></td>
					  <td><?php echo ($data_vip["usetime"]); ?></td>
					  <td>
						<?php if($data_vip["is_check"] == 1): ?><span style="color:#44b549;">已审核</span>
						<?php else: ?>
							<span style="color:#f00;">未审核</span><?php endif; ?>
						<?php if($data_vip["is_huodong"] != 0): ?><br/>
							<span style="color:red">活动中</span><?php endif; ?>
					  </td>
					  <td class="norightborder" align="center">
						<a href="<?php echo U('Member_card/coupons_record',array('id'=>$data_vip['cardid'],'type'=>'coupon','itemid'=>$data_vip['id']));?>">统计</a>
						<?php if($data_vip["is_weixin"] == 1): ?><a href="<?php echo U('Member_card/coupons_save',array('token'=>$token,'itemid'=>$data_vip['id']));?>">编辑</a> 
						<?php else: ?>
							<a href="<?php echo U('Member_card/coupons_set',array('token'=>$token,'itemid'=>$data_vip['id']));?>&coupon_type=<?php if($data_vip["attr"] == '1'): ?>gift<?php else: ?>local<?php endif; ?>">编辑</a><?php endif; ?>
						<a href="<?php echo U('Member_card/coupons_del',array('token'=>$token,'itemid'=>$data_vip['id']));?>" onclick="return confirm('确定要删除？');">删除</a>
					  </td>
				...
				$(function(){
					$('.editStock').click(function(){
						...
						if(ex.test(number) && number != 0){
							...
							$.post("<?php echo U('Member_card/coupons_stock',array('token'=>$token));?>",post_data,function(res){
								if(res.errcode > 0){
									alert(res.errmsg);
								}else{
...
<script>
	$(function(){
		var is_weixin 	= '<?php echo ($wxuser["wx_coupons"]); ?>';
		...
		$('#sub').click(function(){
			...
			var url 	= "<?php echo U('Member_card/coupons_set',array('token'=>$token));?>";
