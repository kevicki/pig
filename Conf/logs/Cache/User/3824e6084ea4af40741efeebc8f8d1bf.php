	...
	<ul>
		...
		<li class="tabli <?php if($type == 6): ?>current<?php endif; ?>"><a href="<?php echo U('Member_card/gifts', array('token' => $token,'id'=>$thisCard['id'],'type' => 6));?>">开卡赠送</a></li>
          ...
          <div class="cLine">
			...
			<a href="<?php echo U('Member_card/staffSet',array('token'=>$token,'id'=>$thisCard['id']));?>" title="" class="btnGrayS vm bigbtn"><img class="vm" src="./tpl/User/default/common/images/product/add.png">添加职员</a>　
			</div>
          ...
          <input name="delall" type="hidden" value="del">
           <input name="wxid" type="hidden" value="gh_423dwjkewad">
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($staffs)): $i = 0; $__LIST__ = $staffs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$staff): $mod = ($i % 2 );++$i;?><tr>
					 <td><?php echo ($staff["id"]); ?></td>
					  <td><?php echo ($staff["companyName"]); ?></td>
					 <td><?php echo ($staff["name"]); ?></td>
					  <td><?php echo ($staff["tel"]); ?></td>
					  <td><?php echo (date('Y-m-d',$staff["time"])); ?></td>
					  <td class="norightborder">
						<a href="<?php echo U('Member_card/staffSet',array('itemid'=>$staff['id'],'id'=>$thisCard['id']));?>">编辑</a> <a href="<?php echo U('Member_card/staffDelete',array('itemid'=>$staff['id'],'id'=>$thisCard['id']));?>">删除</a>
					  </td>
