...
<div class="cLineC" style="line-height: 18px;">
 <a href="javascript:add_device()" class="btnGrayS vm bigbtn" >添加设备</a>　 <a href="<?php echo U('Shakearound/manual_update',array('token'=>$token,'pagfirstRowe'=>$pagfirstRowe,'listRows'=>$listRows));?>" class="right btnGrayS vm" style="margin-top:4px">手动更新设备状态</a>
</div>
...
<?php if(is_array($device_list)): $i = 0; $__LIST__ = $device_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
<td><?php echo ($li["device_id"]); ?></td>
<td><?php echo ($li["device_comment"]); ?></td>
<td><?php echo ($li["uuid"]); ?></td>
<td><?php echo ($li["major"]); ?></td>
<td><?php echo ($li["minor"]); ?></td>
<td><?php if($li["status"] == 0): ?><span style="color:red"> 未激活</span>
<?php elseif($li["status"] == 1): ?>已激活但不活跃<?php elseif($li["status"] == 2): ?>活跃
<?php else: ?> 审核中<?php endif; ?></td>
<td title="<?php echo ($li["page_name"]); ?>"><?php echo (mb_substr($li["page_name"],0,40)); ?></td>
<td class="norightborder">
    <a href="<?php echo U('Shakearound/edit_device',array('id'=>$li['id'],'token'=>$token));?>" >编辑</a>      |
    <a href="<?php echo U('Shakearound/bindpage',array('token'=>$token,'id'=>$li['id']));?>">关联页面</a>  | <a href="javascript:;" onclick="statistics(<?php echo ($li['status']); ?>,'<?php echo ($token); ?>',<?php echo ($li['device_id']); ?>)">统计信息</a>
...
<div class="cLine">
  ...
  <!--底部234-->
	...
	<script>
	function add_device(){
		window.location.href="/index.php?g=User&m=Shakearound&a=apply_device&token=<?php echo ($token); ?>";
