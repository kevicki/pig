...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4><?php echo ($thisGame["title"]); ?>游戏设置</h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div>
         
          <form method="post" enctype="multipart/form-data" >
          <div class="msgWrap form">
            <TABLE class="userinfoArea" border="0" cellSpacing="0" cellPadding="0" width="100%">
              <?php if($_SESSION['is_syn']<= 0): ?><THEAD>
                <TR>
                  <TH style="width:120px" valign="top"><label for="keyword">关键词：</label></TH>
                  <TD><input type="text" class="px" id="keyword" value="<?php echo ($info["keyword"]); ?>"  name="keyword" style="width:580px;"></TD>
                </TR>
              </THEAD><?php endif; ?>
              <TBODY>
               <TR>
                ...
                <TR>
                  ...
                  <TD><textarea  class="px" id="intro" name="intro" style="width:580px;  height:100px"><?php echo ($info["intro"]); ?></textarea><br />限制200字内
				</TD>
                </TR>
				<TR>
                  <TH valign="top"><label for="pic">封面图片地址：</label></TH>
                  <TD><input type="text" class="px" id="picurl"  name="picurl" value="<?php echo ($info["picurl"]); ?>" style="width:580px;"  />  <script src="/tpl/static/upyun.js?<?php echo date('YmdHis',time());?>"></script><a href="###" onclick="upyunPicUpload('picurl',700,420,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('picurl')">预览</a>
					</TD>
                </TR>
				<TR>
                  <TH valign="top"><label for="text">游戏规则：</label></TH>
                  <TD><textarea  class="px" id="rule" name="gameSet_rule" style="width:500px;  height:300px"><?php echo ($info["rule"]); ?></textarea><br />
				</TD>
                </TR>
				<TR>
                  <TH valign="top"><label for="text">游戏奖项：</label></TH>
                  <TD><textarea  class="px" id="awards" name="gameSet_awards" style="width:500px;  height:300px"><?php echo ($info["awards"]); ?></textarea><br /><font color="red">* 不填，则不显示奖项</font>
				</TD>
                </TR>
                <?php if(session('is_syn') !== 2){ ?>
				<tr>
					<th>是否需要关注：</th>
					<td>
					  <input type="radio" name="is_attention" <?php if($info['is_attention']): if($info['is_attention'] == 0): ?>checked="checked"<?php endif; else: ?>checked="checked"<?php endif; ?> value="0" checked="">否
					  &nbsp;&nbsp;&nbsp;
					  <input type="radio" name="is_attention" <?php if($info['is_attention'] == 1): ?>checked="checked"<?php endif; ?> value="1">是
				...
				<tr>
					<th>是否需要填写手机号：</th>
					<td>
					  <input type="radio" name="is_phone" <?php if($info['is_phone']): if($info['is_phone'] == 0): ?>checked="checked"<?php endif; else: ?>checked="checked"<?php endif; ?> value="0" checked="">否
					  &nbsp;&nbsp;&nbsp;
					  <input type="radio" name="is_phone" <?php if($info['is_phone'] == 1): ?>checked="checked"<?php endif; ?> value="1">是
					</td>
				</tr>
                <?php if($selfs): ?><TR style="margin-bottom:20px">
                ...
                <?php if(is_array($selfs)): $i = 0; $__LIST__ = $selfs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><TR>
                  <TH><span class="red"></span><label for="title"><?php echo ($item["name"]); ?>：</label></TH>
                  <TD><input type="text" class="px" id="self_<?php echo ($item["id"]); ?>" value="<?php echo ($item["defaultvalue"]); ?>" name="self_<?php echo ($item["id"]); ?>" style="width:380px;"> 
                  <?php
 if ($item['infotype']=='img'){ echo "&nbsp;<a href=\"###\" onclick=\"upyunPicUpload('self_".$item['id']."',".$item['width'].",".$item['height'].",'".$token."')\" class=\"a_upload\">上传</a>&nbsp;<a href=\"###\" onclick=\"viewImg('self_".$item['id']."')\">预览</a>"; if ($item['example']){ echo "&nbsp;<input type='hidden' id='self_".$item['id']."_example' value='".$item['example']."' /><a href=\"###\" onclick=\"viewImg('self_".$item['id']."_example')\">查看示例图片</a>"; } echo '<br>宽度'.$item['width'].'px&nbsp;&nbsp;高度'.$item['height'].'px'; } if ($item['info']){ echo '<br>'.$item['info']; } ?>
                  </TD>
                </TR><?php endforeach; endif; else: echo "" ;endif; endif; ?>
                <TR>
                  <TH></TH>
                  <TD>
                  <?php if($id): ?><input type="hidden" value="<?php echo ($id); ?>" name="id" /><?php endif; ?>
