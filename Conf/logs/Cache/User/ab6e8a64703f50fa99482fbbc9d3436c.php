...
$(function(){
	$("select[name='cardid']").change(function(){
		var cardid = $(this).val();
		if(cardid == ''){
			$('.couponlist').html('<option value=\'\'>请选择优惠券</option>');
			$(".couponnum").text('');
		}else{
			$.ajax({
				type:"POST",
				url:"<?php echo U('User/Coupon/ajax',array('token'=>$token));?>",
				dataType:"json",
				data:{
					type:"couponlist",
					cardid:cardid
	...
	/*$("select[name='fist']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#fist-couponnum").text('');
		}else{
			var couponnum = $("select[name='fist'] option[value='"+couponid+"']").attr('num');
			$("#fist-couponnum").text('数量不能大于'+couponnum);
		}
	});
	$("select[name='second']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#second-couponnum").text('');
		}else{
			var couponnum = $("select[name='second'] option[value='"+couponid+"']").attr('num');
			$("#second-couponnum").text('数量不能大于'+couponnum);
		}
	});
	$("select[name='third']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#third-couponnum").text('');
		}else{
			var couponnum = $("select[name='third'] option[value='"+couponid+"']").attr('num');
			$("#third-couponnum").text('数量不能大于'+couponnum);
		}
	});
	$("select[name='four']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#four-couponnum").text('');
		}else{
			var couponnum = $("select[name='four'] option[value='"+couponid+"']").attr('num');
			$("#four-couponnum").text('数量不能大于'+couponnum);
		}
	});
	$("select[name='five']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#five-couponnum").text('');
		}else{
			var couponnum = $("select[name='five'] option[value='"+couponid+"']").attr('num');
			$("#five-couponnum").text('数量不能大于'+couponnum);
		}
	});
	$("select[name='six']").change(function(){
		var couponid = $(this).val();
		if(couponid == ''){
			$("#six-couponnum").text('');
		}else{
			var couponnum = $("select[name='six'] option[value='"+couponid+"']").attr('num');
			$("#six-couponnum").text('数量不能大于'+couponnum);
		}
	});*/
...
<table class="userinfoArea" style=" margin: 0;" border="0" cellspacing="0" cellpadding="0" width="100%"><tbody>
<tr>
	<th valign="top"><span class="red">*</span>选择所属会员卡：</th>
	<td>
		<select name="cardid">
			<option value="">请选择所属会员卡</option>
			<?php if(is_array($Member_card_set_list)): $i = 0; $__LIST__ = $Member_card_set_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cardvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($cardvo["id"]); ?>" <?php if($vo['cardid'] == $cardvo['id']){echo "selected";}?>><?php echo ($cardvo["cardname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		&nbsp;<span class="red">只能选择一个所属会员卡的优惠券做本次活动的奖品，网友所持的会员卡可能不同，请在活动说明和宣传中写清楚。</span>
	</td>
</tr>
<tr>
<th valign="top"><span class="red">*</span>优惠券名称1：</th>
<td>
<!-- <input type="input" class="px" id="fist" name="fist" value="<?php echo ($vo["fist"]); ?>" style="width:250px">
请不要多于50字!  -->
<select name="fist" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['fist'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
...
<td><input type="input" class="px" id="fistnums" name="fistnums" value="<?php echo ($vo["fistnums"]); ?>" style="width:60px"> <span id="fist-couponnum" class="red couponnum">在此活动中设置的优惠券数量将取代在会员卡中设置的数量，请谨慎操作。</span>
</td>
                                    </tr>
<tr>
...
请不要多于50字!  -->
<select name="second" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['second'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
                                        </tr>
<tr>
<th valign="top">优惠券数量：</th>
<td><input type="input" class="px" id="secondnums" name="secondnums" value="<?php echo ($vo["secondnums"]); ?>" style="width:60px"> <span id="second-couponnum" class="red couponnum"></span>
</td>
                                        </tr>
<tr>
...
请不要多于50字!  -->
<select name="third" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['third'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
                                        </tr>
<tr>
<th valign="top">优惠券数量：</th>
<td><input type="input" class="px" id="thirdnums" name="thirdnums" value="<?php echo ($vo["thirdnums"]); ?>" style="width:60px"> <span id="third-couponnum" class="red couponnum"></span>
 </td>
                                        </tr>
<tr>
<th valign="top">优惠券名称4：</th>
<td><!-- <input type="input" class="px" id="four" name="four" value="<?php echo ($vo["four"]); ?>" style="width:250px">
请不要多于50字!  -->
<select name="four" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['four'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
                                        </tr>
<tr>
<th valign="top">优惠券数量：</th>
<td><input type="input" class="px" id="fournums" name="fournums" value="<?php echo ($vo["fournums"]); ?>" style="width:60px"> <span id="four-couponnum" class="red couponnum"></span>
 </td>
                                        </tr>
<tr>
<th valign="top">优惠券名称5：</th>
<td><!-- <input type="input" class="px" id="five" name="five" value="<?php echo ($vo["five"]); ?>" style="width:250px">
请不要多于50字!  -->
<select name="five" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['five'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
                                        </tr>
<tr>
<th valign="top">优惠券数量：</th>
<td><input type="input" class="px" id="fivenums" name="fivenums" value="<?php echo ($vo["fivenums"]); ?>" style="width:60px"> <span id="five-couponnum" class="red couponnum"></span>
 </td>
                                        </tr>
<tr>
<th valign="top">优惠券名称6：</th>
<td><!-- <input type="input" class="px" id="six" name="six" value="<?php echo ($vo["six"]); ?>" style="width:250px">
请不要多于50字!  -->
<select name="six" class="couponlist">
<option value="">请选择优惠券</option>
<?php if(is_array($Member_card_coupon_list)): $i = 0; $__LIST__ = $Member_card_coupon_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$couponvo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($couponvo["id"]); ?>" num="<?php echo ($couponvo["total"]); ?>" <?php if($vo['six'] == $couponvo['id']){echo "selected";}?>><?php echo ($couponvo["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</td>
                                        </tr>
<tr>
<th valign="top">优惠券数量：</th>
<td><input type="input" class="px" id="sixnums" name="sixnums" value="<?php echo ($vo["sixnums"]); ?>" style="width:60px"> <span id="six-couponnum" class="red couponnum"></span>
 </td>
                                        </tr>
  </tbody>
...
$(document).ready(function(){
	$("form").submit(function(){
		...
		var sttxt = $('#sttxt').val();//优惠券介绍
		var statdate = $('#statdate').val();//活动开始时间
		var enddate = $('#enddate').val();//活动结束时间
		var info = $('#info').val();//优惠券活动说明
		var endtite = $('#endtite').val();//活动结束公告主题
		var cardid = $('select[name="cardid"]').val();//选择所属会员卡
		var fist = $('select[name="fist"]').val();//请选择优惠券1
		...
		if(keyword == ""){
			...
			alert("请选择所属会员卡");
			buterror(0,"select[name='cardid']");
			return false;
		}else if(fist == ""){
			alert("请选择优惠券1");
			buterror(0,"select[name='fist']");
			return false;
		}else if(fistnums == ""){
			alert("请填写一等奖奖品数量");
			buterror(0,"#fistnums");
