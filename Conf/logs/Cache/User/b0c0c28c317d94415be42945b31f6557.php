...
<link href="<?php echo ($staticPath); ?>/tpl/static/crowdfunding/css/index.css" rel="stylesheet" type="text/css">
        <div class="content">
<div class="cLineB">
<h4 class="left">微众筹</h4>
<div class="clr"></div>
</div>

<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Crowdfunding/basis_set',array('token'=>$token));?>" title="添加众筹" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加众筹</a></div>
<?php if($_SESSION['is_syn'] == 0): ?><div class="searchbar right">
...
<thead>
<tr>
<th width="280">项目信息</th>
<?php if($_SESSION['is_syn'] == 0): ?><th width="60">关键词</th><?php endif; ?>
<th width="70">筹集资金(元)</th>
<th width="50">当前状态</th>
...
<td class="align-left">
	<div class="item_img">
		<img src="<?php echo ($item["pic"]); ?>" alt="<?php echo ($item["name"]); ?>">
	</div>
	<div class="item_content">
		...
		<div class="item_1">
			<p>已筹集</p>
			<p><?php echo ($item["price_count"]); ?></p>
		</div>
		<div class="item_1">
			<p>剩余天数</p>
			<p><?php echo ($item["remain_day"]); ?> 天</p>
		</div>
	</div>
</td>
<?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($item["keyword"]); ?></td><?php endif; ?>
<td>
	<?php echo ($item["fund"]); ?>
	<br/>
	(当前进度：<?php echo ($item["percent"]); ?>)
</td> 
<td>
	<?php if($item["status"] == '1'): ?><b style="color:#993300;">项目信息已保存</b>
	<?php elseif($item["status"] == '2'): ?>
		<b style="color:#993300;">回报设置完成</b>
	<?php elseif($item["status"] == '3'): ?>
		<b style="color:#44b549;">发布中</b>
	<?php elseif($item["status"] == '4'): ?>
		<b style="color:#f05c52;">已结束</b><?php endif; ?>
</td> 
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
   $(document).ready(function(){
      ...
      $("#lay<?php echo ($item["id"]); ?>").click(function(){
   $("#brg<?php echo ($item["id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($item["id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($item["id"]); ?>").click(function(){
      $("#brg<?php echo ($item["id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($item["id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($item["id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($item["id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute;top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($item["id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($item["id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($item["id"]); ?>"></div>
 <div id="showdiv<?php echo ($item["id"]); ?>">
   <div id="close<?php echo ($item["id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($item["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($item["id"]); ?>&modle=Crowdfunding" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($item["id"]); ?>" style="width:100px;">查看二维码</a><br><?php endif; endif; ?>
	<a href="<?php echo U('Crowdfunding/order',array('id'=>$item['id'],'token'=>$token));?>">查看订单</a>&nbsp;
	<a href="<?php echo U('Crowdfunding/basis_set',array('token'=>$token,'id'=>$item['id']));?>">修改</a>&nbsp;
	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Crowdfunding/del',array('token'=>$token,'id'=>$item['id']));?>');">删除</a></td>
