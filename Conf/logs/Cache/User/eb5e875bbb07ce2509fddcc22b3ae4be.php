...
<div class="cLine">
	...
	<form method="POST" action="<?php echo U('Member_card/members',array('token'=>$token,'id'=>$cardid));?>">
		搜索：
    <input  value="<?php echo ($searchkey); ?>" name="searchkey" class="px" style="height:25px;line-height:25px;">
	...
	<a class="right btnGrayS vm" href="<?php echo U('Member_card/index',array('token'=>$token));?>">返回</a>
<span class="clr"></span>
</div>
          <div class="msgWrap">
            ...
            <TH>余额</TH>             
              ...
              <TBODY>
				 <?php if(is_array($members)): $i = 0; $__LIST__ = $members;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><TR>
                  ...
                  <TD width="100">
                  	<?php echo ($list["total_score"]); ?>
                  </TD> 
                  <TD>
                    <?php echo ($list["expensetotal"]); ?>
                  ...
                  <TD class="norightborder">				  
				       <a href="javascript:void(0);" onclick="memberCardRecharge(<?php echo ($list["uid"]); ?>,<?php echo ($thisCard["id"]); ?>)"><strong>充值</strong></a>
				       |  
				       <a href="<?php echo U('Member_card/member',array('token'=>$token,'cardid'=>$thisCard['id'],'number'=>$list['number']));?>" ><strong>消费记录</strong></a>
				       |
				       <a href="javascript:drop_confirm('您确定要删除吗?', '/index.php?g=User&m=Member_card&a=member_del&token=<?php echo ($_GET['token']); ?>&itemid=<?php echo ($list["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>');"><strong>删除</strong></a> 
                  </TD>
