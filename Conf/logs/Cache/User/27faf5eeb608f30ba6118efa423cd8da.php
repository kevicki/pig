...
<div class="ftip" style="margin:25px auto 5px auto;">只有认证服务号才能使用（向微信官方交过300元认证费的服务号）</div>
 ...
 <div class="msgWrap bgfc" style=" padding:5px 10px; margin:0">
   ...
   <table class="ListProduct" border="" cellspacing="0" cellpadding="0" width="100%">
	...
	<tbody>
			  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i; if($list['title'] != '餐饮二维码'): ?><tr>
                  ...
                  <td>
					<?php if($list['code_url'] != ''): ?><a href="<?php if(intval($wxuser['winxintype']) == 3){echo U("User/Recognition/showqr",array('token'=>$token,'qrurl'=>urlencode('https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket='.$list['code_url'])));}else{echo "javascript:alert('只有认证的服务号才可以使用！')";}?>" target="_blank">点击查看微信二维码</a>
					<?php else: ?>
						<a href="<?php echo U('Recognition/get_code',array('id'=>$list['id']));?>">获取微信二维码</a><?php endif; ?>
					<?php if($fuwu == 'yes'){?>
						<br/>
						<?php if($list['fuwu_code_url'] != ''){?>
							<a href="<?php if(intval($wxuser['winxintype']) == 3 || $wxuser['fuwuappid'] != ''){echo U("User/Recognition/showqr",array('token'=>$token,'qrurl'=>urlencode($list['fuwu_code_url'])));}else{echo "javascript:alert('<?php echo ($isgostr); ?>')";}?>" target="_blank">点击查看服务窗二维码</a>
						<?php }else{?>
							<a href="<?php echo U('Recognition/fuwu_code',array('token'=>$token,'id'=>$list['id']));?>">获取服务窗二维码</a>
						<?php }?>
					<?php }?>
					</td>
                  ...
                  <td class="norightborder">
						...
						<br>
						<a href="<?php if(intval($wxuser['winxintype']) == 3 || $wxuser['fuwuappid'] != ''){ echo U('RecognitionData/index',array('rid'=>$list['id'])); }else{echo "javascript:alert('<?php echo ($isgostr); ?>')";}?>">分析</a>
				   </td>
