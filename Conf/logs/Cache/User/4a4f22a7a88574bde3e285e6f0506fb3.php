...
<script src="<?php echo ($staticPath); ?>/tpl/User/default/common/js/date/WdatePicker.js"></script>
...
<li class="current tabli" id="tab2"><a href="<?php echo U('DishOut/basePrice', array('token' => $token,'cid' => $cid));?>">外送类型设置</a></li>
     ...
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody>
       <tr> 
		...
		<label>
		<input type="radio"  name="stype" value="1" <?php if($mtime['stype'] != 2): ?>checked="checked"<?php endif; ?> >
		 设定为起送价
		</label>
		&nbsp;&nbsp;&nbsp;
		<label>
		<input type="radio"  name="stype" id="zc_edate" value="2"  <?php if($mtime['stype'] == 2): ?>checked="checked"<?php endif; ?> > 设定为起步价</label>
		</td>
		<td rowspan="2"><span class="red">设定为起送价：客户点菜下单的金额达到下面设置的金额方生成外送订单。设定为起步价：客户点菜下单金额若未达到下面设置的金额则按照下面设置的金额生成订单外送</span></td>
       </tr>
       <tr> 
        <th> 定价：</th> 
        <td style="width:400px"><span id="tipsforradio1"><?php if($mtime['stype'] == 2): ?>将外送起步价设定为<?php else: ?>客户订单达到<?php endif; ?></span>&nbsp;
        <input type="text" name="pricing" id="pricing" class="px" style="width:70px;" value="<?php if($mtime['pricing'] > 0): echo ($mtime['pricing']); endif; ?>"/>&nbsp;<span id="tipsforradio2"><?php if($mtime['stype'] == 2): ?>元<?php else: ?>元方外送<?php endif; ?></span>
        </td>
		<td></td>
       </tr>
       <tr>
       <th>&nbsp;</th>
       <td>
       <a class="btnGreen" id="saveThis" href="javascript:;">保存</a> &nbsp; <a href="<?php echo U('DishOut/index',array('token' => $token, 'cid' => $cid));?>" class="btnGray vm">取消</a></td> 
       </tr> 
      </tbody> 
     </table> 
     </div>
