...
<div class="content">
          ...
          <div class="cLineB">
              <h4 class="left"><?php echo ($data["name"]); ?> 虚拟商品管理</h4>
                  <div>
  ...
  <div class="cLineB">
            虚拟物品数量：<span class="redamount"><?php echo ($count); ?> </span>个　　已销售的商品：<span class="redamount"><?php echo ($saleCount); ?></span>个          </div>
            ...
            <a href="<?php echo U('User/Groupon/sn', array('token'=>$token, 'id'=>$data['id'], 'status'=>1));?>" class="btnGrayS vm <?php if (!empty($_GET['status'])):?>navHover<?php endif?>" style="float:right;background-color:#B6B8B4;">已出售</a>
            <a href="<?php echo U('User/Groupon/sn', array('token'=>$token, 'id'=>$data['id']));?>" class="btnGrayS vm <?php if (empty($_GET['status'])):?>navHover<?php endif?>" style="float:right;background-color:#B6B8B4;">未出售</a>
            </div>
             <div style="margin-top:20px;display:none" id="import">
             	<form enctype="multipart/form-data" action="?g=User&m=Groupon&a=uploadSNExcel" id="thumbForm" method="POST" style="font-size:14px;padding:10px 20px 10px 0px;">
					...
					注意：不支持xlsx格式</div><input type="hidden" value="<?php echo ($data["id"]); ?>" name="pid" />
				</form>
			</div>
            ...
            <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
              <THEAD>
                <TR style="text-align: center;">
					...
					<TH width="25%"><?php echo ($data["sn_name"]); ?></TH>
					<TH width="25%"><?php echo ($data['sn_pass'] ? $data['sn_pass'] : '密码'); ?></TH>
					<TH width="10%">是否已发</TH>
					<TH>发送时间</TH>
					<TH>操作</TH>
                </TR>
              ...
              <TBODY>
              	<form method="post" action="" id="info">
				<?php if(is_array($models)): $i = 0; $__LIST__ = $models;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$record): $mod = ($i % 2 );++$i;?><TR style="line-height:30px;height: 43px;line-height: 59px;">
					 	<td><input type="checkbox" value="<?php echo ($record["id"]); ?>" class="cbitem" name="id[]"></td>
            			...
            			<td><?php if($record['sendtime'] == 0): else: echo (date('Y-m-d H:i:s',$record["sendtime"])); endif; ?></td>
						<td>
							<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Groupon/snDelete',array('token'=>$token,'id'=>$record['id']));?>');">删除</a>             
            			</td>
