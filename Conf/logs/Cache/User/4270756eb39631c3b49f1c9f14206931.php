...
<script src="/tpl/static/artDialog/plugins/iframeTools.js"></script>
        <div class="content">
<div class="cLineB">
<h4 class="left">外卖订单管理(<?php echo ($page); ?>) </h4>
...
<li class="current tabli" id="tab4"><a href="<?php echo U('DishOut/orders', array('token' => $token,'cid' => $cid));?>">外卖订单管理</a></li>
...
<select onchange="change(this,1);">
	...
	<option value="1" <?php if ($status == 1) {echo "selected";} ?>>未付款</option>
	<option value="2" <?php if ($status == 2) {echo "selected";} ?>>已付款</option>
</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
选择发货：
<select onchange="change(this,2);">
	<option value="0" <?php if ($fstatus == 0) {echo "selected";} ?>>全部状态</option>
	<option value="1" <?php if ($fstatus == 1) {echo "selected";} ?>>未发货</option>
	<option value="2" <?php if ($fstatus == 2) {echo "selected";} ?>>已发货</option>
</select>
<?php if($ischild != 1): ?><a class="btnGrayS vm bigbtn" style="float:right;margin-right: 15px;" title="添加" href="<?php echo U('DishOut/orders',array('token' => $token,'cid'=>$cid,'fd'=>'on'));?>">查看分店订单</a><?php endif; ?>
<div class="clr"></div>
</div>
...
<tbody>
<tr></tr>
<?php if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i; $statustr=''; if($o['paid']==1){ $statustr='<span style="color:green">已付款</span>'; }else{ $statustr='<span style="color:red">未付款</span>'; } if($o['isuse']==1){ $statustr.='<span style="color:green">/已发货</span>'; }else{ $statustr.='<span style="color:red">/未发货</span>'; } ?>
<tr>
<!--<td><input type="checkbox" value="<?php echo ($o["id"]); ?>" class="cbitem" name="id_<?php echo ($i); ?>"></td>-->
<td><?php echo (htmlspecialchars_decode($o["name"],ENT_QUOTES)); ?></td>
<td><?php echo (htmlspecialchars_decode($o["tel"],ENT_QUOTES)); ?></td>
<td><?php echo ($o["total"]); ?>/份</td>
<td><?php echo ($o["price"]); ?></td>
<td><?php echo ($statustr); ?></td>
<td><?php echo (date("Y-m-d H:i:s",$o["time"])); ?></td>
<td><?php if($o["reservetime"] > 0): echo (date("Y-m-d H:i:s",$o["reservetime"])); else: ?>尽快送达<?php endif; ?></td>
<td><?php echo (htmlspecialchars_decode($o["address"],ENT_QUOTES)); ?></td>
<td class="norightborder"><a href="###" onclick="showIntroDetail(<?php echo ($o["id"]); ?>)">详细</a> <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('DishOut/deleteOrder',array('token'=>$token,'id'=>$o['id']));?>');">删除</a></td>
...
function showIntroDetail(id){
	art.dialog.open('<?php echo U('DishOut/orderInfo',array('token'=>$token));?>&id='+id,{lock:false,title:'订单详情',width:1000,height:620,yesText:'关闭',background: '#000',opacity: 0.87, close: function(){location.reload();}});
}
</script>
</div>
<div class="cLine">
...
function change(obj,t) {
	window.location.href = "<?php echo U('DishOut/orders',array('token' => $token, 'cid' => $cid));?>&t="+t+"&status=" + $(obj).val();
}
</script>
</div>
</div>
