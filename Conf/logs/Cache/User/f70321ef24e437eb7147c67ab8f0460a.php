...
<div class="content">
          ...
          <div class="cLineB">
             <h4>3G投票活动信息 (<?php echo ($count); ?>) 条<span class="FAQ">你本月有 <?php echo ($group["activitynum"]); ?> 次机会可免收活动创建费，已经使用了 <?php echo ($activitynum); ?> 次机会!</span></h4>
...
<?php if($is_scene > 0): ?><a href=""  title="微现场投票" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/text.png" class="vm">微现场投票</a><?php endif; ?>
            <?php else: ?>
            <a href="<?php echo U('Vote/add',array('type'=>'text'));?>"  title="发起文字投票" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/text.png" class="vm">发起文字投票</a>　<!--a href="<?php echo U('Voteimg/add_voteimg',array('type'=>'img'));?>" title="发起图片投票" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/pic.png" class="vm">发起图片投票</a-->
            <?php if($is_scene > 0): ?><a href="<?php echo U('Vote/add',array('type'=>'scene'));?>"  title="微现场投票" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/text.png" class="vm">微现场投票</a><?php endif; endif; ?>

  　　      <span class="right">
              <form action="<?php echo U('Vote/index',array('token'=>$token));?>" method="post">
              投票类型：
              <select name="type" id="">
                <option value="">请选择</option>
                <option value="text">文字投票</option>
                <!--option value="img">图文投票</option-->
              <?php if($is_scene > 0): ?><option value="scene">微现场投票</option><?php endif; ?>
              </select>
              关键词：
          ...
          <th>是否开始</th>
              ...
              <tbody>
                <tr></tr>
                  ...
                  <td>
                    <?php if($list['type'] == 'text'): ?><img src="<?php echo RES;?>/images/text.png" class="vm"> 文字投票
                      <?php elseif($list['type'] == 'img'): ?>
                        <img src="<?php echo RES;?>/images/pic.png" class="vm"> 图片投票
                      <?php else: ?>
                        微现场投票<?php endif; ?>
                  </td>
                  ...
                  <td>                    
                    <?php if($list['status'] == 0): ?>已关闭(<a href="javascript:drop_confirm('开始投票活动', '<?php echo U('Vote/setinc',array('type'=>$list['type'],'id'=>$list['id']));?>');">开启</a>) 
                    <?php else: ?>
                      已开启(<a href="javascript:drop_confirm('你是否真的想结束投票活动.', '<?php echo U('Vote/setdes',array('type'=>$list['type'],'id'=>$list['id']));?>');">关闭</a>)<?php endif; ?>
                  </td>
                   <td class="norightborder">
                    <a href="<?php echo U('Vote/totals',array('type'=>$list['type'],'id'=>$list['id']));?>" title="投票结果">投票结果</a> |
                    <a href="<?php echo U('Vote/edit',array('type'=>$list['type'],'id'=>$list['id']));?>" title="编辑">编辑</a> |
                    <a href="javascript:drop_confirm('您确定要删除吗?删除会把投票结果也一起删除！', '<?php echo U('Vote/del',array('type'=>$list['type'],'id'=>$list['id']));?>');">删除</a> 
