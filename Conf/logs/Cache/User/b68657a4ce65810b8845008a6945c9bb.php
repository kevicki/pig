...
<script src="<?php echo STATICS;?>/jquery.qrcode.min.js" type="text/javascript"></script>
<div class="content">
<div class="cLineB">
...
<div class="cLine">
<div class="pageNavigator left"> <a href='/index.php?g=User&m=Person_card&a=design&token=<?php echo ($token); ?>' title='新增微名片' class='btnGrayS vm bigbtn'><img class="vm" src="./tpl/User/default/common/images/product/add.png">添加微名片</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/index.php?g=User&m=Person_card&a=index&token=<?php echo ($token); ?>&o=1' title='我的微名片' class='btnGrayS vm bigbtn'>我的微名片</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='/index.php?g=User&m=Person_card&a=index&token=<?php echo ($token); ?>&o=2' title='用户自己创建的微名片' class='btnGrayS vm bigbtn'>用户微名片</a></div>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<TBODY>
<TR></TR>
<?php if(is_array($card)): $i = 0; $__LIST__ = $card;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; if($falg==2): ?><TR height="80">
            <TD><?php if($item["portrait"] != ''): ?><img src="<?php echo ($item["portrait"]); ?>" style="height:50px;width:50px;"> <?php else: ?><img src="tpl/static/person/images/tx.png" style="height:50px;width:50px;"><?php endif; ?></TD><!--转发时的图标-->
            <TD><?php if($item["username"] != ''): echo ($item["username"]); else: endif; ?></TD><!--转发标题-->
            <TD><?php if($item["mobile"] == ''): ?>尚未填写手机号码<?php else: ?> <?php echo ($item["mobile"]); endif; ?></TD><!--手机号码-->
            <TD>
                <?php if($item["company"] == ''): ?>尚未填写公司名称<?php else: ?> <?php echo ($item["company"]); endif; ?>
            </TD>
            <TD>
                <a class="ewm_view" href="javascript:;">
                  <img src="tpl/static/person/images/6975cd6a733a1ca24313ad.png" width="40" height="40" onmouseover="moveover(<?php echo ($item["id"]); ?>,'{cardUrl}/index.php?m=Wap&c=card&a=index&uid=<?php echo ($item["uid"]); ?>&id=<?php echo ($item["id"]); ?>
')" onmouseout="moveout(<?php echo ($item["id"]); ?>)">
                    <div class="ewm_big" id="showmove<?php echo ($item["id"]); ?>" style="display: none;"></div>
                </a>
            </TD>
        </TR>
    <?php else: ?>
        <TR height="80">
            <TD><?php if($falg==2): else: ?><input type="checkbox" name="check" value="<?php echo ($item["id"]); ?>"><?php endif; ?></TD><!--名片id-->
            <TD><?php if($item["portrait"] != ''): ?><img src="<?php echo ($item["portrait"]); ?>" style="height:50px;width:50px;"> <?php else: ?><img src="tpl/static/person/images/tx.png" style="height:50px;width:50px;"><?php endif; ?></TD><!--转发时的图标-->
            <TD><?php if($item["username"] != ''): echo ($item["username"]); ?>的名片<?php else: endif; ?></TD><!--转发标题-->
            <TD><?php echo ($item["username"]); ?></TD><!--转发姓名-->
            <TD>
                <span onclick="setcontent(<?php echo ($item["id"]); ?>);" id="content_<?php echo ($item["id"]); ?>" style="cursor:pointer;word-break:break-all"><div class="deftxt"><?php if($item["forward_content"] == ''): ?>点击添加转发内容<?php else: ?> <?php echo ($item["forward_content"]); endif; ?></div></span>
            </TD>
            <TD>
                <a class="ewm_view" href="javascript:;">
                    <img src="tpl/static/person/images/6975cd6a733a1ca24313ad.png" width="40" height="40" onmouseover="moveover(<?php echo ($item["id"]); ?>,'{cardUrl}/index.php?m=Wap&c=card&a=index&uid=<?php echo ($item["uid"]); ?>&id=<?php echo ($item["id"]); ?>
')" onmouseout="moveout(<?php echo ($item["id"]); ?>)">
            ...
            <TD class="norightborder">
                <a href="<?php echo U('Person_card/design', array('token' => $token,'id'=>$item['id']));?>">修改名片</a>
                <a href="javascript:drop_confirm('您确定要删除吗?', '/index.php?g=User&m=Person_card&a=del&token=<?php echo ($token); ?>&id=<?php echo ($item["id"]); ?>');">删除名片</a>
                <a href="<?php echo U('Person_card/picer', array('token' => $token,'id'=>$item['id'],'uid'=>$item['uid']));?>"   id=pic1 onclick="savepic();return false;" style="cursor:hand">下载名片</a>
            </TD>
...
<div class="pages">
    <div class="float-lf mg-left-10">
       <?php if($num == 0 or $falg == 2): else: ?>
       <input type="checkbox" id="qx" onclick="allcheck()"><span style="cursor: pointer;margin-left:5px;margin-top:-5px">全选</span><a href="javascript:;" onclick="deletered();" style="margin-left:10px;">批量删除</a><?php endif; ?>
    </div>
...
function deletered(){
	...
	$.ajax({
		url:"/index.php?g=User&m=Person_card&a=delALL&token="+"<?php echo ($token); ?>",
		type:"post",
		dataType: 'json',
		data:{"id":idAll},
...
function showallkey(){
	location.href="/index.php?g=User&m=Person_card&a=index&token="+"<?php echo ($token); ?>";
}
function moveover(id,url){
    url = url.replace('{cardUrl}','<?php echo ($serverUrl); ?>');
    $("#showmove"+id).html('');
    $("#showmove"+id).qrcode({
...
function chgcontent(tid) {
	...
	if(content.length>50) {
		...
		//alert("<?php echo ($item["id"]); ?>");
		//alert("index.php?g=User&m=Person_card&a=forward&token="+"<?php echo ($token); ?>"+"&id="+tid);
		if(content!=$("#texthidd").val()){
			$.post("/index.php?g=User&m=Person_card&a=forward&token="+"<?php echo ($token); ?>"+"&id="+tid,{content:content},function(data) {
				if(data.status == '1') {
					if(content=='') {
