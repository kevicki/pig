...
<div class="content"> 
	<div class="cLineB"> 
		<h4>模板设置</h4> 
		<a href="<?php echo U('TemplateMsg/index',array('token' => $token));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
	</div> 
	
	<form method="post" action="" id="formID">
		<input type="hidden" name="id" value="<?php echo ($kitchen["id"]); ?>" />
		<div class="msgWrap bgfc"> 
			<table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
				<tbody>
					<tr> 
						<th><span class="red">*</span>模板编号：</th> 
						<td>
						<?php if($tempmsg['id'] > 0): echo ($tempmsg["tempkey"]); ?>
							<input type="hidden" name="tempkey" value="<?php echo ($tempmsg["tempkey"]); ?>"/>
						<?php else: ?>
							<input type="text" name="tempkey" id="tempkey" value="<?php echo ($tempmsg["tempkey"]); ?>" class="px" style="width:400px;"/><?php endif; ?>
						</td> 
					</tr>
					<tr> 
						<th><span class="red">*</span>模板ID：</th> 
						<td>
							<input type="text" name="tempid" id="tempid" value="<?php echo ($tempmsg["tempid"]); ?>" class="px" style="width:400px;" />
						</td> 
					</tr>
					<tr> 
						<th>模板名：</th> 
						<td>
						<?php if($tempmsg['id'] > 0 AND $tempmsg['type'] == 0): echo ($tempmsg["name"]); ?>
							<input type="hidden" name="name" value="<?php echo ($tempmsg["name"]); ?>"/>
						<?php else: ?>
							<input type="text" name="name" id="name" value="<?php echo ($tempmsg["name"]); ?>" class="px" style="width:400px;"/><?php endif; ?>
						</td> 
					</tr>
					<tr> 
						<th>所属行业：</th> 
						<td>
						<?php if($tempmsg['id'] > 0 AND $tempmsg['type'] == 0): echo ($tempmsg["industry"]); ?>
							<input type="hidden" name="industry" value="<?php echo ($tempmsg["industry"]); ?>"/>
						<?php else: ?>
							<input type="text" name="industry" id="industry" value="<?php echo ($tempmsg["industry"]); ?>" class="px" style="width:400px;"/><?php endif; ?>
						</td> 
					</tr>
					<tr> 
						<th>头部颜色：</th> 
						<td>
							<input type="text" name="topcolor" id="topcolor" value="<?php echo ($tempmsg["topcolor"]); ?>" class="px color" style="width: 55px; background:<?php echo ($tempmsg["topcolor"]); ?>; color: rgb(255, 255, 255);" />
						</td> 
					</tr>
					<tr> 
						<th>文字颜色：</th> 
						<td>
							<input type="text" name="textcolor" id="textcolor" value="<?php echo ($tempmsg["textcolor"]); ?>" class="px color" style="width: 55px; background:<?php echo ($tempmsg["topcolor"]); ?>; color: rgb(255, 255, 255);" />
						</td> 
					</tr>
					<tr> 
						<th>状态：</th> 
						<td>
				        	<label> <input type="radio" name="status" value="0" class="px" <?php if($tempmsg['status'] == 0): ?>checked="checked"<?php endif; ?>>关闭</label>　
				        	<label> <input type="radio" name="status" value="1" class="px" <?php if($tempmsg['status'] == 1): ?>checked="checked"<?php endif; ?>>开启</label>
				        </td> 
					</tr>
					<tr> 
						<th><span class="red">*</span>回复内容：</th> 
						<td>
						<?php if($tempmsg['id'] > 0): ?><pre><?php echo ($tempmsg["content"]); ?></pre>
							<input type="hidden" name="content" value="<?php echo ($tempmsg["content"]); ?>"/>
						<?php else: ?>
							<textarea style="width:400px;height:120px;" name="content" id="content" class="px"><?php echo ($tempmsg["content"]); ?></textarea><?php endif; ?>
						</td> 
					</tr>
					<tr>  
						<th>&nbsp;</th>
						<td>
							<button type="submit" name="submit" class="btnGreen" id="save">保存</button> &nbsp; 
							<a href="<?php echo U('TemplateMsg/index',array('token' => $token));?>" class="btnGray vm">取消</a>
						</td> 
					</tr> 
				</tbody> 
			</table> 
