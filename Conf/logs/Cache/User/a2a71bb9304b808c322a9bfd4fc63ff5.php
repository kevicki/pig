...
'insertunorderedlist', '|', 'emoticons', 'image','link', 'unlink','baidumap','lineheight','table','anchor','preview','print','template','code','cut', 'music', 'video']
  ...
  <div class="content"> 
   <div class="cLineB"> 
    <h4>活动设置</h4>
    <a href="<?php echo U('Problem/index',array('token'=>$token));?>" class="right  btnGreen" style="margin-top:-27px">返回</a> 
   </div> 
...
<input type="hidden" name="id" value="<?php echo ($set["id"]); ?>" />
    <div class="msgWrap bgfc"> 
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody> 
 <tr>
  <th valign="top"><span class="red">*</span><label for="keyword">活动名称：</label></th>
  <td>
      <input type="input" class="px" id="mtitle" value="<?php echo ($set['name']); ?>" name="name" style="width:400px">
    </td>
 </tr>
 <tr>
  <th valign="top"><span class="red">*</span><label for="keyword">触发关键词</label></th>
  <td>
    <input type="input" class="px" id="keyword" value="<?php echo ($set['keyword']); ?>" name="keyword">
         10个字以内         
  </td>
</tr>         
<tr>
...
<tr>
  ...
  <td>
    <input type="input" class="px" id="logo_pic" value="<?php if($set['logo_pic'] == ''): echo ($default); else: echo ($set['logo_pic']); endif; ?>" name="logo_pic"  style="width:320px">
<script src="<?php echo STATICS;?>/upyun.js"></script>
<a href="javascript:void(0);" onclick="upyunPicUpload('logo_pic',360,200,'<?php echo ($token); ?>')" class="a_upload">上传</a>
<a href="javascript:void(0);" onclick="viewImg('logo_pic')">预览</a> [360*200] 
       ...
       <tr> 
        <th><span class="red">*</span>活动横幅：</th> 
        <td>
          <input type="text" name="banner" value="<?php if($set["banner"] == ''): ?>./tpl/static/problem/image/default.png<?php else: echo ($set["banner"]); endif; ?>" class="px" id="banner" style="width:320px;" />
          <script src="/tpl/static/upyun.js"></script>
          <a href="###" onclick="upyunPicUpload('banner',700,700,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('banner')">预览</a>
        </td> 
       </tr> 
       ...
       <tr> 
        ...
        <td>
          <textarea name="explain" class="px" id="explain" style="width:400px;height:80px;"><?php if($set["explain"] == ''): ?>亲，点击进入问答页面开始答题，累计答题赢礼品呦，快来参加活动吧！<?php else: echo ($set["explain"]); endif; ?></textarea>
        </td> 
       </tr>
       
      <tr> 
        <th><span class="red">*</span>答题数量：</th>
        <td><input type="text" id="question_num" name="question_num" value="<?php if($set["question_num"] == ''): ?>5<?php else: echo ($set["question_num"]); endif; ?>" class="px" style="width:100px;" /> (题) 请设置每天的答题数</td> 
      </tr>

      <tr> 
        <th><span class="red">*</span>答题奖励：</th>
        <td><input type="text" id="score" name="score" value="<?php if($set["score"] == ''): ?>10<?php else: echo ($set["score"]); endif; ?>" class="px" style="width:100px;" /> (分) 请设置正确答题的奖励积分</td> 
      </tr>

      <tr> 
        <th><span class="red">*</span>答题时间：</th>
        <td><input type="text" id="answer_time" name="answer_time" value="<?php if($set["answer_time"] == ''): ?>20<?php else: echo ($set["answer_time"]); endif; ?>" class="px" style="width:100px;" /> (秒) 请设置每题的答题时间，超过时限答题错误</td> 
      </tr>
      <!--
      <tr> 
        <th><span class="red">*</span>参与限制：</th>
        <td><input type="text" id="sub_limit" name="sub_limit" value="<?php if($set["sub_limit"] == ''): ?>24<?php else: echo ($set["sub_limit"]); endif; ?>" class="px" style="width:100px;" /> (次/天) 设置每天参与活动的次数限制</td> 
      </tr>
      -->
      
      <tr> 
        <th><span class="red">*</span>持续时间：</th>
        <td><input type="text" id="end_day" name="end_day" value="<?php if($set["end_day"] == ''): ?>7<?php else: echo ($set["end_day"]); endif; ?>" class="px" style="width:100px;" /> (天) 设置活动持续时间</td> 
      </tr>
      
       <tr> 
        <th><span class="red"></span>结束提示：</th> 
        <td>
          <textarea name="over_hint" class="px" id="over_hint" style="width:400px;height:50px;"><?php if($set["over_hint"] == ''): ?>今日问题已结束,明天继续来征战<?php else: echo ($set["over_hint"]); endif; ?></textarea>如需更改，请尽量不要超过15个字符
        </td> 
       </tr>
        <TR>
          <TH valign="top"><label for="info">活动规则：</label></TH>
          <TD>
            <textarea name="rule" id="rule"  rows="5" style="width:400px;height:180px"><?php echo ($set["rule"]); ?></textarea>
            <p>规则示例：</p>
            <p>1、每天只能答5题，答题正确获得10积分，错误无积分</p>
            <p>2、每题答题时间不能超过20秒，超过时间则答题错误自动进入下一题</p>
            <p>3、活动持续7天，结束后积分最高者获得胜利，同积分会自动按照答题时间进行排名</p>
          </TD>
        </TR>  
      <tr>
        <th>是否开启：</th>
        <td>
          <input type="radio" name="is_open" value="0" <?php if($set["is_open"] == '0' or ($set["is_open"] == '')): ?>checked<?php endif; ?>>关闭
          &nbsp;&nbsp;&nbsp;
          <input type="radio" name="is_open" value="1" <?php if($set["is_open"] == '1'): ?>checked<?php endif; ?>>开启   （注意：开启游戏后开始计算进行时间）
          <input type="checkbox" name="reset"   value="1">重置游戏 （重置游戏后游戏进行时间变为当前时间并清空用户积分和答题记录，请谨慎选择）
        </td>
      </tr>  
      <tr>         
       <th>&nbsp;</th>
