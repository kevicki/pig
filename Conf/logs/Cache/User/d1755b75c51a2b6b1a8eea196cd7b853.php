<?php if (!defined('THINK_PATH')) exit(); if($ischild !==1){ ?>
...
<link rel="stylesheet" type="text/css" href="<?php echo ($staticPath); ?>/tpl/static/unitary/css/cymain.css" />
<div class="content">
	<div class="cLineB">
	...
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('Unitary/add',array('token'=>$token));?>" title="新增活动商品" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 新增活动商品</a>
		</div>
		<!-- <div class="pageNavigator right">
			<form class="form" method="post"  action="">
				<input type="text" id="" class="px" placeholder="输入名称/关键词搜索"  name="nameorkeyword" value="<?php echo ($_POST['nameorkeyword']); ?>" style="margin-top:10px">
		...
		<script>
			$(function(){
				$(".radio").click(function(){
					...
					$.ajax({
						type:"POST",
						url:"<?php echo U('Unitary/indexajax',array('token'=>token));?>",
						dataType:"json",
						data:{
	...
	<div class="alert alert-success alert-dismissable">
		<?php if($_SESSION['is_syn']!= 2): if($_SESSION['is_syn'] != 1): ?>温馨提示：本功能使用了模板消息中的"订单发货提醒"模板消息和"中奖结果通知"模板消息,模板消息编号为OPENTM200565259和TM00695。<br/>
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		开通微信支付功能的公众号在使用此功能的时候可以在"基本设置——微信模板消息"中配置对应的模板消息。<br/><br/><?php endif; endif; ?>
	...
	<div class="msgWrap">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			<thead>
				...
				<?php if($_SESSION['is_syn'] == 0): ?><th width="90px">关键字</th><?php endif; ?>
				<th width="120px">活动状态</th>
				<th width="150px">创建时间</th>
				<th class="norightborder">操作</th>
			</thead>
			<tbody>
				<?php if(is_array($unitary_list)): $i = 0; $__LIST__ = $unitary_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					<td class="select"><input type="checkbox" name="" value="" class="checkitem"></td>
					<td><?php echo ($vo["name"]); ?></td>
					<td>&#165;<?php echo ($vo["price"]); ?>.00</td>
					<?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($vo["keyword"]); ?></td><?php endif; ?>
					<?php if($vo["state"] == 1): ?><td style="color:green">开始-已有<?php echo ($vo["pay_count"]); ?>人</td>
					<?php elseif($vo["state"] == 2): ?>
					<td style="color:blue">结束-已有<?php echo ($vo["pay_count"]); ?>人</td>
					<?php else: ?>
					<td style="color:red">关闭-已有<?php echo ($vo["pay_count"]); ?>人</td><?php endif; ?>
					<td><?php echo date("Y-m-d H:i:s",$vo['addtime']);?></td>
					<td class="norightborder">
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
   $(document).ready(function(){
      var bheight=document.body.clientHeight;
      $("#lay<?php echo ($vo["id"]); ?>").click(function(){
   $("#brg<?php echo ($vo["id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($vo["id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($vo["id"]); ?>").click(function(){
      $("#brg<?php echo ($vo["id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($vo["id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($vo["id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($vo["id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute;top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($vo["id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($vo["id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($vo["id"]); ?>"></div>
 <div id="showdiv<?php echo ($vo["id"]); ?>">
   <div id="close<?php echo ($vo["id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($vo["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["id"]); ?>&modle=Unitary" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($vo["id"]); ?>" style="width:70px;">查看二维码</a><?php endif; endif; ?>
						<a href="<?php echo U('Unitary/update',array('token'=>$token,'unitaryid'=>$vo['id']));?>">修改</a>
						<?php if($vo["state"] == 0): ?><a href="javascript:drop_confirm('开始后将不能修改价格，确定开始吗?','<?php echo U('Unitary/operate',array('token'=>$token,'unitaryid'=>$vo['id'],'type'=>'start','p'=>$_GET['p']));?>')">开始</a>
						<?php elseif($vo["state"] == 1): ?>
						<a href="javascript:drop_confirm('关闭后将不再手机上显示，确定关闭吗?','<?php echo U('Unitary/operate',array('token'=>$token,'unitaryid'=>$vo['id'],'type'=>'stop','p'=>$_GET['p']));?>')">关闭</a><?php endif; ?>
						<a href="<?php echo U('Unitary/data',array('token'=>$token,'unitaryid'=>$vo['id']));?>">数据</a>
						<?php if(($vo["state"] == 0) and ($vo["pay_count"] == 0)): ?><a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Unitary/operate',array('token'=>$token,'unitaryid'=>$vo['id'],'type'=>'del'));?>')">删除</a><?php endif; ?>
					</td>
