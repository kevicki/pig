...
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('Red_packet/set',array('token'=>$this->token));?>" title="添加活动" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 添加活动</a> </div>
...
<tr>                                 
	...
	<?php if($_SESSION['is_syn'] == 0): ?><th width="70">关键词</th><?php endif; ?>
	<th width="50">红包类型</th>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($list["keyword"]); ?></td><?php endif; ?>
	<td>
	...
	<td><?php echo (date("Y-m-d H:i",$list["start_time"])); ?><br/><?php echo (date("Y-m-d H:i",$list["end_time"])); ?></td>
	<td>
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
 ...
 <div id="showdiv<?php echo ($list["id"]); ?>">
   ...
   <div id="testdiv<?php echo ($list["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($list["id"]); ?>&modle=Red_packet" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($list["id"]); ?>"  style="width:151px;">查看二维码</a><br><?php endif; endif; ?>
		...
		|
		-->
		<a href="<?php echo U('Red_packet/set',array('token'=>$token,'id'=>$list['id']));?>">修改</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Red_packet/del',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
