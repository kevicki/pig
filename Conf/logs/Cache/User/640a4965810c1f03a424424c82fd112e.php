 ...
 <div class="cLine">
      <div style="width:330px;height:40px;float:left;">
        <ul id="tags" style="margin:0;width:auto;">
          <li <?php if($type == 1): ?>class="selectTag"<?php endif; ?>><a href="<?php echo U('Member_card/member',array('token'=>$token,'cardid'=>$cardid,'number'=>$number,'type'=>1));?>">积分、线下交易</a> </li>
          <li <?php if($type == 2): ?>class="selectTag"<?php endif; ?>><a href="<?php echo U('Member_card/member',array('token'=>$token,'cardid'=>$cardid,'number'=>$number,'type'=>2));?>">会员卡消费记录</a></li>
          <div class="clr"></div>
        </ul>　
      </div>
      <div style="height:30px;line-height:30px;float:right">
        <div class="right">
<?php if($type == 1): ?><a href="<?php echo ($staticPath); ?>/tpl/static/card/cardUseRecord.xls" class="btnGrayS vm">下载导入示例Excel</a>
          <a href="###" onclick="$('#import').css('display','')" class="btnGrayS vm">导入线下记录</a>
          <a href="<?php echo U('Member_card/exportCardUseRecord',array('token'=>$token,'company_id'=>$company_id,'cardid'=>$cardid,'number'=>$number));?>" class="btnGrayS vm">导出线下记录</a>
<?php else: ?>
          <a href="<?php echo ($staticPath); ?>/tpl/static/card/CardPayRecord.xls" class="btnGrayS vm">下载导入示例Excel</a>
          <a href="###" onclick="$('#import').css('display','')" class="btnGrayS vm">导入消费记录</a>
          <a href="<?php echo U('Member_card/exportrmb',array('token'=>$token,'company_id'=>$company_id,'cardid'=>$cardid,'number'=>$number));?>" class="btnGrayS vm">导出消费记录</a><?php endif; ?>
        </div>
      </div>
      <div class="clr"></div>
  </div>
...
<!-- 导入 -->	
        ...
        <form  action="" method="POST">
          <select name="company_id">
            ...
            <?php if(is_array($company)): $i = 0; $__LIST__ = $company;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$company): $mod = ($i % 2 );++$i;?><option value="<?php echo ($company["id"]); ?>" <?php if($company['id'] == $company_id): ?>selected<?php endif; ?>>ID<?php echo ($company["id"]); ?>-<?php echo ($company["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
          </select>
          -
          <select  name="cardid">
          ...
          会员卡号
          <input type="text" name="number" value="<?php echo ($number); ?>" placeholder="输入卡号,多个“,”链接" class="px">
          <input type="submit" value="查找" class="btnGreen vm">
        </form>
      </div>
      <div class="right" id="import" style="display:none;">

<?php if($type == 1): ?><form enctype="multipart/form-data" action="?g=User&amp;m=Upyun&amp;a=localUploadUsecordExcel" id="thumbForm" method="POST">
...
<?php if($type == 1): ?><TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
      <THEAD>
        <TR>
  				<TH>日期</TH>
  				<TH>金额(元)</TH>
...
<?php else: ?>
      <TABLE class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
        ...
        <TBODY>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rmb): $mod = ($i % 2 );++$i;?><TR>
            ...
            <TD><a href="javascript:drop_confirm('您确定要删除吗?', '/index.php?g=User&m=Member_card&a=payRecord_del&pid=<?php echo ($rmb["id"]); ?>');"><strong>删除</strong></a></TD> 
          </TR><?php endforeach; endif; else: echo "" ;endif; ?>
        </TBODY>
      </TABLE><?php endif; ?>
      <div class="cLine">
