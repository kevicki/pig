...
<div class="tab">
<ul>
<?php if($isgroup != 1 OR $ischild != 1): ?><li class="current tabli" id="tab2"><a href="<?php echo U('Store/index',array('token'=>$token));?>">商品分类管理</a></li>
...
<a href="<?php echo U('Store/catAdd', array('token' => $token, 'cid' => $cid, 'parentid' => $parentid));?>" title="新增分类" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" class="vm">新增分类</a>
</div>
...
<td class="norightborder">
	<a href="<?php echo U('Store/catSet',array('token' => $token, 'id' => $hostlist['id'], 'cid' => $cid));?>">修改</a>
	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Store/catDel',array('id'=>$hostlist['id'],'token'=>$token, 'cid' => $cid));?>');">删除</a>
	...
	<?php if($hostlist['isfinal'] != 1): ?><span>&nbsp;|&nbsp;<a href="<?php echo U('Store/index',array('parentid' => $hostlist['id'],'level' => $hostlist['level'],'token'=>$token));?>" style="color:#f00">子分类列表</a></span><?php endif; ?>
	<?php if($hostlist['isfinal'] != 2): ?><span>&nbsp;|&nbsp;<a href="<?php echo U('Store/product',array('catid' => $hostlist['id'],'token'=>$token));?>" style="color:#f00">商品管理</a></span><?php endif; ?>
</td>
