...
<link rel="stylesheet" type="text/css" href="<?php echo ($staticPath); ?>/tpl/static/bargain/css/cymain.css" />
<div class="content">
	<div class="cLineB">
		<h4 class="left">微砍价</h4>
		<div class="clr"></div>
	</div>
<?php if($_SESSION['is_syn']!= 2): ?><div class="alert alert-success alert-dismissable">
	...
	<div class="tab">
		<ul>
			<li class="current tabli" id="tab1"><a href="<?php echo U('Bargain/index',array('token'=>$token));?>">砍价商品</a></li>
			<li class="tabli" id="tab2"><a href="<?php echo U('Bargain/order',array('token'=>$token));?>">订单管理</a></li>
		</ul>
	</div>
	...
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('Bargain/add',array('token'=>$token));?>" title="添加砍价" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加砍价</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入砍价名称"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
		...
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			<thead>
				<tr>
					...
					<?php if($_SESSION['is_syn'] == 0): ?><th width="80px">关键词</th><?php endif; ?>
					<th width="60px">原价</th>
					<th width="60px">底价</th>
					<th width="60px">库存</th>
					...
					<th <?php if($usertplid == 2): ?>width="200px"<?php endif; ?>>操作</th>
					<th width="120px" class="norightborder">添加时间</th>
				</tr>
			</thead>
			<tbody>
				<?php if(is_array($bargain_list)): $i = 0; $__LIST__ = $bargain_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($vo["name"]); ?></td>
						<?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($vo["keyword"]); ?></td><?php endif; ?>
						<td><?php echo ($vo["original"]); ?>元</td>
						<td><?php echo ($vo["minimum"]); ?>元</td>
						<td><?php echo ($vo["inventory"]); ?></td>
						<td><?php echo ($vo["count_canyu"]); ?></td>
						<td><?php echo ($vo["count_pay"]); ?></td>
						<?php if($vo["state"] == 1): ?><td><a href="javascript:state(<?php echo ($vo["pigcms_id"]); ?>)"><img id="state<?php echo ($vo["pigcms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/bargain/images/start.png" width="100%"/></a></td>
						<?php else: ?>
						<td><a href="javascript:state(<?php echo ($vo["pigcms_id"]); ?>)"><img id="state<?php echo ($vo["pigcms_id"]); ?>" src="<?php echo ($staticPath); ?>/tpl/static/bargain/images/stop.png" width="100%"/></a></td><?php endif; ?>
						<td class="norightborder" style="border-right:1px solid #eee">
							<script type="text/javascript">
   $(document).ready(function(){
      var bheight=document.body.clientHeight;
      $("#lay<?php echo ($vo["pigcms_id"]); ?>").click(function(){
   $("#brg<?php echo ($vo["pigcms_id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($vo["pigcms_id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($vo["pigcms_id"]); ?>").click(function(){
      $("#brg<?php echo ($vo["pigcms_id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($vo["pigcms_id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($vo["pigcms_id"]); ?>{ width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($vo["pigcms_id"]); ?>{z-index:20; display:none;position:fixed; *position:absolute;top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($vo["pigcms_id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($vo["pigcms_id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($vo["pigcms_id"]); ?>"></div>
 <div id="showdiv<?php echo ($vo["pigcms_id"]); ?>">
   <div id="close<?php echo ($vo["pigcms_id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($vo["pigcms_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["pigcms_id"]); ?>&modle=Bargain" width="300" height="300" class="cateimg_small"></div>
 </div>
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><a id="lay<?php echo ($vo["pigcms_id"]); ?>" style="width:150px;">查看二维码</a><br><?php endif; endif; ?>
							<a href="<?php echo U('Bargain/order',array('token'=>$token,'id'=>$vo['pigcms_id']));?>">订单</a>
							<a href="<?php echo U('Bargain/update',array('token'=>$token,'id'=>$vo['pigcms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Bargain/operate',array('token'=>$token,'id'=>$vo['pigcms_id'],'type'=>'del'));?>')">删除</a>
						</td>
...
<script>
	...
	function state(id){
		$.ajax({
			type:"POST",
			url:"<?php echo U('Bargain/ajax',array('token'=>$token));?>",
			dataType:"json",
			data:{
				type:"state",
				id:id,
			...
			success:function(data){
				if(data.error == 0){
					$("#state"+id).attr("src","<?php echo ($staticPath); ?>/tpl/static/bargain/images/stop.png");
				}else if(data.error == 1){
					$("#state"+id).attr("src","<?php echo ($staticPath); ?>/tpl/static/bargain/images/start.png");
				}
			}
		});
	}
	function seach(){
		var name = $("#name").val();
		if(name != ""){
			window.location.href="<?php echo U('Bargain/index',array('token'=>$token));?>&name="+name;
		}else{
			window.location.href="<?php echo U('Bargain/index',array('token'=>$token));?>";
		}
	}
</script>
</div>
