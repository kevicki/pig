...
$(document).ready(function(){
	...
	$('#company').change(function(){
		$.post("<?php echo U('Hardware/getkitchen', array('token' => $token));?>", {'companyid':$(this).val()}, function(data){
			if (data.error) {
...
<div class="ftip" style="margin:25px auto 5px auto;">无线订单打印机（小票打印机）是指无需人工处理，有微信订单的时候会自动打印订单信息的小型打印机，<span style="color:#f00">如果背部没有终端号，我们称之为A打印机</span></div>
<style>
.dtas{padding:20px}
...
<td><input type="text" class="px" id="mp"  name="mp" style="width:200px"  value="<?php echo ($info["mp"]); ?>" />    A打印机无需填写</td>
<td>&nbsp;</td>
</tr>
<tr>
<th valign="top"><label for="acttitle">绑定账号：</label></th>
<td><input type="text" class="px" id="name"  name="name" style="width:200px"  value="<?php echo ($info["name"]); ?>" />    A打印机填写</td>
<td>&nbsp;</td>
</tr>
<tr>
<th valign="top"><label for="acttitle">终端号：</label></th>
<td><input type="text" class="px" id="mcode"  name="mcode" style="width:500px"  value="<?php echo ($info["mcode"]); ?>" />  非A打印机底部查看 / A打印机点击打印机后面黑色小按钮查看 </td>
<td>&nbsp;</td>
</tr>
<tr>
<th valign="top"><label for="acttitle">密钥：</label></th>
<td><input type="text" class="px" id="mkey"  name="mkey" style="width:500px"  value="<?php echo ($info["mkey"]); ?>" />  非A打印机底部查看 / A打印机在注册页面查看 </td>
<td>&nbsp;</td>
</tr>
<tr>
<th valign="top"><label for="acttitle">打印份数：</label></th>
<td><input type="text" class="px" id="count"  name="count" style="width:100px"  value="<?php echo ($info["count"]); ?>" />  每个订单打印几份 </td>
<td>&nbsp;</td>
</tr>
<tr style="display:none">
<th valign="top"><label for="acttitle">二维码链接：</label></th>
<td>
<input type="text" class="px" id="qr"  name="qr" style="width:250px"  value="<?php echo ($info["qr"]); ?>" />
...
<option value="0" <?php if($info['companyid'] == 0): ?>selected<?php endif; ?>>请选择</option>
<?php if(is_array($companys)): $i = 0; $__LIST__ = $companys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$company): $mod = ($i % 2 );++$i;?><option value="<?php echo ($company["id"]); ?>" <?php if($info['companyid'] == $company['id']): ?>selected<?php endif; ?>><?php echo ($company["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                            
</select> 用于哪个店面&nbsp;&nbsp;<a href="/index.php?g=User&m=Company&a=index&token=<?php echo ($token); ?>">点击这里添加店面</a>   
</td>
<td>&nbsp;</td>
</tr>
...
<option value="0" <?php if($info['number'] == 0): ?>selected<?php endif; ?>>请选择</option>
<?php if(is_array($kitchens)): $i = 0; $__LIST__ = $kitchens;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$kitchen): $mod = ($i % 2 );++$i;?><option value="<?php echo ($kitchen["id"]); ?>" <?php if($info['number'] == $kitchen['id']): ?>selected<?php endif; ?>><?php echo ($kitchen["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>                            
...
<td><?php echo ($moduleStr); ?>   选择打印哪些类型的订单</td>
<td>&nbsp;</td>
</tr>

<tr>
<th valign="top">打印类型：</th>
<td><label for="radio3"><input id="radio3" class="radio" type="radio" name="paid" value="1"  <?php if(($info["paid"]) == "1"): ?>checked="checked"<?php endif; ?> /> 只打印付过款的</label>&nbsp;&nbsp;<label for="radio4"><input class="radio" id="radio4" type="radio" name="paid" value="0" <?php if(($info["paid"]) == "0"): ?>checked="checked"<?php endif; ?> /> 无论是否付款都打印</label></td>
</tr>
<tr>
<th></th>
