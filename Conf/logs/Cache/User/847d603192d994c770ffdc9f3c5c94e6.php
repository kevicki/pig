...
<script src="/tpl/static/artDialog/plugins/iframeTools.js"></script>
 <div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          <div class="cLineB"><h4><?php if($tip == 1): ?>编辑幻灯片<?php else: ?>编辑背景图<?php endif; ?></h4><a href="javascript:history.go(-1);"  class="right btnGrayS vm" style="margin-top:-27px" >返回</a></div> 
<div class="msgWrap">
...
<TR>
  ...
  <TD><input type="text" class="px" id="img" value="<?php echo ($info["img"]); ?>" name="img" style="width:500px" >  <script src="/tpl/static/upyun.js?2013"></script><a href="###" onclick="chooseFile('img','<?php if($tip == 1): ?>focus<?php else: ?>background<?php endif; ?>')" class="a_upload">选择</a> <a href="###" onclick="upyunPicUpload('img',<?php if($tip == 1): ?>700,420<?php else: ?>640,1136<?php endif; ?>,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('img')">预览</a> 建议图片(<?php if($tip == 1): ?>宽640高320<?php else: ?>宽640高1136<?php endif; ?>)  </TD>
  <TD>&nbsp;</TD>
</TR>
<?php if($tip == 1): ?><TR>
  <TH valign="top"><label for="keyword">幻灯片链接地址 ：</label></TH>
  <TD><input type="text" class="px" id="url" value="<?php echo (htmlspecialchars_decode($info["url"])); ?>" name="url" style="width:300px" >  <?php if(session('isQcloud') != true): ?><a href="###" onclick="addLink('url',0)" class="a_choose">从功能库添加</a><?php endif; ?><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<?php else: ?>
