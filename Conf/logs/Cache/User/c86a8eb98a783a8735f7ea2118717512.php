...
<div class="msgWrap">
  <form class="form" method="post"   action="<?php echo U('Classify/upsave');?>" enctype="multipart/form-data" >
...
<TR>
  <TH valign="top"><label for="url">外链网站或活动 ：</label></TH>
  <TD><input type="text" class="px" id="url" value="<?php echo ($info["url"]); ?>" name="url" style="width:300px" > <?php if(session('isQcloud') != true): ?><a href="###" onclick="addLink('url',0)" class="a_choose">从功能库添加</a><?php endif; ?><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
  <TH valign="top"><label for="keyword">排序：</label></TH>
  <TD><input type="text" class="px" id="keyword" value="<?php echo ($info["sorts"]); ?>" name="sorts" style="width:500px" ><br />
                   </TD>
  <TD>&nbsp;</TD>
</TR>
<TR>
...
<TR>
  ...
  <TD>
	<input type="text" id="selecttpl" class="px" value="已选择模板<?php echo ($info["tpid"]); ?>" disabled size="40"  />
...
<tr>
  ...
  <TD>
	...
	<input type="text" id="selectconttpl" class="px" value="已选择模板<?php echo ($info["conttpid"]); ?>" disabled size="40"  />
	<a class="a_upload" onclick="chooseTpl('conttpid','selectconttpl',3)">选择模板</a>
	<a class="a_upload" onclick="viewTpl2()">预览</a>
	<input type="hidden" class="px" value="<?php echo ($info["conttpid"]); ?>" id="conttpid" name="conttpid" style="width:250px">
	<script>
		function viewTpl2(){
...
<?php if($info['pc_cat_id']): ?><TR>
	<TH valign="top">是否解绑电脑<br/>网站文章分类：</TH>
	<TD>
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;如果选择是，此分类将不再与电脑网站文章分类挂钩，以后添加文章时不会出现“是否添加到电脑网站分类”的提示。改变后不可改回。
    </TD>
	<TD>&nbsp;</TD>
</TR><?php endif; ?>
<tr>
