...
<div class="content">
	...
	<div class="tab">
		<ul>
			<li class="tabli" id="tab1"><a href="<?php echo U('Cutprice/index',array('token'=>$token));?>">商品</a></li>
			<li class="current tabli" id="tab2"><a href="<?php echo U('Cutprice/order',array('token'=>$token));?>">订单</a></li>
		</ul>
	</div>
	...
	<div class="cLine">
		<div class="pageNavigator left">
			&nbsp;&nbsp;&nbsp;&nbsp;订单号：<input type="text" id="orderid" class="px name" placeholder="输入订单号"  name="orderid" value="<?php echo ($_GET['orderid']); ?>" style="margin-top:10px">
			...
			<tbody>
				<?php foreach($order_list as $vo){?>
					<tr>
						...
						<?php if($vo['paid'] == 1){?>
							<?php if($vo['fahuo'] == 1){?>
							<a href="<?php echo U('Cutprice/operate',array('token'=>$token,'type'=>'qxfahuo','orderid'=>$vo['pigcms_id']));?>">取消发货</a><br/>
							<?php }elseif($vo['fahuo'] == 0){?>
							<a href="javascript:fahuo(<?php echo ($vo["pigcms_id"]); ?>)">发货</a><br/>
							<?php }?>
						<?php }?>
							<a href="javascript:drop_confirm('您确定要删除订单【<?php echo ($vo["orderid"]); ?>】吗?\n警告！如果删除此订单，网友手机端将看不到此订单！', '<?php echo U('Cutprice/operate',array('token'=>$token,'orderid'=>$vo['pigcms_id'],'type'=>'delorder','cid'=>$_GET['id']));?>')">删除</a><br/>
							<a href="<?php echo U("Cutprice/updateorder",array('token'=>$token,'orderid'=>$vo['pigcms_id'],'cid'=>$_GET['id']));?>">修改</a>
						</td>
...
<script>
		...
		var dialog = art.dialog({
			...
			ok: function () {
				...
				if(fahuoname == ''){
					...
					$.ajax({
						type:"POST",
						url:"<?php echo U('Cutprice/ajax',array('token'=>$token));?>",
						dataType:"json",
						data:{
							type:"fahuo",
	...
	function seach(){
		...
		if(orderid != ""){
			window.location.href="<?php echo U('Cutprice/order',array('token'=>$token));?>&orderid="+orderid;
		}else{
			window.location.href="<?php echo U('Cutprice/order',array('token'=>$token));?>";
		}
	}
</script>
</div>
