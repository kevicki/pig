...
<TR>
  ...
  <TD>
	<input type="input" class="px" id="keyword" value="<?php if($set["keyword"] == ''): ?>微召唤<?php else: echo ($set["keyword"]); endif; ?>" name="keyword" style="width:400px" ><br />
  	<span class="red">只能写一个关键词</span>，用户输入此关键词将会触发此活动。
  </TD>
  <TD rowspan="5" valign="top">
	  <div style="margin-left:20px">
	  <span class="red"><b>回复缩略图：<b></span>
		<img id="pic1_src" src="<?php if($set['pic'] == ''): echo ($f_siteUrl); ?>/tpl/Wap/default/common/weizhaohuan/images/hd.jpg<?php else: echo ($set["pic"]); endif; ?>" width="373px" >
		<br />
		<input class="px"  name="pic" value="<?php if($set['pic'] == ''): echo ($f_siteUrl); ?>/tpl/Wap/default/common/weizhaohuan/images/hd.jpg<?php else: echo ($set["pic"]); endif; ?>" id="pic1" style="width:363px;"  />
		<br />
		  <script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('pic1',600,267,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic1')">预览</a>&nbsp;&nbsp;消息回复图片
	  </div>
  </TD>
</TR>
<TR>
  <th valign="top"><span class="red">*</span>回复活动名称：</th>
  <TD>
	<input type="input" class="px" id="title" value="<?php if($set["title"] == ''): ?>微召唤人气榜<?php else: echo ($set["title"]); endif; ?>" name="title" style="width:400px" />
  	<br />
  	请不要多于50字!
  </TD>
</TR>
<TR>
<th valign="top">回复活动摘要：</th>
<td colspan="2">
	<textarea  class="px" id="intro"  name="intro"  style="width:300px; height:125px" ><?php echo ($set["intro"]); ?></textarea>
</td>
</TR>
<TR>
  <th valign="top"><span class="red">*</span>分享转发缩略图：</th>
  <TD>
		<input class="px"  name="top_pic" value="<?php if($set['top_pic'] == ''): echo ($f_siteUrl); ?>/tpl/Wap/default/common/weizhaohuan/images/banner.jpg<?php else: echo ($set["top_pic"]); endif; ?>" id="pic0" style="width:310px;"  />
...
<TR>
	...
	<td><input type="input" class="px" id="statdate" value="<?php if($set['start'] != ''): echo (date("Y-m-d H:i:s",$set["start"])); else: echo (date("Y-m-d H:i:s",$start_date)); endif; ?>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd  HH:mm:ss'})" name="start" />
		到
		<input type="input" class="px" id="enddate" value="<?php if($set['end'] != ''): echo (date("Y-m-d H:i:s",$set["end"])); else: echo (date("Y-m-d H:i:s",$end_date)); endif; ?>" name="end"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd  HH:mm:ss'})"  />
	</td>
</TR>
<TR>
<th valign="top">活动规则：</th>
<td colspan="2">
	<textarea  class="px" id="info"  name="info"  style="width:400px; height:175px" ><?php echo ($set["info"]); ?></textarea>
</td>
</TR>
<TR>
...
<TR>
 	<th valign="top"><span class="red"></span>非关注用户：</th>
 	<td>
		<input type="radio" name="is_attention" value="1" <?php if($set['is_attention'] == '' or $set['is_attention'] == 1): ?>checked<?php endif; ?>>关注&nbsp;&nbsp;&nbsp;
		<input type="radio" name="is_attention" value="2" <?php if($set['is_attention'] == 2): ?>checked<?php endif; ?>>注册
		  &nbsp;&nbsp;&nbsp;
  		  &nbsp;&nbsp;
  		 （非关注用户参加活动的处理方式）
	</td>
...
<TR>
 	<th valign="top"><span class="red"></span>无粉丝信息：</th>
 	<td>
		<input type="radio" name="is_reg" value="1" <?php if($set['is_reg'] == '' or $set['is_reg'] == 1): ?>checked<?php endif; ?>>注册&nbsp;&nbsp;&nbsp;
		<input type="radio" name="is_reg" value="2" <?php if($set['is_reg'] == 2): ?>checked<?php endif; ?>>不注册
		  &nbsp;&nbsp;&nbsp;
  		（没有粉丝信息参加活动的处理方式）
	</td>
</TR>
...
<h4>详细设置</h4></div>
<div class="msgWrap bgfc">
<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;font-size:12px;">温馨提示：推荐增加会员积分随机值。单个奖项参数请填写完整，否则前台将不予显示。奖品图片宽高比例请尽量一致
</div>
...
<tr class="alis">
	<th width="120">随机获得会员积分范围：</th>
	<td><input style="width:55px;" type="text" name="min" value="<?php echo ($set["min"]); ?>" class="px" >~~~~~~<input style="width:55px;" type="text" name="max" value="<?php echo ($set["max"]); ?>" class="px" ></td>
</tr>
<tr>
<th width="120">活动背景：</th>
	<td>  <img id="pic3_src" src="<?php if($set['bgpic'] == ''): echo ($f_siteUrl); ?>/tpl/Wap/default/common/weizhaohuan/images/bg.jpg<?php else: echo ($set["bgpic"]); endif; ?>" style="max-width:300px;"></br></br>
	<input class="px" name="bgpic" value="<?php if($set['bgpic'] == ''): echo ($f_siteUrl); ?>/tpl/Wap/default/common/weizhaohuan/images/bg.jpg<?php else: echo ($set["bgpic"]); endif; ?>" onclick="document.getElementById('pic3_src').src=this.value;" style="width:363px;" id="pic3">
	
	 <a href="###" onclick="upyunPicUpload('pic3',1000,5000,'<?php echo ($token); ?>')" class="a_upload">上传</a> <a href="###" onclick="viewImg('pic3')">预览</a>&nbsp;活动背景图片</td>
</tr>
<tr>
	<th width="120">背景颜色：</th>
	<td><input type="text" name="bgcolor" value="<?php if($set['bgcolor'] == ''): ?>#ca1a48<?php else: echo ($set["bgcolor"]); endif; ?>" id="bgcolor" class="px color" style="width: 55px; background:<?php if($set['bgcolor'] == ''): ?>#ca1a48<?php else: echo ($set["bgcolor"]); endif; ?>; color: rgb(255, 255, 255);" /></td>
</tr>
