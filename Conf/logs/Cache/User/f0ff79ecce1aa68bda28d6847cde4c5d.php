...
<a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 ...
 <input type="hidden" name="mbid" value="<?php echo ($mbid); ?>" id="mb_id">
...
<td>
  <?php if($nomodify): ?><span class="px" style="display: inline-block;width:350px;height:24px;color:red;"><?php echo ($autodomain); ?></span>
  <?php else: ?>
  
  <input type="text" name="sitename" id="sitename" class="px" style="width:350px;height:24px;" <?php if(is_array($infos) && !empty($infos['owndomain'])): ?>value="<?php echo ($infos['owndomain']); ?>"<?php endif; ?>><?php endif; ?>
<?php if($nomodify): ?><br/>
<span style="color:red;">系统自动为您分配生成域名，不可更改</span><?php endif; ?>
</td>
</tr>
<tr>
<th width="120">第三方js统计代码：</th>
 <td><textarea class="px" id="str_script" name="str_script" style="width:350px;height:150px;"><?php if(is_array($infos) && !empty($infos['tjscript'])): echo ($infos['tjscript']); endif; ?></textarea>
  <br/>
  <br/>
 <span>
...
<script type="text/javascript">
   var lock=false;
   var nomodify=false;
   <?php if($nomodify): ?>nomodify=true;<?php endif; ?>
   ...
   var scriptREG=/<script\s*(.*)>(.*)<\/script>/;
	$("#toSaveData").click(function(){
        ...
        if(!lock){
		 lock=true;
        $.ajax({
        	    url : "<?php echo U('Phone/saveData',array('token'=>$token));?>",
                dataType : 'JSON',
                type : 'POST',
                data : datas,
 ...
 function toDownload(){
		...
		if((strscript!='') && !scriptREG.test(strscript)){
		  ...
		  window.location.href="<?php echo U('Phone/downloadFile',array('token'=>$token));?>&tjscr="+encodeURIComponent(strscript);
	        /*$.ajax({
        	    url : "<?php echo U('Phone/downloadFile',array('token'=>$token));?>",
                dataType : 'JSON',
                type : 'GET',
