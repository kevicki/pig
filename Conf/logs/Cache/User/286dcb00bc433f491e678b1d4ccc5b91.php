...
<link rel="stylesheet" type="text/css" href="<?php echo ($staticPath); ?>/tpl/static/cutprice/css/cymain.css" />
<div class="content">
	<div class="cLineB">
...
<?php else: ?>
	...
	<div class="tab">
		<ul>
			<li class="current tabli" id="tab1"><a href="<?php echo U('Cutprice/index',array('token'=>$token));?>">商品</a></li>
			<li class="tabli" id="tab2"><a href="<?php echo U('Cutprice/order',array('token'=>$token));?>">订单</a></li>
		</ul>
	</div>
	...
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('Cutprice/add',array('token'=>$token));?>" title="添加拍卖商品" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加拍卖商品</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入拍卖商品名称"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
		...
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			<thead>
				<tr>
					...
					<?php if($_SESSION['is_syn'] == 0): ?><th width="100px">关键词</th><?php endif; ?>
			...
			<tbody>
				<?php if(is_array($cutprice_list)): $i = 0; $__LIST__ = $cutprice_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($vo["name"]); ?></td>
						<?php if($_SESSION['is_syn'] == 0): ?><td><?php echo ($vo["keyword"]); ?></td><?php endif; ?>
						<td>
							原价：￥<?php echo ($vo["original"]); ?><br/>
							起拍价：￥<?php echo ($vo["startprice"]); ?><br/>
							最低价：￥<?php echo ($vo["stopprice"]); ?><br/>
							每<?php echo ($vo["cuttime"]); ?>分钟下降<?php echo ($vo["cutprice"]); ?>元<br/>
						</td>
						<td><?php echo ($vo["inventory"]); ?></td>
						<td><?php echo ($vo["paycount"]); ?></td>
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
 ...
 <div id="showdiv<?php echo ($vo["pigcms_id"]); ?>">
   ...
   <div id="testdiv<?php echo ($vo["pigcms_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($vo["pigcms_id"]); ?>&modle=Cutprice" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($vo["pigcms_id"]); ?>" style="width:149px;"><center>查看二维码</center></a><br><?php endif; endif; ?>
							<a href="<?php echo U('Cutprice/order',array('token'=>$token,'id'=>$vo['pigcms_id']));?>">订单</a>
							<a href="<?php echo U('Cutprice/update',array('token'=>$token,'id'=>$vo['pigcms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Cutprice/operate',array('token'=>$token,'id'=>$vo['pigcms_id'],'type'=>'del'));?>')">删除</a>
						</td>
						<td  style="border-right:0">
							拍卖开始时间：<?php echo (date("Y-m-d H:i:s",$vo["starttime"])); ?><br/>
							预计结束时间：<?php echo (date("Y-m-d H:i:s",$vo["stoptime"])); ?><br/>
							商品添加时间：<?php echo (date("Y-m-d H:i:s",$vo["addtime"])); ?><br/>
						</td>
...
<script>
	...
	function seach(){
		var name = $("#name").val();
		if(name != ""){
			window.location.href="<?php echo U('Cutprice/index',array('token'=>$token));?>&name="+name;
		}else{
			window.location.href="<?php echo U('Cutprice/index',array('token'=>$token));?>";
		}
	}
</script>
</div>
