...
<div class="content">
          <div class="cLineB"><h4>关键词触发第三方接口</h4>
			<a href="<?php echo U('Api/index',array('token'=>$token));?>" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
		  </div> 
<div class="msgWrap">
  ...
  <thead>
	<tr>
	  ...
	  <td>
		<input type="input" class="px" id="keyword" value="<?php echo ($api["keyword"]); ?>" name="keyword" style="width:600px"><br>
		(注：关键词必须填写，用于触发接口！) 开启包含匹配触发本接口!  用户输入的只要包含本关键词自动交给本接口处理!  </td>
	</tr>
  </thead>
<thead>
	<tr>
	  ...
	  <td>  
		<input type="checkbox" id="is_colation" value="1" name="is_colation" <?php if($api['is_colation'] == 1): ?>checked<?php endif; ?>> 是否过滤触发词<br>
			比如设置的电影为触发词,用户输入“电影 美国派” 时，如果启用过滤我们只将“美国派”这个词发送到的你的接口，如果不过滤 就是整个“电影 美国派”全部发送到的接口!
	  </td>
	</tr>
</thead>
<thead>
...
<thead>
  ...
  <tbody>
<tr>
  <th valign="top"><label for="text">第三方微信api地址：</label></th>
  <td><input type="input" class="px" id="apiurl" value="<?php echo ($api["url"]); ?>" name="url" style="width:600px"><br></td>
</tr>
<tr>
  <th valign="top"><label for="text">第三方微信token：</label></th>
  <td><input type="input" class="px" id="apitoken" value="<?php echo ($api["apitoken"]); ?>" name="apitoken" style="width:200px"><br>只有是标准的微信接口时才需要填写</td>
</tr>
<tr style="display:none">
  <th valign="top"><label for="token">忽略本接口回复特征词：</label></th>
  <td><input type="input" class="px" id="token" value="<?php echo ($api["colation_keyword"]); ?>" name="colation_keyword" style="width:600px"><br>设置 包含关键词用|隔开，包含此特定词则忽略，交给我们处理!  无需忽略请留空!
...
<tr>
  ...
  <th></th>
  <td><button type="submit" name="button" class="btnGreen left">保存</button>　　<a href="<?php echo U('Api/index',array('token'=>$token));?>" class="btnGray vm">取消</a>
  <div class="clr"></div>
</td>
</tr>
  </tbody>
...
<div id="copyright" style="color:white;">
	<?php echo ($f_siteName); ?>(c)版权所有 <a href="http://www.miibeian.gov.cn" target="_blank" style="color:white"><?php echo ($f_ipc); ?></a><br/>
