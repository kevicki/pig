     ...
     <table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"> 
      <tbody> 
       ...
       <?php if($isgroup == 1): ?><tr> 
        ...
        <?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g['id']); ?>" <?php if($set['gid'] == $g['id']): ?>selected<?php endif; ?>><?php echo ($g['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
        </td> 
       </tr><?php endif; ?>
       <tr> 
       ...
       <tr> 
        <th>销量基数：</th> 
        <td><input type="text" id="fakemembercount" name="fakemembercount" value="<?php echo ($set["fakemembercount"]); ?>" class="px" style="width:100px;" />(如果您不做假数据就设置为0)</td> 
      </tr>
       <tr> 
        <th>是否下架：</th> 
        <td>
	        <input type="radio" name="status" class="status" value="0" id="status_0" <?php if($set['status'] == 0): ?>checked<?php endif; ?> /><label for="status_0">正常</label>&nbsp;&nbsp;&nbsp;&nbsp;
	        <input type="radio" name="status" class="status" value="1" id="status_1" <?php if($set['status'] == 1): ?>checked<?php endif; ?> /><label for="status_1">下架</label>
		</td> 
       </tr>
       <tr> 
        <th>邮费：</th> 
       ...
       <TR>
		...
		<?php if($productCatData['pc_cat_id'] && empty($set)): ?><TR id="pc_show">
			<TH valign="top">是否同时添加到<br/>电脑网站产品：</TH>
			<TD>
				<input type="hidden" name="pc_cat_id" id="pc_cat_id" value="<?php echo ($productCatData["pc_cat_id"]); ?>"/>
		...
		$.post('index.php?g=User&m=Store&a=productSave', data, function(response){
			if (response.error_code == false) {
				art.dialog({
				    ...
				    ok: function () {
				    	this.time(3);
				    	location.href="index.php?g=User&m=Store&a=product&token=<?php echo ($token); ?>&catid=<?php echo ($catid); ?>&p=<?php echo ($_GET['p']); ?>";
				        return false;
				    },
				    cancelVal: '关闭'
				});
