...
<div class="content">
<div class="cLineB">
  <h4 style="margin-bottom:10px;">粉丝昵称：<?php echo ($userinfo['nickname']); ?></h4><span style="color:red"> 粉丝关注时间：<?php echo (date('Y-m-d H:i:s',$userinfo['subscribe_time'])); ?> <?php if($endtime != 0): ?>粉丝最后一次访问时间：<?php echo ((date('Y-m-d H:i:s',$endtime))?(date('Y-m-d H:i:s',$endtime)):"早期粉丝，查无音讯"); endif; ?></span>
  <a href="" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
 </div>
 <div class="msgWrap form">
		<ul id="tags">
		<li <?php if($type != 'list'): ?>class="selectTag"<?php endif; ?>>
				<a href="<?php echo U('Wechat_behavior/statisticsOfSingleFans',array('openid'=>strip_tags($_GET['openid'])));?>">行为统计</a> 
			</li>
			<li <?php if($type == 'list'): ?>class="selectTag"<?php endif; ?>>
				<a href="<?php echo U('Wechat_behavior/wechatList',array('openid'=>strip_tags($_GET['openid'])));?>">行为列表</a> 
			</li>
		...
		<fieldset>
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			    ...
			    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                  ...
                  <td><?php echo ($list["behavior"]); ?></td>
                  <td><?php echo (date('Y-m-d H:i:s',$list["enddate"])); ?></td>
                  <td><?php echo (($list["num"])?($list["num"]):'1'); ?></td>
                  <td>
					<?php echo ($list["keyword"]); ?>
               
                  <td class="norightborder" style="display:none">
						<a href="<?php echo U('Wechat_behavior/wechatList',array('openid'=>$list['openid']));?>">粉丝行为管理</a>　
