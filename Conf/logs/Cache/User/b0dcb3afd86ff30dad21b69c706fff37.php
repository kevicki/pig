...
<div class="cLineC" style="line-height: 32px;">
 <a href="<?php echo U('Shakearound/add_page',array('token'=>$token));?>" class="btnGrayS vm bigbtn" >添加页面</a>　 
</div>  <div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;font-size:12px;">温馨提示：图标、主标题、副标题都会在摇出的页面中展示,跳转地址为点击摇出页面跳转的地址。
</div>
...
<thead>
<tr>
<th style=" width:6%;">页面ID</th>
<th style=" width:8%;">主标题</th>    
<th style=" width:10%;">副标题</th>    
<th style=" width:15%;">图标</th>    
<th style=" width:15%;">跳转地址</th>   
<th style=" width:15%;">页面备注</th>         
<th class="norightborder">操作</th>
</tr>
</thead>
<?php if(is_array($page_list)): $i = 0; $__LIST__ = $page_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$li): $mod = ($i % 2 );++$i;?><tr>
<td><?php echo ($li["page_id"]); ?></td>
<td><?php echo ($li["title"]); ?></td>
<td><?php echo ($li["description"]); ?></td>
<td><img src="<?php echo ($li["icon_url"]); ?>" width="50px" height="50px"/></td>
<td><?php echo ($li["page_url"]); ?></td>
<td><?php echo ($li["page_comment"]); ?></td>
<td class="norightborder">
 <a href="<?php echo U('Shakearound/edit_page',array('id'=>$li['id'],'token'=>$li['token']));?>" >编辑页面</a>  
    |
<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Shakearound/del_page',array('token'=>$token,'id'=>$li['id']));?>');">删除页面</a>
