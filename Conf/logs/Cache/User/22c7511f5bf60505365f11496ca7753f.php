...
<div class="content">
	<div class="cLineB">
		<h4 class="left">模板消息配置</h4>
		<div class="clr"></div>
	</div>
	<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;font-size:12px;margin-top:5px;">温馨提示：此功能仅限于已开通微信支付功能的公众号使用。<br /> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 必须填写【模板ID】才能【开启】模板消息推送状态。<br /> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;【模板ID】请到 <a href="http://mp.weixin.qq.com" target="_blank" style="color:#029700">微信公众平台</a> >> 模板消息 >> 模板库 选择对应【模板编号】的模板，然后获取【模板ID】填写到此处。 <br /> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 以下所有模板均属于“IT科技/互联网|电子商务。</div>
	<div class="cLine">
		<div class="pageNavigator left"> <a href="<?php echo U('TemplateMsg/add',array('token' => $token));?>" title="添加" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?>添加新模板</a></div>
		<div class="clr"></div>
	</div>            
	<table class="ListProduct" border="0" cellSpacing="0" cellPadding="0" width="100%">
		...
		<tbody>
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><tr>
					<td><?php echo ($t["tempkey"]); ?></td>
					<td><?php echo ($t["name"]); ?></td>
					<td><pre><?php echo ($t["content"]); ?></pre></td>
					<td><?php echo ($t["industry"]); ?></td>
					<td><span style="width: 55px; background:<?php echo ($t["topcolor"]); ?>; color: rgb(255, 255, 255);"><?php echo ($t["topcolor"]); ?></span></td>
					<td><span style="width: 55px; background:<?php echo ($t["textcolor"]); ?>; color: rgb(255, 255, 255);"><?php echo ($t["textcolor"]); ?></span></td>
					<td><?php if($t['status'] == 0): ?>关闭<?php else: ?>开启<?php endif; ?></td>
					<td><?php echo ($t["tempid"]); ?></td>
					<td class="norightborder">
						<a href="<?php echo U('TemplateMsg/add', array('token' => $token, 'id' => $t['id']));?>">编辑</a>
						<?php if($t['type']): ?><a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('TemplateMsg/del', array('token' => $token, 'id' => $t['id']));?>');">删除</a><?php endif; ?>
					</td>
