...
<div class="content">
  ...
  <div class="cLineB">
            ...
            <div class="ftip" style="margin:0 auto">导出Excel后把Excel表格中的手机号等列弄宽一点就会显示真正完整的手机号等信息
    </div>
            <div style="margin-top:20px;"><a href="###" onclick="$('#import').css('display','')" class="btnGrayS vm">导入SN码</a>&nbsp;&nbsp;<a href="?g=User&m=Lottery&a=exportSN&id=<?php echo ($thisLottery["id"]); ?>" class="btnGrayS vm">导出中奖数据</a>&nbsp;&nbsp;<a href="/tpl/User/default/common/images/sndemo.xls" class="btnGrayS vm">下载导入Excel模板</a></div>
             <div style="margin-top:20px;display:none" id="import"><form enctype="multipart/form-data" action="?g=User&m=Upyun&a=localUploadSNExcel" id="thumbForm" method="POST" style="font-size:14px;padding:10px 20px 10px 0px;"><div>选择本地文件：<input type="file" style="width:250px;border:1px solid #ddd" name="photo"></input> <input id="submitbtn" name="doSubmit" type="submit" class="btnGreen" value="上传" onclick="this.value='上传中...'"></input> <a href="###" onclick="$('#import').css('display','none')" class="btnGrayS vm">取消导入</a> 注意：不支持xlsx格式</div><input type="hidden" value="<?php echo ($thisLottery["id"]); ?>" name="lid" /></form></div>
           ...
           <input name="wxid"  type="hidden" value="" />
            ...
            <td><?php if($record['sendtime'] == 0): else: echo (date('Y-m-d H:i:s',$record["sendtime"])); endif; ?>
						...
						<td>
							...
							<?php }else{ echo $phone[$record['id']]; } ?>
							
						</td>
						<td><?php echo ($record["wecha_name"]); ?></td>
						<td><?php if($record['time'] != 0): echo (date('Y-m-d H:i:s',$record["time"])); endif; ?></td>
						<td>
               <?php if($record['sendstutas'] == 0): if($record['sn'] == null){?>
				未中奖
				<?php }else{?>
                <a href="index.php?g=User&m=Lottery&a=sendprize&id=<?php echo ($record["id"]); ?>">发奖</a>
				<?php }?>
              <?php else: ?>
