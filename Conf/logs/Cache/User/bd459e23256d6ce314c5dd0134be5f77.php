...
<div class="left"> 
	<a href="<?php echo U('Custom/set',array('token'=>$token));?>" title="添加" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?> 添加自定义</a>
</div>
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
   $(document).ready(function(){
      ...
      $("#lay<?php echo ($item["set_id"]); ?>").click(function(){
   $("#brg<?php echo ($item["set_id"]); ?>").css("display","block");
   $("#showdiv<?php echo ($item["set_id"]); ?>").css("display","block");
   //$("#testdiv").load("test.html");
   });
   $("#close<?php echo ($item["set_id"]); ?>").click(function(){
      $("#brg<?php echo ($item["set_id"]); ?>").css("display","none");
   $("#showdiv<?php echo ($item["set_id"]); ?>").css("display","none");
   });
   });
 </script>
 <style type="text/css">
   #brg<?php echo ($item["set_id"]); ?>{ z-index:9999;width:100%; height:100%; background:#333; position:absolute; top:0; left:0; filter:alpha(opacity=60); -moz-opacity:0.6; opacity: 0.6; position:absolute; top:0; left:0; display:none;}
   #showdiv<?php echo ($item["set_id"]); ?>{z-index:9999; display:none;position:fixed; *position:absolute; width:100px; height:100px; top:30%; left:50%; margin:-50px 0 0 -50px;}
   #testdiv<?php echo ($item["set_id"]); ?>{ width:300px; height:300px; margin:0 auto; border:1px solid #4d4d4d; background:#f2f2f2;}
   #close<?php echo ($item["set_id"]); ?>{width:300px; height:27px; line-height:27px; font-size:14px; font-weight:bold; border:1px solid #4d4d4d; text-align:center; cursor:pointer; margin:0 auto; background:#333; color:#fff;margin-top: 15%;}
 </style>
    <div id="brg<?php echo ($item["set_id"]); ?>"></div>
 <div id="showdiv<?php echo ($item["set_id"]); ?>">
   <div id="close<?php echo ($item["set_id"]); ?>">您想关闭就点我吧</div>
   <div id="testdiv<?php echo ($item["set_id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($item["set_id"]); ?>&modle=Custom" width="300" height="300" class="cateimg_small"></div>
 </div>
   <a id="lay<?php echo ($item["set_id"]); ?>" style="width:70px;">查看二维码</a> &nbsp;|&nbsp;<?php endif; endif; ?>
	<a href="<?php echo U('Custom/info',array('set_id'=>$item['set_id'],'token'=>$token));?>">提交信息管理</a> &nbsp;|&nbsp;
	<a href="<?php echo U('Custom/forms',array('set_id'=>$item['set_id'],'token'=>$token));?>">输入项管理</a> &nbsp;|&nbsp;
	<a href="<?php echo U('Custom/record',array('set_id'=>$item['set_id'],'token'=>$token));?>">表单统计</a> &nbsp;|&nbsp;
	<a href="<?php echo U('Custom/set',array('set_id'=>$item['set_id'],'token'=>$token));?>">修改</a> &nbsp;|&nbsp;
	<a href="javascript:drop_confirm('删除将清除所有相关的设置信息和用户提交信息?', '<?php echo U('Custom/del',array('set_id'=>$item['set_id'],'token'=>$token));?>');">删除</a></td>
