...
<div class="content">
	...
	<div class="cLine">
		<div class="left"> 
			<a href="javascript:void(0);" onclick="showLog('')" title="卡券核销" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img class="vm" src="./tpl/User/default/common/images/product/add.png"><?php endif; ?> 卡券核销</a>
		</div>
		<div class="right" style="line-height:30px;margin:5px 0;">
       	<form action="" method="POST">
		   <select name="cardid" style="width:150px">
				<option value="">请选择所属会员卡</option>
				<?php if(is_array($allcard)): $i = 0; $__LIST__ = $allcard;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$allcard): $mod = ($i % 2 );++$i;?><option value="<?php echo ($allcard["id"]); ?>" <?php if($allcard['id'] == $cardid): ?>selected<?php endif; ?>><?php echo ($allcard["cardname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			-
          	<input type="text" name="keyword" value="<?php echo ($keyword); ?>" placeholder="请填写卡券号或者卡券名称" class="px" style="width:150px">
            ...
            <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
                	<td>
						<?php echo (date("Y-m-d H:i:s",$list["use_time"])); ?>
					</td>

					<td>
						<?php echo ($list["cancel_code"]); ?>
					</td> 
					...
					<td>
						<?php if($list["coupon_type"] == 2): ?>礼品券
						<?php elseif($list["coupon_type"] == 1): ?>
							优惠券
						<?php else: ?>
							代金券<?php endif; ?>
						<?php if($list["is_weixin"] == 1): ?>(微)
						<?php elseif($list["coupon_type"] == 1): ?>
							(赠)
						<?php else: ?>
							(普)<?php endif; ?>
...
function showLog(code){
	art.dialog.open('<?php echo U('Member_card/consume_use',array('token'=>$token));?>&code='+code,{lock:false,title:'卡券核销',width:750,height:600,yesText:'关闭',background: '#000',opacity: 0.87});
}
</script>
</div>
</div>
