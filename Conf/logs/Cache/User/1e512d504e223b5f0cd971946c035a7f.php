...
<li class="tabli" id="tab1"><a href="<?php echo U('Repast/kitchen', array('token' => $token,'cid' => $cid));?>">厨房管理</a></li>
<li class="current tabli" id="tab1"><a href="<?php echo U('Repast/index', array('token' => $token,'cid' => $cid));?>">餐台管理</a></li>
...
<li class="tabli" id="tab5"><a href="<?php echo U('Repast/Statistics', array('token' => $token,'cid' => $cid));?>">菜品统计</a></li>
...
<!--tab end-->
<div class="cLine">
<div class="pageNavigator left"> <a href="<?php echo U('Repast/add',array('token' => $token,'cid'=>$cid));?>" title="添加" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" class="vm"><?php endif; ?>添加桌台</a></div>
<div class="clr"></div>
</div>
<div class="msgWrap">
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tablelist): $mod = ($i % 2 );++$i;?><tr>
		...
		<td><?php if ($tablelist['isbox']) {echo "包间";} else {echo "大厅";} ?></td>
		<td><?php echo ($tablelist["num"]); ?></td>
		<td><?php if($tablelist['status'] == 1): ?>正在使用...<?php else: ?>未使用<?php endif; ?><p style="padding-top: 5px"><a class="btnGray vm" onclick="toSwitchStatus(1,<?php echo ($tablelist['id']); ?>,<?php echo ($tablelist['status']); ?>,0)">更新状态</a><p></td>
		<td><?php if(array_key_exists($tablelist['id'],$reserve)){ echo $reserve[$tablelist['id']]['reservestr'];echo '<p style="padding-top: 5px"><span style="color:red;">'.$reserve[$tablelist["id"]]["isusestr"].'<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class="btnGray vm" onclick="toSwitchStatus(2,'.$tablelist["id"].','.$reserve[$tablelist["id"]]["isuse"].','.$reserve[$tablelist["id"]]["id"].')">更新就餐状态</a><p>';} ?></td>
		<td class="norightborder">
			<a href="<?php echo U('Repast/add', array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>">修改</a> 
			<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Repast/del',array('id' => $tablelist['id'], 'token' => $token,'cid' => $tablelist['cid']));?>');">删除</a><?php if($wxuser['winxintype'] == 3): ?><a onclick="showEwmDetail(<?php echo ($tablelist['id']); ?>,'<?php echo ($tablelist['name']); ?>')" class="red">查看二维码</a><?php endif; ?>
		</td>
...
<script type="text/javascript">
/*href="<?php echo U('Repast/QRcode', array('token' => $token,'cid' => $cid,'tid'=>$tablelist['id'],'down'=>1));?>"*/
function toSwitchStatus(type,tid,sv,dtid){
	var tips=type==1 ? '你确定要修改餐桌使用状态吗？': '你确定要修改预约就餐状态吗？';
	if(confirm(tips)){
		$.post("<?php echo U('Repast/toSwitchStatus', array('token' => $token,'cid' => $cid));?>",{typ:type,tid:tid,vv:sv,dtid:dtid},function(ret){
			if(ret.error>0){
			  alert(ret.msg);
			}else{
...
function showEwmDetail(id,tname){
	art.dialog.open('<?php echo U('Repast/tableEwm',array('token'=>$token,'cid'=>$cid));?>&tid='+id,{lock:false,title:tname+' 餐桌二维码',width:580,height:380,yesText:'关闭',background: '#000',opacity: 0.87, close: function(){location.reload();}});
}
</script>
</div>
</div>
