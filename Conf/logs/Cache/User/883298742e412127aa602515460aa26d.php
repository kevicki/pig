...
<div class="content">
	<div class="cLineB">
		<h4 class="left">优惠活动</h4>
		<div class="clr"></div>
	</div>
	
	<div class="cLine">
		<div class="pageNavigator left">
			<a href="<?php echo U('Service/preferentialadd',array('token'=>$token,'type'=>'preferential'));?>" title="添加优惠" class="btnGrayS vm bigbtn">添加优惠</a>
		</div>
		<div class="pageNavigator right">
			<input type="text" id="name" class="px name" placeholder="输入优惠名称"  name="name" value="<?php echo ($_GET['name']); ?>" style="margin-top:10px">
	...
	<div class="msgWrap">
		<table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
			...
			<tbody>
				<?php if(is_array($preferential_list)): $i = 0; $__LIST__ = $preferential_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td><img src="<?php echo ($vo["img"]); ?>" width="60px"/></td>
						<td><?php echo ($vo["name"]); ?></td>
						<td><?php echo ($vo["info"]); ?></td>
						<td class="norightborder" style="border-right:1px solid #eee">
							<a href="<?php echo U('Service/preferentialupdate',array('token'=>$token,'type'=>'preferential','id'=>$vo['pigcms_id']));?>">修改</a>
							<a href="javascript:drop_confirm('您确定要删除【<?php echo ($vo["name"]); ?>】吗?', '<?php echo U('Service/preferentialdel',array('token'=>$token,'id'=>$vo['pigcms_id'],'type'=>'preferential'));?>')">删除</a>
						</td>
...
<script>
	...
	function seach(){
		var name = $("#name").val();
		if(name != ""){
			window.location.href="<?php echo U('Service/preferentiallist',array('token'=>$token,'type'=>'preferential'));?>&name="+name;
		}else{
			window.location.href="<?php echo U('Service/preferentiallist',array('token'=>$token,'type'=>'preferential'));?>";
		}
	}
</script>
</div>
