...
<style>
	...
	#tags LI.hove,#tags ul.large li.hove,#tags ul.large li.selectTag{
		BACKGROUND-POSITION: left top;
		MARGIN-BOTTOM: -2px;
		POSITION: relative;
		HEIGHT: 40px; 
	}
	#tags .hove a,#tags ul.large li.hove,#tags ul.large li.selectTag{
		COLOR: #000;
	...
	#tags .hove,#tags ul.large li.hove,#tags ul.large li.selectTag{
		background:#EEEEEE;
	}
</style>
<div class="msgWrap form">
	<ul id="tags" style="width:100%">
		<li <?php if($catid == 0): ?>class="hove"<?php endif; ?>>
			<a href="?g=User&m=Card&a=cardLibrary">全部</a>
		</li>
	<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li <?php if($catid == $list['cid'] OR $topid == $list['cid']): ?>class="hove"<?php endif; ?>>
			<a href="?g=User&m=Card&a=cardLibrary&catid=<?php echo ($list["cid"]); ?>"><?php echo ($list["title"]); ?></a>
			<?php if($list['down']): ?><ul class="large">
					<?php if(is_array($list['down'])): $i = 0; $__LIST__ = $list['down'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$aot): $mod = ($i % 2 );++$i;?><li <?php if($catid == $aot['cid']): ?>class="hove"<?php endif; ?>><a href="?g=User&m=Card&a=cardLibrary&catid=<?php echo ($aot["cid"]); ?>"><?php echo ($aot["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul><?php endif; ?>
...
<script>
	$(function(){
		$('#tags').find('li').mouseover(function(){
			$(this).toggleClass('selectTag');
			if($(this).find('.large').size() > 0){
		...
		$('.large').find('li').mouseover(function(e){
          ...
          <div class="ftip" style="margin:25px auto 5px auto;">微贺卡是我们与第三方微贺卡平台合作功能，将会不断推出热门微信贺卡</div>
...
<td><img src="<?php echo ($item["logo"]); ?>" width="60" /></td>
<td><?php echo ($item["title"]); ?></td>
<td><?php echo ($item["intro"]); ?></td>
<td class="norightborder">
<a href="<?php echo U('Card/cardSet',array('cardid'=>$item['id'],'token'=>$token));?>" >使用此贺卡</a>
