...
<!--tab end-->
<div class="msgWrap">
<form method="post" action="<?php echo U('Forum/delMessage');?>" id="info">
<div class="cLine">
<div class="pageNavigator left"> <a href="###" onclick="$('#info').submit()" title="批量删除" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/arrow_switch.png" class="vm">批量删除</a></div>
<div class="clr"></div>
</div>
...
<tr></tr>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><?php echo ($t["id"]); ?></td>
			<td><?php echo (($t["touname"])?($t["touname"]):'游客'); ?></td>
			<td><?php echo (htmlspecialchars_decode($t["content"])); ?></td>
			<td><?php echo (date("Y-m-d H:i:s",$t["createtime"])); ?></td>
			<td><?php if($t['isread'] == 1): ?><font color="red">未读</font><?php else: ?>已读<?php endif; ?></td> 
			<td class="norightborder"><a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Forum/delMessage',array('token'=>$_GET['token'],'id'=>$t['id']));?>');">删除</a></td>
