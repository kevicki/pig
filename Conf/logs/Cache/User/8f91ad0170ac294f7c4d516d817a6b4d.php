...
<div class="content">
<div class="cLineB">
<h4 class="left">微信合体红包</h4>
<div class="clr"></div>
</div>
<div class="cLine">
<div class="pageNavigator left"><a href="<?php echo U('Hongbao/action_add',array('token'=>$token));?>" title="添加活动" class="btnGrayS vm bigbtn"><?php if($usertplid == 2): ?><i class="fa fa-hand-o-right"></i><?php else: ?><img src="<?php echo RES;?>/images/product/add.png" width="16" class="vm"><?php endif; ?> 添加活动</a> </div>
<div class="searchbar right">
<form method="post" action="<?php echo U('Hongbao/index',array('token'=>$token));?>">
<input type="text" id="msgSearchInput" class="txt left" placeholder="输入关键词搜索" name="search_word" value="<?php echo ($search_word); ?>">
...
<tr>                                 
	...
	<th width="70">关键词</th>
	<th width="50">分享人数</th>
	<th width="55">红包总金额</th>
 	<th width="170">活动时间</th>
 	<th width="50">是否开启</th>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	...
	<td><?php echo ($list["sharetimes"]); ?></td>
	<td><?php echo ($list["total_money"]); ?></td>
	<td><?php echo (date("Y-m-d H:i",$list["start_time"])); ?> 至  <?php echo (date("Y-m-d H:i",$list["end_time"])); ?></td>
	<td>
		<?php if($list["status"] == '1'): ?>已开启
		<?php else: ?>
			未开启<?php endif; ?>
	</td>
...
<?php if($_SESSION['is_syn']== 2): if($usertplid != 2): ?><script type="text/javascript">
 ...
 <div id="showdiv<?php echo ($list["id"]); ?>">
   ...
   <div id="testdiv<?php echo ($list["id"]); ?>"><img src="<?php echo (session('source_domain')); ?>/wap.php?c=Wxapp&a=qrcode&id=<?php echo ($list["id"]); ?>&modle=Hongbao" width="300" height="300" class="cateimg_small"></div>
 </div>
	<a id="lay<?php echo ($list["id"]); ?>"  style="width:191px;">查看二维码</a><br><?php endif; endif; ?>
		<a href="<?php echo U('Hongbao/get_packet_list',array('token'=>$token,'id'=>$list['id']));?>">领取记录</a>
		|
		<a href="<?php echo U('Hongbao/action_add',array('token'=>$token,'id'=>$list['id']));?>">修改</a>
		|
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hongbao/del',array('token'=>$token,'id'=>$list['id']));?>');">删除</a>
    </td>
