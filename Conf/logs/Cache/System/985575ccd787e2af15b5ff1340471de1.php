...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="alist">
	  <tr>
	    ...
	    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				...
				<td ><?php if($vo['is_admin'] == 1){echo '系统超级管理员';}else{ echo ($role[$vo['role']]); }?></td>
				...
				<td align='center'>
					<?php if($_SESSION[C('ADMIN_AUTH_KEY')] || $vo['is_admin'] == 0){?><a href="<?php echo U('User/edit/',array('id'=>$vo['id']));?>">修改</a><?php }else{?><font color="#cccccc">修改</font><?php }?>
					| <?php if($vo['is_admin'] == 1 || session('username') == $vo['username']){ ?><font color="#cccccc">删除</font><?php }else{ ?><a href="javascript:void(0)" onclick="return confirmurl('<?php echo U('User/del/',array('id'=>$vo['id']));?>','确定删除该用户吗?')">删除</a><?php }?>
				</td>
