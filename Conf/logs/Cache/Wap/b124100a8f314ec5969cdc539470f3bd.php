...
<div class="cardexplain">
	<li class="nob">
		<div class="beizhu">
			<?php if ($cardid){?>
			<?php if($cardInfo != false): ?>您可以修改你的会员卡信息。以下信息将作为消费凭证，请认真填写！ <?php else: ?>填写以下信息即可领取vip会员卡,红色*标志的必填<?php endif; ?>
			<?php }else {echo '请先完善您的个人信息，红色*标志的为必填项';}?>
		</div>
	</li>
	<ul class="round">
		<?php echo ($formData); ?>
...
<script type="text/javascript">
//var configs = <?php echo ($configs); ?>;

$(document).ready(function(){
	$('.px').blur(function(){
		var field = $(this).attr('id');
		if ($(this).attr('data-empty') && $(this).val() == '') {
	...
	$("#showcard").bind("click",
			function() {
				...
				var data = new Array();
			 	<?php if(is_array($verify)): $i = 0; $__LIST__ = $verify;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i; if($item['item_name'] == 'portrait'): ?>data[<?php echo ($i); ?>] = {'id':$("#field_<?php echo ($item['id']); ?>").val(), 'val':$('#portrait').val(), 'type':"<?php echo ($item['type']); ?>", 'item_name':"<?php echo ($item['item_name']); ?>"};		
					<?php elseif($item['type'] == 'checkbox'): ?>			
						<?php if($item['is_empty'] == 1): ?>if($(".field_id_<?php echo ($item['id']); ?>:checked").length == 0){
								alert("<?php echo ($item['name']); ?>不能为空")
								return false;
							}<?php endif; ?>			
						
						var val = '', pre = '';
						$(".field_id_<?php echo ($item['id']); ?>").each(function(){
							if ($(this).attr('checked')) {
								val += pre + $(this).val();
								pre = '|';
							}
						});
						data[<?php echo ($i); ?>] = {'id':$("#field_<?php echo ($item['id']); ?>").val(), 'val':val, 'type':"<?php echo ($item['type']); ?>", 'item_name':"<?php echo ($item['item_name']); ?>"};
					<?php elseif($item['type'] == 'radio'): ?>	
						data[<?php echo ($i); ?>] = {'id':$("#field_<?php echo ($item['id']); ?>").val(), 'val':$(".field_id_<?php echo ($item['id']); ?>:checked").val(), 'type':"<?php echo ($item['type']); ?>", 'item_name':"<?php echo ($item['item_name']); ?>"};
					<?php else: ?>
						data[<?php echo ($i); ?>] = {'id':$("#field_<?php echo ($item['id']); ?>").val(), 'val':$("#field_id_<?php echo ($item['id']); ?>").val(), 'type':"<?php echo ($item['type']); ?>", 'item_name':"<?php echo ($item['item_name']); ?>"};
						<?php if($item['is_empty'] == 1): ?>if($.trim($("#field_id_<?php echo ($item['id']); ?>").val()) == ""){
								alert("<?php echo ($item['name']); ?>不能为空");
								$("#field_id_<?php echo ($item['id']); ?>").css('background', 'red');
								return false;
							}<?php endif; ?>
						<?php if($item['match'] != '' AND $item['item_name'] != 'portrait'): ?>if (!new RegExp(/<?php echo ($item['match']); ?>/).test($.trim($("#field_id_<?php echo ($item['id']); ?>").val())) && $("#field_id_<?php echo ($item['id']); ?>").val() != "") {
								alert("<?php echo ($item['name']); ?>的输入格式不正确")
								return false;
							}<?php endif; endif; endforeach; endif; else: echo "" ;endif; ?>
				
				/*if (tel == '' && configs.tel == 1 && configs.is_tel == 1) {
					document.getElementById('tel').style.background="red";
			        alert("请认真输入手机号");
			        return
			    }
			    if (wechaname == '' && configs.wechaname == 1 && configs.is_wechaname == 1) {
					document.getElementById('wechaname').style.background="red";
			        alert("请认真输入微信号");
			        return
			    }
			    if (truename == '' && configs.truename == 1 && configs.is_truename == 1) {
					document.getElementById('truename').style.background="red";
			        alert("请认真输入真实姓名");
			        return
			    }
			    if (qq == '' && configs.qq == 1 && configs.is_qq == 1) {
					document.getElementById('qq').style.background="red";
			        alert("请认真输入QQ号");
			        return
			    }
			    if (paypass == '' && configs.paypass == 1 && configs.is_paypass == 1) {
					document.getElementById('paypass').style.background="red";
			        alert("请认真输入支付密码");
			        return
			    }
			    if (portrait == '' && portrait_img == '/tpl/User/default/common/images/portrait.jpg' && configs.portrait == 1 && configs.is_portrait == 1) {
					$('#ul_portrait').css('background-color','red');
					$('.a_upload').css('color','#333');
			        alert("请设置您的头像");
			        return
			    }
			    if (sex == '请选择你的性别' && configs.sex == 1 && configs.is_sex == 1) {
					document.getElementById('sex').style.background="red";
			        alert("请选择你的性别");
			        return
			    }
			    if (bornyear == '' && configs.bornyear == 1 && configs.is_bornyear == 1) {
					document.getElementById('bornyear').style.background="red";
			        alert("请认真输入出生年");
			        return
			    }
			    if (bornmonth == '' && configs.bornmonth == 1 && configs.is_bornmonth == 1) {
					document.getElementById('bornmonth').style.background="red";
			        alert("请认真输入出生月");
			        return
			    }
			    if (bornday == '' && configs.bornday == 1 && configs.is_bornday == 1) {
					document.getElementById('bornday').style.background="red";
			        alert("请认真输入出生日");
			        return
			    }
			    if (address == '' && configs.address == 1 && configs.is_address == 1) {
					document.getElementById('address').style.background="red";
			        alert("请认真输入地址");
			        return
			    }
			    if (origin == '' && configs.origin == 1 && configs.is_origin == 1) {
