	...
	public function bus($x='',$y='',$companyindex=1){
		...
		if (!$this->isamap){
			...
			if ($rt['route']['transits']){
				...
				$link = 'http://m.amap.com/?from='.$x.','.$y.'&to='.$thisCompany['latitude'].','.$thisCompany['longitude'].'&type=1&opt=0';
				$return[]=array('公交到'.$thisCompany['name'].'行程'.$distanceStr.$durationStr,"点击查看具体方案".$stepStr,$imgUrl,$link);
				return array($return,'news');
