...
class WechatCoupons extends apiOauth
{
	...
	public function createCard($id, $is_edit = 0)
	{
		...
		$base_info = new BaseInfo($logo_url['url'], $brand_name, "CODE_TYPE_BARCODE", $title, $info['color'], '使用时向店员员出示此券', $company[0]['tel'], $info['info'], new DateInfo(1, intval($info['statdate']), intval($info['enddate'])), new Sku($info['total']));
