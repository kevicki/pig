...
class FunctionAction extends UserAction
{
	...
	'where' => array('token' => $this->token)
		...
		$this->redirect('User/Function/welcome', array('token' => $token, 'id' => $id));
		exit();
		session('token', $token);
	...
	'where' => array('token' => $this->token)
		...
		if ($this->usertplid != 1) {
			$this->redirect('User/Home/set', array('token' => $token));
			exit();
		}

		$data = array();
		$data['mp'] = M('Wxuser')->where(array('uid' => intval(session('uid'))))->count();
		$data['card'] = M('Member_card_create')->where(array(
	'token'    => $this->token,
	'wecha_id' => array('neq', '')
	))->count();
