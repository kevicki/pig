...
class SceneAction extends UserAction
{
				...
				if ($scene_info) {
					...
					if ($this->scene_db->where(array('token' => $this->token, 'id' => $id))->save($_POST)) {
						if ($this->_post('wall_id', 'intval')) {
							$this->scene_db->where(array(
	'token' => $this->token,
	'id'    => array('neq', $id)
	))->save(array('is_open' => '0'));
	...
	'token' => $this->token,
	'type'  => 'scene',
	'id'    => array('in', explode(',', $scene_info['vote_id']))
	))->select();
	...
	public function ajaxWall()
	{
		...
		$where = array(
			...
			'check_time' => array('gt', intval($_GET['ajax_time']))
			);
	...
	public function _getWallList($where, $order, $limit = '', $sceneid = '', $type = '')
	{
		...
		foreach ($message as $key => $value) {
			$message[$key]['nickname'] = $wuser_db->where(array('id' => $value['uid']))->getField('nickname');
			$message[$key]['portrait'] = $wuser_db->where(array('id' => $value['uid']))->getField('portrait');
	...
	public function getConnectNum()
	{
		...
		if ($sceneid) {
			...
			$count = M('Wall_member')->alias('member')->where($where)->count();
		}
	...
	public function shakeRun()
	{
		...
		$is_end = $rt_db->where(array(
	'is_scene' => $is_scene,
	'token'    => $this->token,
	'shakeid'  => $id,
	'round'    => 0,
	'count'    => array('egt', $shake_info['endshake'])
	))->find();
		$result = array();
		if ($is_end || (($shake_info['endtime'] + $shake_info['usetime'] + $shake_info['starttime']) < time())) {
			$result['status'] = 1;
			$result['info'] = '游戏已经结束';
	...
	public function show_shake()
	{
		...
		$max = M('Shake_rt')->where(array(
	'is_scene' => $is_scene,
	'token'    => $this->token,
	'shakeid'  => $id,
	'round'    => array('neq', 0)
	))->order('round desc,count desc')->group('round')->order('round asc')->getField('round', true);
	...
	public function percent($p, $t, $offset = 0)
	{
		if ($t == 0) {
			$val = 1;
		}
	...
	public function prize_data()
	{
		...
		$prize_user = M('Wall_prize_record')->where(array('sceneid' => $id, 'prize' => $pid))->order('id asc')->select();
	...
	{
		$vote_list = M('Vote')->where(array(
	'token' => $this->token,
	'id'    => array('in', $this->info['vote_id'])
	))->select();
		$now = time();
	...
	public function ajaxVcount()
	{
		...
		$result['res'] = $res;
		$time = time();
		if (($vote_info['statdate'] < $time) && ($time < $vote_info['enddate'])) {
			$result['flag'] = 1;
		}
		else {
			$result['flag'] = 0;
