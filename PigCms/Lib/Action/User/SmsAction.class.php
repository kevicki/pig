...
class SmsAction extends UserAction
{
	...
	public function set()
	{
		...
		if (IS_POST) {
			...
			else if ($sms_config_model->create()) {
	...
	public function buy_post()
	{
		...
		if (($needFee < $moneyBalance) || ($needFee == $moneyBalance)) {
			...
			$indent = array();
			$indent['id'] = time();
			M('Indent')->data(array('uid' => session('uid'), 'month' => 0, 'title' => '购买短信' . (int) abs($_POST['count']) . '条', 'uname' => $this->user['username'], 'gid' => 0, 'create_time' => time(), 'indent_id' => $indent['id'], 'price' => $spend, 'status' => 1))->add();
			M('Users')->where(array('id' => $this->user['id']))->setDec('moneybalance', (int) abs($needFee));
			M('Users')->where(array('id' => $this->user['id']))->setInc('smscount', (int) abs($_POST['count']));
			M('Sms_expendrecord')->add(array('uid' => $this->user['id'], 'price' => (int) abs($needFee), 'count' => (int) abs($_POST['count']), 'time' => time()));
