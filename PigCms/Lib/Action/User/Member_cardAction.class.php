...
class Member_cardAction extends UserAction
{
	...
	public function design()
	{
		...
		if (IS_POST) {
			...
			if ($data == false) {
				$data = array('token' => $this->token, 'cardname' => C('site_name') . '会员卡', 'logo' => '/tpl/User/default/common/images/cart_info/logo-card.png', 'bg' => './tpl/User/default/common/images/card/card_bg15.png', 'diybg' => '/tpl/User/default/common/images/card/card_bg17.png', 'msg' => '微信会员卡，方便携带收藏，永不挂失', 'numbercolor' => '#000000', 'vipnamecolor' => '#121212', 'Lastmsg' => '/tpl/User/default/common/images/cart_info/news.jpg', 'vip' => '/tpl/User/default/common/images/cart_info/vippower.jpg', 'qiandao' => '/tpl/User/default/common/images/cart_info/qiandao.jpg', 'shopping' => '/tpl/User/default/common/images/cart_info/shopping.jpg', 'memberinfo' => '/tpl/User/default/common/images/cart_info/user.jpg', 'membermsg' => '/tpl/User/default/common/images/cart_info/vippower.jpg', 'contact' => '/tpl/User/default/common/images/cart_info/addr.jpg', 'recharge' => '/tpl/User/default/common/images/cart_info/recharge.jpg', 'payrecord' => '/tpl/User/default/common/images/cart_info/payrecord.jpg');
			}
	...
	public function coupons_save()
	{
		...
		if (IS_POST) {
			...
			else if ($_POST['enddate'] < $info['enddate']) {
	...
	public function coupons_stock()
	{
		...
		else if (M('Member_card_coupon')->where(array('token' => $this->token, 'id' => $itemid))->setInc('total', $number)) {
			echo json_encode(array('errcode' => '0'));
		}
		else {
	...
	public function members()
	{
		...
		if ($members) {
			...
			foreach ($members as &$member) {
				...
				else if (!in_array($member['wecha_id'], $wecha_ids)) {
					array_push($wecha_ids, $member['wecha_id']);
				}
			}
	...
	public function member()
	{
		...
		if ($number) {
			...
			if (!empty($num)) {
				$wecha_id = M('Member_card_create')->where(array(
	'token'  => $this->token,
	'number' => array('in', $num)
	))->getField('wecha_id', true);
	...
	public function custom()
	{
		...
		foreach ($fields as $ff) {
			unset($no_delete_fields[$ff['item_name']]);
		}
	...
	public function center()
	{
		...
		$wecha_ids && ($user_infos = M('Userinfo')->where(array(
	'token'    => $this->token,
	'wecha_id' => array('in', $wecha_ids)
		...
		$cardids && ($sets = M('Member_card_set')->where(array(
	'token' => $this->token,
	'id'    => array('in', $cardids)
	))->select());
		$slist = array();
	...
	public function import_company()
	{
		...
		if ($res['res']['errcode'] == 0) {
			...
			for ($count = count($location_id_list); $i < $count; $i++) {
	...
	public function donate_set()
	{
		...
		if (IS_POST) {
			...
			$data = array('cardid' => $this->_post('cardid', 'intval'), 'token' => $this->token, 'min_price' => $min_price, 'max_price' => $max_price, 'donate_price' => $donate_price, 'is_open' => $is_open);
	...
	public function consume_record()
	{
		...
		$Page = new Page($count, 15);
		$list = M('Member_card_coupon_record')->alias('record')->join('LEFT JOIN ' . C('DB_PREFIX') . 'member_card_coupon coupon ON coupon.id=record.coupon_id')->where($where)->field('record.id,record.wecha_id,record.staff_id,record.company_id,record.cancel_code,record.use_time,record.is_use,record.coupon_type,coupon.title,coupon.is_weixin')->order('record.is_use asc,record.add_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
	...
	public function coupons_record()
	{
		...
		$count = M('Member_card_coupon_record')->alias('record')->join('LEFT JOIN ' . C('DB_PREFIX') . 'member_card_coupon coupon ON coupon.id=record.coupon_id')->join('LEFT JOIN ' . C('DB_PREFIX') . 'userinfo user ON record.wecha_id=user.wecha_id')->where($where)->count();
		$Page = new Page($count, 15);
		$list = M('Member_card_coupon_record')->alias('record')->join('LEFT JOIN ' . C('DB_PREFIX') . 'member_card_coupon coupon ON coupon.id=record.coupon_id')->join('LEFT JOIN ' . C('DB_PREFIX') . 'userinfo user ON record.wecha_id=user.wecha_id AND record.token= user.token')->where($where)->field('record.id,record.add_time,record.staff_id,record.company_id,record.cancel_code,record.use_time,record.is_use,record.coupon_type,record.iswhere,record.whereid,coupon.title,coupon.is_weixin,user.wechaname,user.tel')->order('record.add_time desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
