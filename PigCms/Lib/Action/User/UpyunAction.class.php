...
class UpyunAction extends UserAction
{
	...
	public function upload()
	{
		...
		if ($this->upload_type == 'upyun') {
			...
			$options['return-url'] = $this->siteUrl . '/index.php?g=User&m=Upyun&a=uploadReturn';
	...
	public function localUploadPayrecordExcel()
	{
		...
		if ($return['error']) {
			...
			if ($rows) {
				...
				foreach ($rows as $r) {
					if ($i != 0) {
						if (!M('Company')->where(array('token' => $this->token, 'id' => $r[12]))->find()) {
							$this->error('请输入有效的商户ID', U('Member_card/member', array('token' => $this->token, 'type' => $type)));
							exit();
						}

						if (!M('Member_card_set')->where(array('token' => $this->token, 'id' => $r[13]))->find()) {
							$this->error('请输入有效的卡ID', U('Member_card/member', array('token' => $this->token, 'type' => $type)));
							exit();
						}
	...
	public function localUpload($filetypes = '', $wximg)
	{
		...
		if (!$upload->upload()) {
			...
			if ($wximg) {
				$Files->index($msg, intval($info[0]['size']), $info[0]['extension'], $this->user['id'], $this->token, 0, $wximg->url, $wximg->media_id);
			}
			else {
				$Files->index($msg, intval($info[0]['size']), $info[0]['extension'], $this->user['id'], $this->token, 0);
			}
		}
