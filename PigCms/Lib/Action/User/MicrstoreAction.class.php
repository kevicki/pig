...
class MicrstoreAction extends UserAction
{
	...
	public function withdraw()
	{
		...
		if ($return['error_code'] == 0) {
			...
			foreach ((array) $return['withdrawals'] as $key => $val) {
				$insert_data['pigcms_id'] = $val['pigcms_id'];
				...
				$exists = M('microsoft_withdraw')->where(array('pigcms_id' => $val['pigcms_id']))->find();

				if ($exists) {
					$update = M('microsoft_withdraw')->where(array('pigcms_id' => $val['pigcms_id']))->save($insert_data);
				}
				else {
	...
	public function edit_withdraw()
	{
		if (IS_POST) {
			...
			$pigcms_id = $this->_post('pigcms_id', 'intval');
			...
			if (empty($status) || empty($pigcms_id)) {
