...
class CustomAction extends WapAction
{
	...
	public function index()
	{
		...
		if (IS_POST) {
			...
			if (0 < $limit_info['today_total']) {
				...
				$total = $this->info_db->where(array(
	'token'    => $this->token,
	'wecha_id' => $this->wecha_id,
	'set_id'   => $this->thisForm['set_id'],
	...
	private function _createForms($token, $set_id)
	{
		...
		foreach ($forms as $key => $value) {
			...
			$str .= '</td></tr>';
			$arr[] = array('id' => $value['item_name'], 'name' => $value['field_name'], 'type' => $value['field_type'], 'match' => $value['field_match'], 'is_empty' => $value['is_empty'], 'err_info' => $value['err_info']);
		}
