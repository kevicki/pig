...
class ResearchAction extends LotteryBaseAction {
	...
	public function __construct(){
		...
		$this->wecha_id = isset($_REQUEST['wecha_id']) ? htmlspecialchars($_REQUEST['wecha_id']) :  '';//$this->wecha_id;
		$this->token = isset($_REQUEST['token']) ? htmlspecialchars($_REQUEST['token']) :  '';//$this->_get('token');
	...
	public function lotter()
	{
		...
		if ($this->_research['starttime'] > time()) {
			$this->error('活动未开始，还不能抽奖', U("Research/index", array('reid' => $this->_rid, 'wecha_id' => $this->wecha_id, 'token' => $this->token)));
		} elseif ($this->_research['endtime'] < time()) {
			$this->error('活动已结束，下次来早点吧', U("Research/index", array('reid' => $this->_rid, 'wecha_id' => $this->wecha_id, 'token' => $this->token)));
		}
