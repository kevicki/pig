<?php
class GoldenEggAction extends LotteryBaseMoreAction{
	...
	public function getajax(){	
		...
		if($fwy == $_POST['fwy']){
			...
			if ($data['prizetype'] >= 1 && $data['prizetype'] <= 6) {
				...
				$arr=array('success'=>1,'data'=>array('prize_type'=>$data['prizetype'],'sn'=>$data['sncode'],'prize'=>$prizeName,'rid'=>$data['rid']),'msg'=>'','usenums'=>$data['usenums'],'rid'=>$data['rid']);
			}else{
