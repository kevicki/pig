...
class UnitaryAction extends WapAction
{
	...
	public function cartajax()
	{
		...
		case "delorder":
			...
			foreach ($order_list2 as $vo2 ) {
				...
				$where_cart2["order_id"] = $vo2["pigcms_id"];
		...
		case "click_cart_count":
			...
			foreach ($cart_list as $vo ) {
				$lucknum_count = $this->m_lucknum->where(array("token" => $_POST["token"], "cart_id" => $vo["id"]))->count();
				if (($lucknum_count < 1) && ($vo["addtime"] < (time() - 120))) {
	...
	public function dobuy()
	{
		...
		foreach ($cart_list as $vo ) {
			...
			foreach ($cart_list2 as $cvo ) {
				$lucknum_count = $this->m_lucknum->where(array("token" => $_POST["token"], "cart_id" => $cvo["id"]))->count();
				if (($lucknum_count < 1) && ($vo["addtime"] < (time() - 120))) {
		...
		if (0 < $id_order) {
			...
			if (0 < $update_cart) {
				$this->redirect("Alipay/pay", array("token" => $this->token, "price" => $total, "wecha_id" => $this->wecha_id, "from" => "Unitary", "orderid" => $save_order["orderid"], "single_orderid" => $save_order["orderid"], "notOffline" => 1));
			}
		}
	}
	...
	public function buyres()
	{
		...
		foreach ($cart_list as $k => $vo ) {
			...
			if (($cart_list[$k]["count"] < 1) && ($vo["addtime"] < (time() - 120))) {
				$this->m_cart->where(array("token" => $_POST["token"], "id" => $vo["id"]))->delete();
			}
