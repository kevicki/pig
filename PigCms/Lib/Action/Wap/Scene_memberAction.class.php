...
class Scene_memberAction extends WapAction
{
	...
	public function set()
	{
		...
		if ((1 == $where['act_type']) || (3 == $where['act_type'])) {
			$data['act_type'] = $where['act_type'] = 1 == $where['act_type'] ? 3 : 1;
			$info2 = M('Wall_member')->where($where)->find();

			if ($info2) {
				M('wall_member')->where($where)->save($data);
			}
			else {
				M('wall_member')->add($data);
			}

			$data['act_type'] = $where['act_type'] = $this->_get('act_type', 'intval');
		}
		...
		else if (M('wall_member')->add($data)) {
