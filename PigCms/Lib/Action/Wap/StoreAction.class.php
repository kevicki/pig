...
class StoreAction extends WapAction{
	...
	public function _initialize() 
	{
		...
		if ($twitter_set && empty($this->wecha_id) && empty($this->mytwid) && empty($login) && !in_array(ACTION_NAME, array('register', 'login'))) {
			...
			//点击链接时的推广记录
			session("login", 1);
			$this->redirect(U('Store/login', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->_twid, 'rget' => 1)));
		}
...
// 		}
	...
	public function select()
	{
		...
		$company = M('Company')->where("`token`='{$this->token}' AND ((`isbranch`=1 AND `display`=1) OR `isbranch`=0) AND (`business_type` LIKE '%Store%' OR `business_type`='')")->select();
		if (count($company) == 1) {
	...
	public function cats() 
	{
		...
		D("Product_setting")->where(array('token' => $this->token, 'cid' => 0))->save(array('cid' => $company['id'], 'price' => -1));
	...
	public function products() 
	{
		...
		$products = $this->product_model->where($where)->order($order . ' ' . $method)->limit('0, 8')->select();
...
// 		}
	...
	public function ordersave()
	{
		...
		$row['truename'] = $_REQUEST['truename'];
		$row['tel'] = $_REQUEST['tel'];
		$row['address'] = $_REQUEST['address'];
		$row['token'] = $this->token;
		$row['wecha_id'] = $wecha_id;
		$row['paymode'] = isset($_REQUEST['paymode']) ? intval($_REQUEST['paymode']) : 0;
		...
		if ($carts){
			...
			if ($normal_rt && empty($orid)) {
				...
				Sms::sendSms($this->token, "您的顾客{$row['truename']}刚刚下了一个订单，订单号：{$orderid}，手机号：{$row['tel']}请您注意查看并处理",$company['mp']);
			}
		}
		if ($normal_rt){
			$product_model = M('product');
...
// 					$userinfo_model->where(array('id' => $thisUser['id']))->save($userRow);
			...
			$model->sendTempMsg('TM00184', array('href' => U('Store/my',array('token' => $this->token, 'wecha_id' => $wecha_id, 'success' => 1, 'twid' => $this->_twid), true, false, true), 'wecha_id' => $wecha_id, 'first' => '购买商品提醒', 'ordertape' => date("Y年m月d日H时i分s秒"), 'ordeID' => $orderid, 'remark' => '下单成功，请尽快支付订单！'));
			if ($totalprice) {
...
// 			}
	...
	{
//         if (!session('twid')) {
//             $this->redirect(U('Store/login', array('token' => $this->token, 'wecha_id' => $this->wecha_id, 'twid' => $this->mytwid)));
//         }
		if ($this->mytwid) {
