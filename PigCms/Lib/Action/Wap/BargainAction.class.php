...
class BargainAction extends WapAction
{
	...
	public function home()
	{
		...
		foreach ($bargain_list as $k => $v) {
			...
			$where_order_paynum['bargain_id'] = $v['pigcms_id'];
	...
	public function dao()
	{
		...
		$where_order1['pigcms_id'] = (int) $_GET['orderid'];
		...
		if (($order1 == '') || ($order1['wecha_id'] == $this->wecha_id)) {
			...
			if ($order2 == '') {
				...
				$where_kanuser['orderid'] = $order2['pigcms_id'];
			...
			if ($kanuser == '') {
				...
				$where_kanuser2['orderid'] = $order1['pigcms_id'];
	...
	public function operate()
	{
		...
		case 'firstdao':
			...
			if ($order == '') {
				...
				$where_order_orderid['pigcms_id'] = $id_order;
		...
		case 'friendkan':
			...
			if ($kanuser == '') {
				...
				if ($order['bargain_nowprice'] == $bargain['minimum']) {
					...
					$add_kanuser['bargain_id'] = (int) $_GET['id'];
					$add_kanuser['orderid'] = $order['pigcms_id'];
					$add_kanuser['friend'] = $this->wecha_id;
	...
	public function payuserinfo()
	{
		...
		if ($_GET['orderid'] == '') {
			...
			$id_order = $this->m_order->add($add_order);
			$where_order['token'] = $this->token;
			$where_order['pigcms_id'] = $id_order;
			$randnum = rand(1000, 9999);
			$save_order_orderid['orderid'] = $id_order . 'bargain' . time() . $randnum;
