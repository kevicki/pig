...
class MedicalAction extends WapAction
{
	...
	public function registered()
	{
		...
		$user = $mdt_user->where($where2)->order('iid DESC')->find();
	...
	public function add()
	{
		if (IS_POST) {
			...
			$op = new orderPrint();
			$paid = (0 < $resdata['price'] ? 0 : 1);
			$msg = $this->content_layout($da);
			$op->printit($token, 0, 'Medical', $msg, $paid);

			if (0 < $resdata['price']) {
				$this->success('正在提交中...', U('Alipay/pay', array('token' => $da['token'], 'wecha_id' => $da['wecha_id'], 'success' => 1, 'from' => 'Medical', 'orderName' => '预约挂号费用', 'single_orderid' => $da['orderid'], 'price' => $resdata['price'], 'orderid' => $da['orderid'], 'notOffline' => 1)));
			}
			else if (!empty($insertdata)) {
				Sms::sendSms($da['token'], '您好,您的会员 ' . $da['truename'] . ',已经预约了 ' . $da['orderName'] . ' ,订单号为 ' . $da['orderid'] . ' 下单时间: ' . date('Y-m-d H:i:s'));
				Sms::sendSms($da['token'], '亲爱的 ' . $da['truename'] . ',您预约的 ' . $da['orderName'] . ' 已经成功,订单号为 ' . $da['orderid'] . '! 下单时间:' . date('Y-m-d H:i:s'), $da['utel']);
				$savedata['paid'] = 1;
				$book->where(array('id' => $insertdata, 'token' => $token))->save($savedata);
				$this->success('恭喜预约成功,请不要重复刷新页面...', U('Medical/ReserveBooking', array('token' => $token, 'wecha_id' => $wecha_id)));
				exit();
			}
			else {
				$this->redirect('Medical/registered', array('token' => $token, 'wecha_id' => $wecha_id), 3, '您好,预约失败，请重新预约,请不要重复刷新页面,请耐心等待...');
				exit();
			}
		}
	}
	...
	private function content_layout($order)
	{
		...
		$return .= chr(10) . '预约科室：' . $order['yyks'];
		$return .= chr(10) . '预约专家：' . $order['yyzj'];
	...
	private function generateOrderSn()
	{
		...
		return date('ymdHis', $sec) . substr($msec, 2, 4);
	}
