...
class WeizhaohuanAction extends WapAction{
	...
	public function rank(){
		...
		$where 	= array('token'=>$this->token,'pid'=>$this->Weizhaohuan['id'],'is_real'=>1);
	...
	public function add_puser($wecha_id,$is_real=1,$id){
		...
		$user = M('Weizhaohuan_user')->where(array('token'=>$this->token,'wecha_id'=>$wecha_id,'is_real'=>$is_real,'pid'=>$this->Weizhaohuan['id']))->find();
		if(empty($user)){
