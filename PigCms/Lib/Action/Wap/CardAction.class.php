...
/*              $owhere         = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'is_open'=>'1','cardid'=>$thisCard['id'],'start'=>array('lt',time()),'end'=>array('gt',time()));
    ...
    public function bind() {
		...
		if (IS_POST) {
			$number = $this -> _post('new_number');

			// 验证线下会员卡号是否与erp系统相同
			$trim_number = preg_replace('/^0+/', '', $number);
			$erp_phone = $this -> _post('new_phone');
			// 切换到erp数据库
			$model = M();
			$model = $model -> db(1, 'mysql://blbook:ONoiE8EVO0TDtp@119.29.116.37/weixin');

			$card_where['card_no'] = $number;
			$card_info = $model -> db(1) -> table('memberinfo') -> where($card_where) -> find();
			if (!$card_info) {
				$this -> error('请输入正确的卡号');
			}
			// 判断电话是否正确
			if ($erp_phone != $card_info['card_tel'] && $erp_phone != $card_info['card_bb']) {
				$this -> error('卡号与您预留的电话号码不一致');
			}
			...
			/**
			 * feixiang: 2015年10月14日 需求 
			 * 首次绑定成功后，增加200
			 */	
			// 判断是否是首次绑定,会员卡都有初始卡号	
			$is_bind = M('Member_card_create')->field("id")-> where(array('is_bind' => 1,'token' => $this -> token, 'wecha_id' => $this->wecha_id )) -> find();
			...
			if ( M('Member_card_create') -> where($where) -> save(array('is_bind' => 1, 'old_number' => $create['number'], 'number' => $number))) {
				...
				if( empty($is_bind) ){
					$model = M('Userinfo');
					$where_user  = array('token'=>$this->token,'wecha_id'=>$this->wecha_id);
            		$ret = $model->where($where_user)->setInc("total_score",$jf_first_add);
					if( $ret != FALSE ){
						$msg .= ",首次绑定送您{$jf_first_add}积分";
					}
				}
				
				// 2015年10月15日 新需求：马上同步积分
				$model -> db(1);
				$update_data['jf_history'] = $card_info["total_my"];
				$ret = $model -> db(1) -> table('memberinfo') -> where($card_where)->save($update_data);
				if( $ret != FALSE ){
						$msg .= ",初始化积分成功";
				}
...
//				var_dump($ret);
				...
				$this -> success("{$msg}", U('Card/index', array('token' => $this -> token, 'wecha_id' => $this -> wecha_id)));
			} else {
